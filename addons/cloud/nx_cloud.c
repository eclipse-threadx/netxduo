...
/* The ThreadX thread/timer entry input is a ULONG, which is 32-bit on the
   ...
   thread entry recovers the cloud control block from the running thread; the
   timer entry (which runs in the timer thread) reconstructs the pointer from
   this cached high-address base OR'd with the truncated low 32 bits.  On
   LP64/ILP32 platforms the base is zero, so behavior is unchanged. */
static ALIGN_TYPE _nx_cloud_control_block_base;
...
UINT old_threshold = 0;
    ...
    _nx_cloud_control_block_base = (ALIGN_TYPE)cloud_ptr & ~(ALIGN_TYPE)0xFFFFFFFFUL;
    status = tx_thread_create(&(cloud_ptr -> nx_cloud_thread), (CHAR*)cloud_name,
                              _nx_cloud_thread_entry, (ULONG)(ALIGN_TYPE)cloud_ptr,
    ...
    status = tx_timer_create(&(cloud_ptr -> nx_cloud_periodic_timer), (CHAR*)cloud_name,
                             _nx_cloud_periodic_timer_entry, (ULONG)(ALIGN_TYPE)cloud_ptr,
...
ULONG           module_own_events;
    ...
    /* The ThreadX thread entry input is a ULONG, which truncates a 64-bit
       pointer on the win64 simulator; recover the control block from the
       currently executing thread instead. */
    NX_PARAMETER_NOT_USED(cloud_ptr_value);
    cloud_ptr = (NX_CLOUD *)((UCHAR *)tx_thread_identify() -
                (ALIGN_TYPE)&(((NX_CLOUD *)0) -> nx_cloud_thread));
...
NX_CLOUD *cloud_ptr = (NX_CLOUD *)(_nx_cloud_control_block_base | (ALIGN_TYPE)cloud_ptr_value);
