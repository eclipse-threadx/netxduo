...
typedef struct NX_TCPSERVER_STRUCT
{
    ...
    /* Holds a back-pointer to the owning protocol server (e.g. NX_WEB_HTTP_SERVER).
       Must be pointer-width: ULONG is 32-bit on the win64 LLP64 simulator and would
       truncate a 64-bit pointer, so use ALIGN_TYPE. */
    ALIGN_TYPE              nx_tcpserver_reserved;
} NX_TCPSERVER;
