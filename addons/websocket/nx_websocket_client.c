...
/* Send a PONG carrying the supplied payload, in reply to a received PING.

   The client mutex is held on entry and is still held on return.  It is not dropped for the
   send: _nx_websocket_client_send() takes the mutex itself, and ThreadX mutexes are recursive,
   so the nested acquire and release leave this function's ownership intact.  Dropping it would
   open a window in which another thread could run _nx_websocket_client_cleanup() and release
   the packet the caller is still parsing.

   Every wait is NX_NO_WAIT so that no unbounded block is taken while the mutex is held.  A PONG
   is a courtesy reply -- losing one costs a keepalive round trip -- so resource pressure is
   reported to the caller rather than tearing the connection down.  */
static UINT _nx_websocket_client_pong_send(NX_WEBSOCKET_CLIENT *client_ptr, UCHAR *payload, ULONG payload_length)
{
UINT       status;
NX_PACKET *pong_packet;


    status = _nx_websocket_client_packet_allocate(client_ptr, &pong_packet, NX_NO_WAIT);
    if (status != NX_SUCCESS)
    {
        return(status);
    }
    ...
    if (payload_length > 0)
    {
        status = _nx_packet_data_append(pong_packet, payload, payload_length,
                                        client_ptr -> nx_websocket_client_packet_pool_ptr, NX_NO_WAIT);
        if (status != NX_SUCCESS)
        {
    ...
    status = _nx_websocket_client_send(client_ptr, pong_packet, NX_WEBSOCKET_OPCODE_PONG, NX_TRUE, NX_NO_WAIT);
    if (status != NX_SUCCESS)
    {

        /* Neither _nx_websocket_client_send() nor the TCP/TLS send releases the packet on
           failure, so ownership is still ours.  */
        _nx_packet_release(pong_packet);
    }
...
UCHAR is_control_frame = NX_FALSE;
    ...
    switch (opcode)
    {
        ...
        case NX_WEBSOCKET_OPCODE_BINARY_FRAME:
        {
            ...
            /* A zero-length data frame carries nothing to hand to the application, and
               trimming its header has already consumed the whole packet.  Keep parsing, which
               is what this function did for every empty frame before control frames were
               dispatched.  */
            if (*packet_ptr == NX_NULL)
            {
                client_ptr -> nx_websocket_client_frame_header_found = NX_FALSE;
        ...
        case NX_WEBSOCKET_OPCODE_PING:
        {
            ...
            /* Copy out the payload, since it must be echoed in the PONG, then trim it from the
               ...
               header has already consumed the whole packet and *packet_ptr is NX_NULL, so both
               steps are skipped.  */
            if (client_ptr -> nx_websocket_client_frame_data_length > 0)
            {
            UCHAR ping_payload[NX_WEBSOCKET_CONTROL_FRAME_PAYLOAD_MAXIMUM_LENGTH];

                status = _nx_packet_data_extract_offset(*packet_ptr, 0, ping_payload,
                                                       client_ptr -> nx_websocket_client_frame_data_length, &bytes_copied);

                /* The whole payload must have been copied: the extract reports a short copy
                   through bytes_copied rather than through the status.  */
                if ((status != NX_SUCCESS) || (bytes_copied != client_ptr -> nx_websocket_client_frame_data_length))
                {
                    _nx_websocket_client_cleanup(client_ptr);
                    return(NX_INVALID_PACKET);
                }
                ...
                status = _nx_websocket_client_pong_send(client_ptr, ping_payload,
                                                        client_ptr -> nx_websocket_client_frame_data_length);
            }
            else
            {
                status = _nx_websocket_client_pong_send(client_ptr, NX_NULL, 0);
            }

            /* A PONG is a courtesy reply: if it could not be sent the connection is still
               usable, so the frame is reported as consumed either way.  */
        ...
        case NX_WEBSOCKET_OPCODE_PONG:
        {
            ...
            /* Trim payload data in the frame.  A PONG may carry no payload, in which case
               trimming the header has already consumed the whole packet and there is nothing
               left to trim.  */
            if (client_ptr -> nx_websocket_client_frame_data_length > 0)
            {
                status = _nx_websocket_client_packet_trim(client_ptr, packet_ptr, client_ptr -> nx_websocket_client_frame_data_length);
