...
/* This is a small demo of the high-performance NetX Duo TCP/IP
   IPv4/IPv6 dual stack.  This demo concentrates on TCP connection,
   disconnection, sending, and receiving in IPv4 and IPv6 network
   in a simulated Ethernet driver in a multihome environment.  */
...
#ifndef NX_DISABLE_IPV6
/*
   Define the number of TCP tests.  If IPv6 is enabled, there are 4 tests:
   (1) TCP connection to the IPv4 address of the primary interface on ip_1;
   (2) TCP connection to the IPv4 address of the secondary interface on ip_1;
   (3) TCP connection to the IPv6 global  address of the primary interface on ip_1;
   (4) TCP connection to the IPv6 global address of the secondary interface on ip_1;
*/
#define NUMBER_OF_TESTS 4
#else /* !NX_DISABLE_IPV6 */
/*
   Define the number of TCP tests.  If IPv6 is enabled, there are 2 tests:
   (1) TCP connection to the IPv4 address of the primary interface on ip_1;
   (2) TCP connection to the IPv4 address of the secondary interface on ip_1;
*/
...
/*
   Define the number of TCP tests.  If IPv6 is enabled, there are 4 tests:
   (1) TCP connection to the IPv6 global  address of the primary interface on ip_1;
   (2) TCP connection to the IPv6 global address of the secondary interface on ip_1;
*/
#define NUMBER_OF_TESTS 2
#endif /* !NX_DISABLE_IPV4 */
...
#ifndef NX_DISABLE_IPV4
        case 0:
            status = nxd_tcp_client_socket_connect(&client_socket, &ip1_primary_ipv4_address, 12, NX_IP_PERIODIC_RATE);
            break;
        case 1:
            status = nxd_tcp_client_socket_connect(&client_socket, &ip1_secondary_ipv4_address, 12, NX_IP_PERIODIC_RATE);
            break;
#ifndef NX_DISABLE_IPV6
        case 2:
            status = nxd_tcp_client_socket_connect(&client_socket, &ip1_primary_global_address, 12, NX_IP_PERIODIC_RATE);
            break;
        case 3:
            status = nxd_tcp_client_socket_connect(&client_socket, &ip1_secondary_global_address, 12, NX_IP_PERIODIC_RATE);
            break;
#endif /* !NX_DISABLE_IPV6 */
#else /* !NX_DISABLE_IPV4  */
#ifndef NX_DISABLE_IPV6
