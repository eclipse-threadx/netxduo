...
/* The CertificateVerify signature is built and checked in scratch buffers that must not be shared
 * between TLS sessions: the handshake hash and the signature block belong to one session's
 * transcript, and two sessions signing or verifying at the same time would corrupt each other.
 * The buffers are therefore carved out of the application-supplied crypto metadata area at
 * session create time. Sending and processing a CertificateVerify never overlap within a single
 * session, so the send and process paths share the same area.
 */
