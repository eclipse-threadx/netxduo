...
ULONG  bytes_copied;
    ...
    /* Check that the record contains the complete MAC. */
    if (hash_size > *length)
    {
    ...
    /* TLS permits zero-length application data, but other record types must contain data. */
    if ((*length == hash_size) &&
        (header_data[0] != (UCHAR)(NX_SECURE_TLS_APPLICATION_DATA)))
    {
        return(NX_SECURE_TLS_HASH_MAC_VERIFY_FAILURE);
    }
