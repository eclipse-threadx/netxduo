...
NX_SECURE_EC_PUBLIC_KEY              *ec_pubkey;
    ...
    /* Point at this session's CertificateVerify scratch area. These buffers used to be file-scope
       statics, which two sessions verifying at the same time would overwrite for each other. They
       are carved out of the crypto metadata area at session create time, so they are per-session.
       Sending and processing a CertificateVerify never overlap within one session, so the send path
    ...
    decrypted_signature = &((UCHAR *)tls_session -> nx_secure_tls_certificate_verify_scratch)[NX_SECURE_TLS_CERTIFICATE_VERIFY_SIGNATURE_OFFSET];
#if (NX_SECURE_TLS_TLS_1_3_ENABLED)
    ...
    NX_SECURE_MEMSET(decrypted_signature, 0x0, NX_SECURE_TLS_CERTIFICATE_VERIFY_SIGNATURE_SIZE);
    ...
    if (client_certificate -> nx_secure_x509_public_algorithm == NX_SECURE_TLS_X509_TYPE_RSA)
    {
        ...
        /* The signature is decrypted into a buffer of a fixed size, so a modulus larger than that
           buffer cannot be verified. Reject it rather than writing past the end of the buffer. */
...
#ifdef NX_SECURE_KEY_CLEAR
                NX_SECURE_MEMSET(handshake_hash, 0, NX_SECURE_TLS_CERTIFICATE_VERIFY_HASH_SIZE);
                NX_SECURE_MEMSET(decrypted_signature, 0, NX_SECURE_TLS_CERTIFICATE_VERIFY_SIGNATURE_SIZE);
#endif /* NX_SECURE_KEY_CLEAR  */
                return(status);
            }
        }
...
#endif /* NX_SECURE_ENABLE_DTLS */
        {
            ...
            working_ptr = &decrypted_signature[data_size - signature_length];
