...
static TX_SEMAPHORE        semaphore_pause_received;
...
#else
void    netx_rtsp_delete_beforehand_test_application_define(void *first_unused_memory)
#endif
{
CHAR    *pointer;
UINT    status;
    ...
    tx_semaphore_create(&semaphore_pause_received, "semaphore teardown received", 0);
}
...
ULONG           content_length = 0;
    ...
    for ( i = 0; i < rtsp_request_num; i++)
    {
        ...
        /* Since the test for teardown command is changed, break after send the request. */
        if (i == (rtsp_request_num - 1))
        {

            tx_semaphore_get(&semaphore_pause_received, NX_WAIT_FOREVER);

            /* Delete the server before nx_rtsp_server_disconnect called.
            This test is to see if the server can be deleted normally. */
            nx_rtsp_server_delete(&rtsp_server);
            break;
        }
...
NX_PACKET *packet_ptr;
    ...
    printf("NetX Test:   RTSP RTP Delete Beforehand Test.......................................");
    ...
    /* Create RTSP server.  */
    status = nx_rtsp_server_create(&rtsp_server, "RTSP Server", sizeof("RTSP Server") - 1,&server_ip, &server_pool, rtsp_stack, DEMO_STACK_SIZE, 5, RTSP_SERVER_PORT, rtsp_disconnect_callback);
    CHECK_STATUS(0, status);
...
UINT status;
    ...
    tx_semaphore_put(&semaphore_pause_received);
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Test:   RTSP RTP Delete Beforehand Test.......................................N/A\n");
