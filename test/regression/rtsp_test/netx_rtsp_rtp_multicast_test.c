...
static UINT                connected_client_num = 0;
...
#define TEST_MULTICAST_ADDRESS  IP_ADDRESS(224, 1, 0, 55)
...
UCHAR           temp_string[256];
    ...
    status = nx_igmp_multicast_join(&client_ip_0, TEST_MULTICAST_ADDRESS);
    CHECK_STATUS(0, status);
    ...
    for ( i = 0; i < rtsp_request_num; i++)
    {
        ...
        *(packet_ptr -> nx_packet_append_ptr) = NX_NULL;
        ...
        else if (i == SETUP_0_INDEX)
        {
            ...
            sprintf(temp_string, "Transport: RTP/AVP;multicast;destination=224.1.0.55;port=%d-%d;ttl=%d",
...
UCHAR           temp_string[256];
    ...
    status = nx_igmp_multicast_join(&client_ip_1, TEST_MULTICAST_ADDRESS);
    CHECK_STATUS(0, status);
...
NXD_ADDRESS client_ip_address;
    ...
    status = nx_igmp_multicast_join(&server_ip, TEST_MULTICAST_ADDRESS);
    CHECK_STATUS(0, status);
    ...
    client_ip_address.nxd_ip_address.v4 = TEST_MULTICAST_ADDRESS;
