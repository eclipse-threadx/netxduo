...
extern UINT _nx_web_http_server_field_value_get(NX_PACKET *packet_ptr, UCHAR *field_name, ULONG name_length, UCHAR *field_value, ULONG field_value_size);
static CHAR *host_test = "express_logic";

#ifdef CTEST
VOID test_application_define(void *first_unused_memory)
#else
void    netx_web_host_field_test_application_define(void *first_unused_memory)
#endif
{
CHAR    *pointer;
UINT    status;
    ...
    for (i = 0; i < loop ; i++)
    {
        ...
        if (i == 0)
        {
            status = nx_web_http_client_get_start(&my_client, &server_ip_address,
                                                  ...
                                                  host_test, "name", "password", NX_WAIT_FOREVER);
        }
#ifdef NX_WEB_HTTPS_ENABLE
        else
        {
...
UINT            server_port = NX_WEB_HTTP_SERVER_PORT;
    ...
    printf("NetX Test:   Web Host Field Test.......................................");
...
CHAR         temp_string[20];
    ...
    if(request_type == NX_WEB_HTTP_SERVER_GET_REQUEST)
    {
        ...
        /* Get the Host field.  */
        if (_nx_web_http_server_field_value_get(packet_ptr, (UCHAR *)"Host", 4, (UCHAR *)temp_string, sizeof(temp_string)) == NX_SUCCESS)
        {
            if (memcmp(temp_string, host_test, strlen(host_test)) != 0)
            {
                error_counter++;
            }
        }
        ...
        status = nx_web_http_server_callback_generate_response_header(server_ptr,
                                                                      &response_pkt, NX_WEB_HTTP_STATUS_OK, 0, NX_NULL, 
                                                                      NX_NULL);
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Test:   Web Host Field Test.......................................N/A\n"); 
