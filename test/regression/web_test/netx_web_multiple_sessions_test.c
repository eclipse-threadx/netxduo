...
#else
void    netx_web_multiple_sessions_test_application_define(void *first_unused_memory)
#endif
{
CHAR    *pointer;
UINT    status;
...
NX_PACKET       *my_packet;
    ...
    for (i = 0; i < loop ; i++)
    {
        ...
        if (i == 0)
        {
            ...
            status = nx_web_http_client_connect(&my_client_1, &server_ip_address, NX_WEB_HTTP_SERVER_PORT, NX_WAIT_FOREVER);
        ...
        else
        {
            ...
            status = nx_web_http_client_secure_connect(&my_client_1, &server_ip_address, NX_WEB_HTTPS_SERVER_PORT, tls_setup_callback, NX_WAIT_FOREVER);
        ...
        /* Allocate a packet.  */
        status = nx_web_http_client_request_packet_allocate(&my_client_1, &my_packet, 1 * NX_IP_PERIODIC_RATE);
        if(status)
            error_counter++;

        /* Write Get packet into the packet payload.  */
        status = nx_packet_data_append(my_packet, &pkt[54] , (sizeof(pkt) - 54), &client_pool_1, 1 * NX_IP_PERIODIC_RATE);
        if(status)
            error_counter++;

        /* Send the packet out.  */
        status = _nx_web_http_client_send(&my_client_1, my_packet, 1 * NX_IP_PERIODIC_RATE);
        if(status)
        {
            nx_packet_release(my_packet);
            error_counter++;
...
UINT            server_port = NX_WEB_HTTP_SERVER_PORT;
    ...
    printf("NetX Test:   Web Multiple Sessions Test................................");
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Test:   Web Multiple Sessions Test................................N/A\n"); 
