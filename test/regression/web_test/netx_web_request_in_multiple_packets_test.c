...
#else
void    netx_web_request_in_multiple_packets_test_application_define(void *first_unused_memory)
#endif
{
CHAR    *pointer;
UINT    status;
...
CHAR            *buffer_ptr;
    ...
    for (i = 0; i < loop ; i++)
    {
        ...
        if(status)
            error_counter++;
        status = nx_packet_data_append(my_packet, "GET /index.htm HTTP/1.0" , 23, &client_pool, 1 * NX_IP_PERIODIC_RATE);
        if(status)
            error_counter++;
        ...
        if(status)
            error_counter++;
        status = nx_packet_data_append(my_packet, "\r" , 1, &client_pool, 1 * NX_IP_PERIODIC_RATE);
        if(status)
            error_counter++;
        status = _nx_web_http_client_send(&my_client, my_packet, NX_WAIT_FOREVER);
        if(status)
...
UINT            server_port = NX_WEB_HTTP_SERVER_PORT;
    ...
    printf("NetX Test:   Web Request in Multiple Packets Test......................");
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Test:   Web Request in Multiple Packets Test......................N/A\n"); 
