/* This case tests reconnection when TLS connection fail. */
#include    "tx_api.h"
#include    "nx_api.h"
#include    "fx_api.h"
...
static UINT tls_setup_callback(NX_SECURE_TLS_SESSION *tls_session);
...
#else
void    netx_web_secure_reconnect_test_application_define(void *first_unused_memory)
#endif
{
CHAR    *pointer;
UINT    status;
...
UINT status;
    ...
    ca_cert_der[0] = ca_cert_der[0] + 1; /* Modify the cert data to make TLS connection fail. */
...
NX_SECURE_TLS_SESSION client_tls_session;
    ...
    for (i = 0; i < loop; i++)
    {
        status = nxd_tcp_client_socket_connect(&client_socket, &server_ip_address, NX_WEB_HTTPS_SERVER_PORT, NX_IP_PERIODIC_RATE);
        if (status)
            error_counter++;

        tls_setup_callback(&client_tls_session);
...
UINT            server_port = NX_WEB_HTTP_SERVER_PORT;
    ...
    printf("NetX Test:   Web Secure Reconnect Test.................................");
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Test:   Web Secure Reconnect Test.................................N/A\n"); 
