/* This case tests multiple sessions. */
#include    "tx_api.h"
#include    "nx_api.h"
#include    "fx_api.h"
...
static NX_WEB_HTTP_CLIENT  my_client_1;
...
#define HTTP_CLIENT_ADDRESS_1  IP_ADDRESS(1,2,3,6)
...
static CHAR crypto_metadata_client[20000];
static CHAR crypto_metadata_client_1[20000];
static UCHAR tls_packet_buffer[18500];
static UCHAR tls_packet_buffer_1[18500];
static NX_SECURE_X509_CERT certificate, trusted_certificate;
...
static NX_TCP_SOCKET test_client_socket;
...
#else
void    netx_web_multiple_sessions_timeout_test_application_define(void *first_unused_memory)
#endif
{
CHAR    *pointer;
UINT    status;
    ...
    /* Create the Client packet pool.  */
    status =  nx_packet_pool_create(&client_pool_1, "HTTP Client Packet Pool 1", CLIENT_PACKET_SIZE, 
                                    pointer, CLIENT_PACKET_SIZE*8);
    pointer = pointer + CLIENT_PACKET_SIZE * 8;
    if (status)
        error_counter++;
    ...
    /* Create an IP instance.  */
    status = nx_ip_create(&client_ip_1, "HTTP Client IP 1", HTTP_CLIENT_ADDRESS_1, 
...
/* Define the TLS setup callback function.  */
static UINT tls_setup_callback_1(NX_WEB_HTTP_CLIENT *client_ptr, NX_SECURE_TLS_SESSION *tls_session)
{
UINT status;


    /* Initialize and create TLS session.  */
    status = nx_secure_tls_session_create(tls_session, &nx_crypto_tls_ciphers, crypto_metadata_client_1, sizeof(crypto_metadata_client_1));
    ...
    /* Allocate space for packet reassembly.  */
    status = nx_secure_tls_session_packet_buffer_set(&(client_ptr -> nx_web_http_client_tls_session), tls_packet_buffer_1, sizeof(tls_packet_buffer_1));
...
NX_PACKET       *recv_packet;
    ...
    status = nx_tcp_socket_create(&client_ip, &test_client_socket, "test client socket", 
                                   NX_WEB_HTTP_TYPE_OF_SERVICE,  NX_WEB_HTTP_FRAGMENT_OPTION, NX_WEB_HTTP_TIME_TO_LIVE,
                                   1536, NX_NULL, NX_NULL);
    if (status)
        error_counter++;

    status = nx_tcp_client_socket_bind(&test_client_socket, NX_ANY_PORT, NX_WAIT_FOREVER);
    if (status)
    {
        error_counter++;
    }

    status = nxd_tcp_client_socket_connect(&test_client_socket, &server_ip_address, NX_WEB_HTTPS_SERVER_PORT, NX_WAIT_FOREVER);
    if (status)
    {
        error_counter++;
    }
    ...
    status += nx_web_http_client_create(&my_client_1, "HTTP Client 1", &client_ip_1, &client_pool_1, 1536);
    ...
    status = nx_web_http_client_get_secure_start_extended(&my_client_1, &server_ip_address,
                                                            NX_WEB_HTTPS_SERVER_PORT, "https://1.2.3.4/test.txt", sizeof("https://1.2.3.4/test.txt") - 1, 
                                                            "1.2.3.4", 7, "name", 4, "password", 8, 
                                                            tls_setup_callback_1, NX_WAIT_FOREVER);
    if (status)
        error_counter++;
    ...
    while(1)
    {
        status = nx_web_http_client_response_body_get(&my_client_1, &recv_packet, 1 * NX_IP_PERIODIC_RATE);
    ...
    status = nx_web_http_client_delete(&my_client_1);
    if (status)
        error_counter++;
...
UINT            server_port = NX_WEB_HTTPS_SERVER_PORT;
    ...
    printf("NetX Test:   Web Multiple Sessions Timeout Test........................");
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Test:   Web Multiple Sessions Timeout Test........................N/A\n"); 
