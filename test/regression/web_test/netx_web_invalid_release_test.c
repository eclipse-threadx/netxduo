/* This case tests sending packet failure. */
#include    "tx_api.h"
#include    "nx_api.h"
#include    "fx_api.h"
...
#else
void    netx_web_invalid_release_test_application_define(void *first_unused_memory)
#endif
{
CHAR    *pointer;
UINT    status;
...
NX_PACKET       *recv_packet;
    ...
    for (i = 0; i < loop ; i++)
    {
        ...
        if (i == 0)
        {
            ...
            status = nx_web_http_client_put_start_extended(&my_client, &server_ip_address,
                                                           NX_WEB_HTTP_SERVER_PORT, "http://1.2.3.4/http_put_test2.html", 34,
                                                           "www.abc.com", 11, "name", 4, "password", 8, 103, NX_WAIT_FOREVER);
        }
#ifdef NX_WEB_HTTPS_ENABLE
        else
        {

            status = nx_web_http_client_put_secure_start_extended(&my_client, &server_ip_address,
                                                                  NX_WEB_HTTPS_SERVER_PORT, "https://1.2.3.4/https_put_test2.html", 36,
                                                                  "www.abc.com", 11, "name", 4, "password", 8, 103, tls_setup_callback, NX_WAIT_FOREVER);
        }
#endif /* NX_WEB_HTTPS_ENABLE  */
        ...
        if (i == 1)
        {
            send_packet -> nx_packet_prepend_ptr = send_packet -> nx_packet_data_start;
            send_packet -> nx_packet_append_ptr = send_packet -> nx_packet_prepend_ptr;
        }

        /* Send the an empty packet.  */
        status = nx_web_http_client_request_packet_send(&my_client, send_packet, NX_FALSE, 1 * NX_IP_PERIODIC_RATE);
        if (status)
        {
            nx_packet_release(send_packet);
        }
...
UINT            server_port = NX_WEB_HTTP_SERVER_PORT;
    ...
    printf("NetX Test:   Web Invalid Release Test..................................");
    ...
    if (client_pool.nx_packet_pool_invalid_releases || server_pool.nx_packet_pool_invalid_releases)
    {
        error_counter++;
    }
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Test:   Web Invalid Release Test..................................N/A\n"); 
