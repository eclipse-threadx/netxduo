/* This case tests chunked resposne.  */
#include    "tx_api.h"
#include    "nx_api.h"
#include    "fx_api.h"
...
#else
void    netx_web_chunked_response_test_application_define(void *first_unused_memory)
#endif
{
CHAR    *pointer;
UINT    status;
...
UINT            total_length = 0;
    ...
    for (i = 0; i < loop ; i++)
    {
        ...
        status = nx_web_http_client_request_initialize(&my_client, NX_WEB_HTTP_METHOD_GET, "/index.htm", "www.abc.com", 
        ...
        /* Check status.  */
        if ((status != NX_WEB_HTTP_GET_DONE) || (content_length != total_length))
            error_counter++;
...
UINT            server_port = NX_WEB_HTTP_SERVER_PORT;
    ...
    printf("NetX Test:   Web Chunked Response Test.................................");
...
UINT         i = 0;
    ...
    if(request_type == NX_WEB_HTTP_SERVER_GET_REQUEST)
    {
        ...
        for (i = 0; i < test_num; i++)
        {
            ...
            status = nx_web_http_server_response_chunked_set(server_ptr, test_chunk[i], response_pkt);
            if (status)
            {
                return(status);
            }

            nx_packet_data_append(response_pkt, pkt, test_chunk[i], server_ptr -> nx_web_http_server_packet_pool_ptr, NX_WAIT_FOREVER);
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Test:   Web Chunked Response Test.................................N/A\n");
