/* This case tests HTTP/1.1 server establish a persistent connection with a HTTP/1.1 client. */
#include    "tx_api.h"
#include    "nx_api.h"
#include    "fx_api.h"
...
#if !defined(NX_DISABLE_IPV4) && !defined(NX_WEB_HTTP_KEEPALIVE_DISABLE)
...
#else
void    netx_web_keep_alive_test_application_define(void *first_unused_memory)
#endif
{
CHAR    *pointer;
UINT    status;
...
{
    if(memcmp(field_name, "Connection", field_name_length) == 0)
    {
        if ((test_count == 0) && (memcmp(field_value, "keep-alive", field_value_length) != 0))
            error_counter++;
        else if((test_count == 1) && (memcmp(field_value, "Close", field_value_length) != 0))
            error_counter++;
    }
}
...
NX_PACKET       *recv_packet;
    ...
    for (i = 0; i < loop ; i++)
    {
        ...
        /* First send GET with "connection: keep-alive", the connection should be kept alive.
           Then send GET with "connection: close", the connection should be colsed.
           If the connection is closed, the third request sending should be failed.  */
        for (test_count = 0; test_count < 3; test_count++)
        {
            ...
            /* For HTTPS, if the connection is closed, the initialization should be failed.  */
            if ((i == 1) && (test_count == 2))
            {
                if (!status)
                    error_counter++;
            ...
            if (test_count == 0)
            {
                status = nx_web_http_client_request_header_add(&my_client, "Connection", 10, "keep-alive", 10, 1 * NX_IP_PERIODIC_RATE);
            }
            ...
            /* Send the request.  */
            status = nx_web_http_client_request_send(&my_client, 1 * NX_IP_PERIODIC_RATE);
...
UINT            server_port = NX_WEB_HTTP_SERVER_PORT;
    ...
    printf("NetX Test:   Web Keep Alive Test.......................................");
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Test:   Web Keep Alive Test.......................................N/A\n"); 
