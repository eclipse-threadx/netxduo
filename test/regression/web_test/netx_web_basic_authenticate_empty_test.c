/* This case tests basic authentication with empty name or password. */
#include    "tx_api.h"
#include    "nx_api.h"
#include    "fx_api.h"
...
#else
void    netx_web_basic_authenticate_empty_test_application_define(void *first_unused_memory)
#endif
{
CHAR    *pointer;
UINT    status;
    ...
    for (i = 0; i < loop ; i++)
    {
        ...
        if (i == 0)
        {
            status = nx_web_http_client_get_start(&my_client, &server_ip_address,
                                                  ...
                                                  "1.2.3.4", "Placeholdername", "", NX_WAIT_FOREVER);
        }
#ifdef NX_WEB_HTTPS_ENABLE
        else
        {
        ...
        /* Check status.  */
        if (status != NX_WEB_HTTP_AUTHENTICATION_ERROR)
            error_counter++;
        ...
        if (i == 0)
        {
            status = nx_web_http_client_get_start(&my_client, &server_ip_address,
                                                  NX_WEB_HTTP_SERVER_PORT, "http://1.2.3.4/test.txt",
                                                  "1.2.3.4", "", "Placeholderpassword", NX_WAIT_FOREVER);
        }
#ifdef NX_WEB_HTTPS_ENABLE
        else
        {
...
UINT            server_port = NX_WEB_HTTP_SERVER_PORT;
    ...
    printf("NetX Test:   Web Basic Authenticate Empty Test.........................");
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Test:   Web Basic Authenticate Empty Test.........................N/A\n"); 
