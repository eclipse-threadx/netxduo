/* This case tests abnormal cases. */
#include    "tx_api.h"
#include    "nx_api.h"
#include    "fx_api.h"
...
#else
void    netx_web_abnormal_test_application_define(void *first_unused_memory)
#endif
{
CHAR    *pointer;
UINT    status;
...
NX_PACKET       *recv_packet;
    ...
    for (i = 0; i < loop ; i++)
    {
        ...
        /* Abnormal case: Request packet is NULL, should return error.  */
        status = nx_web_http_client_request_header_add(&my_client, "", 0, "", 0, NX_NO_WAIT);
        ...
        /* Abnormal case: Request packet isn't sent, should return error.  */
        ...
        status = nx_web_http_client_request_initialize(&my_client, NX_WEB_HTTP_METHOD_GET, "/test.txt", "1.2.3.4", 
                                                       0, NX_FALSE, "", "", NX_WAIT_FOREVER);

        /* Check status.  */
        if (status)
            error_counter++;

        /* Abnorml case: Duplicate initialize, the old request packet should be released.  */
        status = nx_web_http_client_request_initialize(&my_client, NX_WEB_HTTP_METHOD_GET, "/test.txt", "1.2.3.4", 
                                                       0, NX_FALSE, "", "", NX_WAIT_FOREVER);
       ...
       /* Get response from server.  */
        ...
        /* Abnorml case: Error basic authorization field.  */
        status = nx_web_http_client_request_initialize(&my_client, NX_WEB_HTTP_METHOD_GET, "/test.txt", "1.2.3.4", 
        ...
        /* Add error basic authorization field. The correct string should be "Basic Og==".  */
        status = nx_web_http_client_request_header_add(&my_client, "Authorization", 13, "Bassc OgOg", 10, NX_NO_WAIT);
        if (status)
            error_counter++;
       ...
       /* Get response from server.  */
        ...
        /* Check status.  */
        if (status != NX_WEB_HTTP_STATUS_CODE_UNAUTHORIZED)
            error_counter++;
...
UINT            server_port = NX_WEB_HTTP_SERVER_PORT;
    ...
    printf("NetX Test:   Web Abnormal Test.........................................");
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Test:   Web Abnormal Test.........................................N/A\n"); 
