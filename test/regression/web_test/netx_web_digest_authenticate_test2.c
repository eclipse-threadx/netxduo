...
static UINT  authentication_check_extended(NX_WEB_HTTP_SERVER *server_ptr, UINT request_type, CHAR *resource, CHAR **name, UINT *name_length,
...
#else
void    netx_web_digest_authenticate_test_2_application_define(void *first_unused_memory)
#endif
{
CHAR    *pointer;
UINT    status;
...
CHAR            *buffer_ptr;
    ...
    status =  nx_tcp_socket_create(&client_ip, &client_socket, "Socket 0", 
    ...
    for (test_count = 0; test_count < 2; test_count++)
    {
        ...
        status = nx_tcp_socket_receive(&client_socket, &recv_packet, NX_WAIT_FOREVER);
        ...
        nx_packet_data_append(send_packet, nonce_buffer, NX_WEB_HTTP_SERVER_NONCE_SIZE, &client_pool, NX_WAIT_FOREVER);
        ...
        nx_tcp_socket_disconnect(&client_socket, NX_WEB_HTTP_CLIENT_TIMEOUT);
...
static UINT digest_authenticate_callback(NX_WEB_HTTP_SERVER *server_ptr, CHAR *name_ptr,
...
FX_FILE         my_file;
    ...
    printf("NetX Test:   Web Digest Authenticate Test2.............................");
    ...
    nx_web_http_server_digest_authenticate_notify_set(&my_server, digest_authenticate_callback);
    ...
    status = nx_web_http_server_authentication_check_set(&my_server, authentication_check_extended);
    if (status)
        error_counter++;
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Test:   Web Digest Authenticate Test2.............................N/A\n"); 
