...
#else
void    netx_web_client_receive_no_packet_test_application_define(void *first_unused_memory)
#endif
{
CHAR    *pointer;
UINT    status;
...
NX_PACKET       *recv_packet;
    ...
    for (i = 0; i < loop ; i++)
    {
        ...
        /* Continue to get response from server.  */
        while (1)
        {
            status = nx_web_http_client_response_body_get(&my_client, &recv_packet, 2 * NX_IP_PERIODIC_RATE);
...
UINT            server_port = NX_WEB_HTTP_SERVER_PORT;
    ...
    printf("NetX Test:   Web Client Receive No Packet Test.........................");
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Test:   Web Client Receive No Packet Test.........................N/A\n"); 
