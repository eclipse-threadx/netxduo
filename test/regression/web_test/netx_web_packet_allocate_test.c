/* This case tests if the allocated packet is 4-byte alignment. */
#include    "tx_api.h"
#include    "nx_api.h"
#include    "fx_api.h"
...
static UINT driver_packet_check(NX_IP *ip_ptr, NX_PACKET *packet_ptr);
extern UINT (*packet_process_callback)(NX_IP *ip_ptr, NX_PACKET *packet_ptr);

#ifdef CTEST
VOID test_application_define(void *first_unused_memory)
#else
void    netx_web_packet_allocate_test_application_define(void *first_unused_memory)
#endif
{
CHAR    *pointer;
UINT    status;
...
UINT            server_port = NX_WEB_HTTP_SERVER_PORT;
    ...
    printf("NetX Test:   Web Packet Allocate Test..................................");
    ...
    packet_process_callback = driver_packet_check;
...
UINT         status;
CHAR        *content_buffer = "NETX packet allocate test";
UINT         content_length = strlen(content_buffer);
    ...
    if(request_type == NX_WEB_HTTP_SERVER_GET_REQUEST)
    {
        ...
        status = nx_web_http_server_callback_generate_response_header(server_ptr,
                                                                      &response_pkt, NX_WEB_HTTP_STATUS_OK, content_length * 2, "text/html", 
        ...
        nx_packet_data_append(response_pkt, content_buffer, content_length, server_ptr -> nx_web_http_server_packet_pool_ptr, NX_WAIT_FOREVER);
        ...
        status = nx_web_http_server_callback_data_send(server_ptr, content_buffer, content_length);
        if (status)
        {
            return(status);
        }
...
/* Define the packet check function.  */
static UINT driver_packet_check(NX_IP *ip_ptr, NX_PACKET *packet_ptr)
{

    /* Check if the packet is four-byte alignment.  */
    if ((UINT)(packet_ptr -> nx_packet_prepend_ptr) & 0x3)
    {
        error_counter++;
    }
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Test:   Web Packet Allocate Test..................................N/A\n"); 
