/* This case tests basic authentication. */
#include    "tx_api.h"
#include    "nx_api.h"
#include    "fx_api.h"
...
#else
void    netx_web_basic_authenticate_test_application_define(void *first_unused_memory)
#endif
{
CHAR    *pointer;
UINT    status;
...
NX_PACKET       *recv_packet;
    ...
    for (i = 0; i < loop ; i++)
    {
        ...
        if (i == 0)
        {
            status = nx_web_http_client_get_start(&my_client, &server_ip_address,
                                                  NX_WEB_HTTP_SERVER_PORT, "http://1.2.3.4/test.txt",
                                                  "1.2.3.4", "name2", "password", NX_WAIT_FOREVER);
        }
#ifdef NX_WEB_HTTPS_ENABLE
        else
        {
...
UINT            server_port = NX_WEB_HTTP_SERVER_PORT;
    ...
    printf("NetX Test:   Web Basic Authenticate Test...............................");
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Test:   Web Basic Authenticate Test...............................N/A\n"); 
