...
#else
void    netx_web_head_basic_test_application_define(void *first_unused_memory)
#endif
{
CHAR    *pointer;
UINT    status;
...
NX_PACKET       *recv_packet;
    ...
    for (i = 0; i < loop ; i++)
    {
        ...
        if (i == 0)
        {
            status = nx_web_http_client_head_start(&my_client, &server_ip_address, NX_WEB_HTTP_SERVER_PORT,
                                                   "/index.htm", "www.abc.com", NX_NULL, NX_NULL,
                                                     NX_WAIT_FOREVER);
        }
        else if (i == 1)
        {
            status = nx_web_http_client_head_start_extended(&my_client, &server_ip_address, NX_WEB_HTTP_SERVER_PORT,
        ...
        {
            status = nx_web_http_client_head_secure_start(&my_client, &server_ip_address, NX_WEB_HTTPS_SERVER_PORT,
                                                          "/index.htm", "www.abc.com", NX_NULL, NX_NULL,
                                                          tls_setup_callback, NX_WAIT_FOREVER);
        }
        else
        {
            status = nx_web_http_client_head_secure_start_extended(&my_client, &server_ip_address, NX_WEB_HTTPS_SERVER_PORT,
                                                                   "/index.htm", 10, "www.abc.com", 11, NX_NULL, 0, NX_NULL, 0,
                                                                   tls_setup_callback, NX_WAIT_FOREVER);
        }
#endif /* NX_WEB_HTTPS_ENABLE  */
...
UINT            server_port = NX_WEB_HTTP_SERVER_PORT;
    ...
    printf("NetX Test:   Web Head Basic Test.......................................");
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Test:   Web Head Basic Test.......................................N/A\n"); 
