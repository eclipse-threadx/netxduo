...
static NX_TCP_SOCKET       client_socket[NX_WEB_HTTP_SERVER_NONCE_MAX + 1];
...
static CHAR temp_nonce_buffer[NX_WEB_HTTP_SERVER_NONCE_SIZE + 1];
...
#else
void    netx_web_digest_authenticate_timeout_test_application_define(void *first_unused_memory)
#endif
{
CHAR    *pointer;
UINT    status;
...
{
UINT            status;
NX_PACKET       *send_packet[NX_WEB_HTTP_SERVER_NONCE_MAX + 1];
NX_PACKET       *send_packet_replay;
...
INT             i;
    ...
    for (i = 0; i < NX_WEB_HTTP_SERVER_NONCE_MAX + 1; i++)
    {
        ...
        else
        {
            ...
            if (i == NX_WEB_HTTP_SERVER_NONCE_MAX)
            {
                ...
                tx_thread_sleep(NX_WEB_HTTP_SERVER_NONCE_TIMEOUT + NX_IP_PERIODIC_RATE);
        ...
        nx_packet_data_append(send_packet[i], nonce_buffer, NX_WEB_HTTP_SERVER_NONCE_SIZE, &client_pool, NX_WAIT_FOREVER);
    ...
    /* Copy for replay test.  */
    nx_packet_copy(send_packet[1], &send_packet_replay, &client_pool, NX_WAIT_FOREVER);

    for (i = NX_WEB_HTTP_SERVER_NONCE_MAX; i >= 0; i--)
    {
        ...
        if (i != 1)
        {
            ...
            status = nx_tcp_socket_disconnect(&client_socket[i], NX_WEB_HTTP_CLIENT_TIMEOUT);
    ...
    /* Use the copied packet for replay test.  */
    /* Create an HTTP client instance.  */
    status =  nx_tcp_socket_create(&client_ip, &client_socket[0], "Socket 0", 
                                    NX_IP_NORMAL, NX_FRAGMENT_OKAY, NX_IP_TIME_TO_LIVE, 200,
                                    NX_NULL, NX_NULL);
    ...
    /* Bind the client socket.  */
    status = nx_tcp_client_socket_bind(&client_socket[0], NX_ANY_PORT, NX_WAIT_FOREVER);
    ...
    /* Connect to the HTTP server.  */
    status = nxd_tcp_client_socket_connect(&client_socket[0], &server_ip_address,
                                            NX_WEB_HTTP_SERVER_PORT, NX_WAIT_FOREVER);
    ...
    /* Now send the packet to the HTTP server.  */
    status = nx_tcp_socket_send(&client_socket[0], send_packet_replay, NX_WAIT_FOREVER);
    ...
    /* Pickup the response from the Server.  */
    status = nx_tcp_socket_receive(&client_socket[0], &recv_packet, NX_WAIT_FOREVER);
    ...
    else
    {
        ...
        /* This is a replay attack, the response should be 401. */
        if ((buffer_ptr[9] != '4') || (buffer_ptr[10] != '0') || (buffer_ptr[11] != '1'))
            error_counter++;
...
FX_FILE         my_file;
    ...
    printf("NetX Test:   Web Digest Authenticate Timeout Test......................");
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Test:   Web Digest Authenticate Timeout Test......................N/A\n"); 
