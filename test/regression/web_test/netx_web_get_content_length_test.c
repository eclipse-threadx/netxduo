...
static char get_response_packet[130] = {
0x48, 0x54,                                     /* ......HT */
...
static int get_response_size = 130;

static char get_response_packet_nospace[129] = {
0x48, 0x54,                                     /* ......HT */
0x54, 0x50, 0x2f, 0x31, 0x2e, 0x30, 0x20, 0x32, /* TP/1.0 2 */
0x30, 0x30, 0x20, 0x0d, 0x0a, 0x43, 0x6f, 0x6e, /* 00 ..Con */
0x74, 0x65, 0x6e, 0x74, 0x2d, 0x4c, 0x65, 0x6e, /* tent-Len */
...
static int get_response_packet_nospace_size = 129;

static char get_response_packet_3spaces[132] = {
0x48, 0x54,                                     /* ......HT */
0x54, 0x50, 0x2f, 0x31, 0x2e, 0x30, 0x20, 0x32, /* TP/1.0 2 */
0x30, 0x30, 0x20, 0x0d, 0x0a, 0x43, 0x6f, 0x6e, /* 00 ..Con */
0x74, 0x65, 0x6e, 0x74, 0x2d, 0x4c, 0x65, 0x6e, /* tent-Len */
...
static int get_response_packet_3spaces_size = 132;
...
static char get_response_packet_error[10] = {
0x48, 0x54,                                     /* ......HT */
0x54, 0x50, 0x2f, 0x31, 0x0d, 0x0a, 0x0d, 0x0a, /* TP/1.0 2 */
};
static int get_response_packet_error_size = 10;
...
static UINT                packet_number = 0;

#ifdef CTEST
VOID test_application_define(void *first_unused_memory)
#else
void    netx_web_get_content_length_test_application_define(void *first_unused_memory)
#endif
{
CHAR    *pointer;
UINT    status;
...
NX_PACKET       *recv_packet;
    ...
    for (i = 0; i < loop ; i++)
    {
        ...
        for (j = 0; j < RESPONSE_COUNT; j++)
        {
            ...
            /* Check status.  */
            if ((status != NX_SUCCESS) && (status != NX_WEB_HTTP_GET_DONE) && (j < 3))
                error_counter++;
            else if (((status == NX_SUCCESS) || (status == NX_WEB_HTTP_GET_DONE)) && (j > 3))
                error_counter++;
...
UINT            server_port = NX_WEB_HTTP_SERVER_PORT;
    ...
    printf("NetX Test:   Web Get Content-Length Test...............................");
...
static void http_test_initialize()
{
    ...
    http_response[4].http_response_pkt_data = &get_response_packet_error[0];
    http_response[4].http_response_pkt_size = get_response_packet_error_size;
...
UINT         status;
    ...
    if(request_type == NX_WEB_HTTP_SERVER_GET_REQUEST)
    {
        ...
        status = nx_packet_data_append(response_pkt, http_response[packet_number].http_response_pkt_data, http_response[packet_number].http_response_pkt_size, 
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Test:   Web Get Content-Length Test...............................N/A\n"); 
