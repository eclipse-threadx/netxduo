/* This case tests chunked request.  */
#include    "tx_api.h"
#include    "nx_api.h"
#include    "fx_api.h"
...
static UINT test_chunk[] = {20, 120, 19, 34};
static UINT test_num = sizeof(test_chunk)/sizeof(UINT);

#ifdef CTEST
VOID test_application_define(void *first_unused_memory)
#else
void    netx_web_chunked_request_additional_test_application_define(void *first_unused_memory)
#endif
{
CHAR    *pointer;
UINT    status;
...
UINT            i, j;
UINT            status;
NX_PACKET      *send_packet, *recv_packet;
UINT            chunk_size = 0, more_data = NX_FALSE;
CHAR           *put_test;
    ...
    for (i = 0; i < loop ; i++)
    {
        ...
        status = nx_web_http_client_request_initialize(&my_client, NX_WEB_HTTP_METHOD_PUT, put_test, "www.abc.com", 
                                                       0, NX_TRUE, "name", "password", NX_WAIT_FOREVER);
        ...
        /* For test, append chunked data to the header packet.  */
        nx_packet_data_append(my_client.nx_web_http_client_request_packet_ptr, "\r\n19\r\n", 6, my_client.nx_web_http_client_packet_pool_ptr, NX_WAIT_FOREVER);
        nx_packet_data_append(my_client.nx_web_http_client_request_packet_ptr, pkt, 25, my_client.nx_web_http_client_packet_pool_ptr, NX_WAIT_FOREVER);
#endif
        ...
        for (j = 0; j < test_num; j++)
        {
            ...
            status = nx_web_http_client_request_chunked_set(&my_client, chunk_size, send_packet);
            if (status)
                error_counter++;

            nx_packet_data_append(send_packet, pkt, chunk_size, my_client.nx_web_http_client_packet_pool_ptr, NX_WAIT_FOREVER);

            status = nx_web_http_client_request_packet_send(&my_client, send_packet, more_data, 1 * NX_IP_PERIODIC_RATE);
            if (status)
            {
        ...
        /* Initialize the request.  */
        status = nx_web_http_client_request_initialize(&my_client, NX_WEB_HTTP_METHOD_POST, put_test, "www.abc.com", 
                                                       0, NX_TRUE, "name", "password", NX_WAIT_FOREVER);
        ...
        for (j = 0; j < test_num; j++)
        {
            ...
            /* Test one chunk in multiple request packets.  */
            if (j == 1)
            {
                status = nx_web_http_client_request_packet_allocate(&my_client, &send_packet, NX_WAIT_FOREVER);
                if (status)
...
UINT            server_port = NX_WEB_HTTP_SERVER_PORT;
    ...
    printf("NetX Test:   Web Chunked Request Additional Test.......................");
...
UINT         total_length = 0;
    ...
    if (request_type == NX_WEB_HTTP_SERVER_POST_REQUEST)
    {
        ...
        {
            total_length += test_chunk[i];
        }

        status = nx_web_http_server_packet_content_find(server_ptr, &packet_ptr, (ULONG *)&content_length);
        ...
        while (1)
        {
            status = nx_web_http_server_packet_get(server_ptr, &chunked_pkt);
            if (status)
            {
                break;
            }
            ...
            {
                content_length += chunked_pkt -> nx_packet_length;
                nx_packet_release(chunked_pkt);
            }
        }
       ...
       /* Generate HTTP header.  */
        status = nx_web_http_server_callback_generate_response_header_extended(server_ptr, &response_pkt, 
                                                                               NX_WEB_HTTP_STATUS_OK, sizeof(NX_WEB_HTTP_STATUS_OK) - 1, 0, 
                                                                               "text/html", 9, NX_NULL, 0);
        if (status)
        {
            return(status);
        }
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Test:   Web Chunked Request Additional Test.......................N/A\n");
