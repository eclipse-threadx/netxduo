/* This case tests client certificate verification. */
#include    "tx_api.h"
#include    "nx_api.h"
#include    "fx_api.h"
...
#if !defined(NX_DISABLE_IPV4) && defined(NX_WEB_HTTPS_ENABLE) && defined(NX_SECURE_ENABLE_CLIENT_CERTIFICATE_VERIFY)
...
static NX_SECURE_X509_CERT trusted_certificate, client_certificate;
...
static NX_SECURE_X509_CERT server_trusted_certificates[2];
static NX_SECURE_X509_CERT server_remote_certificates[2];
static UCHAR server_remote_cert_buffer[2000 * 2];
...
#else
void    netx_web_certifiacet_verify_test_application_define(void *first_unused_memory)
#endif
{
CHAR    *pointer;
UINT    status;
...
UINT status;
    ...
    /* Add certificate for client certificate verification.  */
    ...
    nx_secure_tls_local_certificate_add(&(client_ptr -> nx_web_http_client_tls_session), &client_certificate);
...
NX_SECURE_X509_CERT *server_trusted_certificates_list[2];
NX_SECURE_X509_CERT *server_remote_certificates_list[2];


    /* Print out test information banner.  */
    printf("NetX Test:   Web Certificate Verify Test...............................");
    ...
    nx_secure_x509_certificate_initialize(&server_trusted_certificates[0], ca_cert_der, ca_cert_der_len, NX_NULL, 0, NULL, 0, NX_SECURE_X509_KEY_TYPE_NONE);
    server_trusted_certificates_list[0] = &server_trusted_certificates[0];
    server_remote_certificates_list[0] = &server_remote_certificates[0];
    server_remote_certificates_list[1] = &server_remote_certificates[1];
    ...
    status = nx_web_http_server_secure_configure(&my_server, &nx_crypto_tls_ciphers,
                                                 ...
                                                 &certificate, server_trusted_certificates_list, 1, server_remote_certificates_list, 2, server_remote_cert_buffer, sizeof(server_remote_cert_buffer));
    if (status)
        error_counter++;
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Test:   Web Certificate Verify Test...............................N/A\n"); 
