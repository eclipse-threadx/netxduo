/* 
   ...
   The POP3 'server' is a TCP socket that sends responses and downloads a mail item
   of three packets.  The third packet of the second mail item only contains the POP#
   end marker so the bytes_received should be zero.
 */
...
static char last_data_packet2[5] = {
 0x0d, 0x0a, 0x2e, 0x0d, 0x0a
};

static int last_data_packet2_size = 5;
...
#else
void    netx_pop3_packet_with_endmarker_test_application_define(void *first_unused_memory)
#endif
{
...
UINT         i =0;
    ...
    printf("NetX Test:   POP3_Packet_With_Endmarker_Test...........................");
...
NX_PACKET   *packet_ptr;
    ...
    while (mail_item <= number_mail_items)
    {
        ...
        do
        {
            ...
            if (final_packet)
            {
                 /* It is. If it is the second mail item verify that no data is reported in the packet. */
                if ((mail_item == 2) && (bytes_retrieved != 0))
                {
                  
                    error_counter++;
                }
                
                /* Let the server know it can delete this mail item. */
    ...
    status = nx_pop3_client_delete(&pop3_client);
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Test:   POP3_Packet_With_Endmarker_Test...........................N/A\n");
