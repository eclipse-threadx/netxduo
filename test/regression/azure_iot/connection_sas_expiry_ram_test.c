...
static UINT time_callback(ULONG *unix_time);
...
ULONG pool_ptr_available_packet;
    ...
    assert_int_equal(nx_azure_iot_create(&iot, (UCHAR *)"Azure IoT", ip_ptr, pool_ptr, dns_ptr, (UCHAR *)demo_cloud_thread_stack,
                                         sizeof(demo_cloud_thread_stack), DEMO_CLOUD_THREAD_PRIORITY, time_callback),
                     NX_AZURE_IOT_SUCCESS);
    ...
    assert_int_equal(nx_azure_iot_hub_client_symmetric_key_set(&iot_client, STRING_UNSIGNED_ARGS(symmetric_key)),
                     NX_AZURE_IOT_SUCCESS);
    ...
    /* Wait until SAS token expired. */
    tx_time_set(tx_time_get() + NX_AZURE_IOT_HUB_CLIENT_TOKEN_CONNECTION_TIMEOUT * NX_IP_PERIODIC_RATE);

    /* Disconnect from server. */
    nx_tcp_socket_disconnect(&server_socket, NX_NO_WAIT);

    /* Check connection status. */
    assert_int_equal(connection_status, NX_AZURE_IOT_SAS_TOKEN_EXPIRED);
...
static UINT time_callback(ULONG *unix_time)
{
    *unix_time = 1612174237 + tx_time_get() / NX_IP_PERIODIC_RATE;
    return(NX_SUCCESS);
}
