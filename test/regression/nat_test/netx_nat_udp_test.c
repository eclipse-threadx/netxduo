...
#else
void    netx_nat_udp_test_application_define(void *first_unused_memory)
#endif
{
...
ULONG       packets_sent, bytes_sent, packets_received, bytes_received, packets_queued, receive_packets_dropped, checksum_errors;
    ...
    printf("NetX Test:   NAT UDP Processing Test...................................");                                                       
    ...
    if ((nat_server.forwarded_packets_received != 4) || (nat_server.forwarded_packets_sent != 3) ||(nat_server.forwarded_packets_dropped != 1)) 
    {
        printf("ERROR!\n");
        test_control_return(1);
    }
    ...
    if ((packets_sent != 3) || (bytes_sent != 3 * 28) || (packets_received != 1) || (bytes_received != 1 * 28))
    {                    
        printf("ERROR!\n");
        test_control_return(1);
    }
    ...
    /* Get UDP external socket information.  */
    status =  nx_udp_socket_info_get(&external_socket, &packets_sent, &bytes_sent, &packets_received, &bytes_received, 
                                     &packets_queued, &receive_packets_dropped, &checksum_errors);
...
NX_PACKET   *my_packet;
    ...
    /* Receive a UDP packet.  */
    status =  nx_udp_socket_receive(&local_socket, &my_packet, 10 * NX_IP_PERIODIC_RATE);
...
#endif
{
    printf("NetX Test:   NAT UDP Processing Test...................................N/A\n");
