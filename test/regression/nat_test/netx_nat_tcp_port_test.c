...
NX_NAT_TRANSLATION_ENTRY            server_inbound_entry_tcp1;  
NX_NAT_TRANSLATION_ENTRY            server_inbound_entry_tcp2;
...
#else
void    netx_nat_tcp_port_test_application_define(void *first_unused_memory)
#endif
{
...
ULONG       packets_sent, bytes_sent, packets_received, bytes_received, retransmit_packets, packets_queued, checksum_errors, socket_state, transmit_queue_depth, transmit_window, receive_window;
    ...
    printf("NetX Test:   NAT TCP Port Processing Test..............................");                                                       
                                      
    /* Create a TCP socket for NAT IP instance.  */
    status = nx_tcp_socket_create(&nat_ip, &nat_socket, "NAT Socket", NX_IP_NORMAL, NX_FRAGMENT_OKAY, NX_IP_TIME_TO_LIVE, 200, NX_NULL, NX_NULL);
    ...
    /* Bind the socket to the IP port as NX_NAT_START_TCP_PORT.  */
    status = nx_tcp_client_socket_bind(&nat_socket, NX_NAT_START_TCP_PORT, NX_WAIT_FOREVER);
    ...
    /* Preload a NAT entry with same external TCP port of NAT socket.  */
    status = nx_nat_inbound_entry_create(&nat_server, &server_inbound_entry_tcp1, NX_NAT_LOCAL_HOST1, NX_NAT_START_TCP_PORT, NX_NAT_START_TCP_PORT, NX_PROTOCOL_TCP);
    ...
    /* Preload a NAT entry with different external TCP port of NAT socket.  */
    status = nx_nat_inbound_entry_create(&nat_server, &server_inbound_entry_tcp1, NX_NAT_LOCAL_HOST1, NX_NAT_START_TCP_PORT - 1, NX_NAT_START_TCP_PORT, NX_PROTOCOL_TCP);
    ...
    /* Preload a NAT entry with same external TCP port of NAT entry.  */
    status = nx_nat_inbound_entry_create(&nat_server, &server_inbound_entry_tcp2, NX_NAT_LOCAL_HOST1, NX_NAT_START_TCP_PORT - 1, NX_NAT_START_TCP_PORT, NX_PROTOCOL_TCP);
    ...
    status = nx_nat_inbound_entry_delete(&nat_server, &server_inbound_entry_tcp1);
...
ULONG       peer_port;
    ...
    status = nx_tcp_socket_peer_info_get(&external_socket, &peer_ip_address, &peer_port);
...
#endif
{
    printf("NetX Test:   NAT TCP Port Processing Test..............................N/A\n");
