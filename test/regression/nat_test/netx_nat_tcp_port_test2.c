...
#else
void    netx_nat_tcp_port_test2_application_define(void *first_unused_memory)
#endif
{
...
ULONG       packets_sent, bytes_sent, packets_received, bytes_received, retransmit_packets, packets_queued, checksum_errors, socket_state, transmit_queue_depth, transmit_window, receive_window;
    ...
    printf("NetX Test:   NAT TCP Port Processing Test2.............................");                                                       
    ...
    /* Bind the socket to the IP port as NX_NAT_START_TCP_PORT.  */
    status = nx_tcp_client_socket_bind(&nat_socket, NX_NAT_START_TCP_PORT + 1, NX_WAIT_FOREVER);
    ...
    /* Create a socket.  */
    status =  nx_tcp_socket_create(&nat_ip, &nat_socket_1, "NAT Socket 1", 
    ...
    status =  nx_tcp_server_socket_listen(&nat_ip, NX_NAT_START_TCP_PORT, &nat_socket_1, 5, NX_NULL);
    
    /* Check for error.  */
    if (status != NX_DUPLICATE_LISTEN)
    {
        printf("ERROR!\n");
        test_control_return(1);
    }    
...
ULONG       peer_port;
    ...
    /* Check status.  */
    if ((status) || (peer_ip_address != NX_NAT_EXTERNAL_IPADR) || (peer_port != NX_NAT_START_TCP_PORT + 2))
    { 
        printf("ERROR!\n");
        test_control_return(1);
    } 
...
#endif
{
    printf("NetX Test:   NAT TCP Port Processing Test2.............................N/A\n");
