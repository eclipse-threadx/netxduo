...
#else
void    netx_nat_tcp_test2_application_define(void *first_unused_memory)
#endif
{
...
ULONG       packets_sent, bytes_sent, packets_received, bytes_received, retransmit_packets, packets_queued, checksum_errors, socket_state, transmit_queue_depth, transmit_window, receive_window;
    ...
    printf("NetX Test:   NAT TCP Processing Test2..................................");                                                       
                                      
    /* Create a TCP external socket.  */
    status = nx_tcp_socket_create(&external_ip, &external_socket, "Local Socket", NX_IP_NORMAL, NX_FRAGMENT_OKAY, NX_IP_TIME_TO_LIVE, 200, NX_NULL, NX_NULL);
    ...
    /* Bind the socket to the IP port 0x89.  */
    status =  nx_tcp_client_socket_bind(&external_socket, 0x89, NX_WAIT_FOREVER);
    ...
    status =  nx_tcp_client_socket_connect(&external_socket, NX_NAT_EXTERNAL_IPADR, 0x88, 5 * NX_IP_PERIODIC_RATE);
    ...
    status = nx_nat_inbound_entry_create(&nat_server, &server_inbound_entry_tcp, NX_NAT_LOCAL_HOST1, 0x88, 0x88, NX_PROTOCOL_TCP);    
    ...
    status =  nx_tcp_socket_state_wait(&external_socket, NX_TCP_ESTABLISHED, 5 * NX_IP_PERIODIC_RATE);
    ...
    /* Send the TCP packet.  */
    status =  nx_tcp_socket_send(&external_socket, my_packet, 5 * NX_IP_PERIODIC_RATE);
    ...
    /* Unbind the socket.  */
    status =  nx_tcp_client_socket_unbind(&external_socket);
    ...
    status =  nx_tcp_socket_info_get(&external_socket, &packets_sent, &bytes_sent, 
...
NX_PACKET   *my_packet;
    ...
    /* Create a socket.  */
    status =  nx_tcp_socket_create(&local_ip, &local_socket, "External Server Socket", 
    ...
    status =  nx_tcp_server_socket_listen(&local_ip, 0x88, &local_socket, 5, NX_NULL);
    ...
    status =  nx_tcp_server_socket_accept(&local_socket, NX_WAIT_FOREVER);
    ...
    status =  nx_tcp_socket_receive(&local_socket, &my_packet, 5 * NX_IP_PERIODIC_RATE);
    ...
    status =  nx_tcp_server_socket_unaccept(&local_socket);
    ...
    status =  nx_tcp_server_socket_unlisten(&local_ip, 0x88);
...
#endif
{
    printf("NetX Test:   NAT TCP Processing Test2..................................N/A\n");
