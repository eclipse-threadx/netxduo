/* PTP API test.  This test case extends the basic test case to cover API not tested by others. */
...
#else
void netx_ptp_client_api_application_define(void *first_unused_memory)
#endif
{
CHAR       *pointer;
UINT       status;
...
UCHAR time_source;
    ...
    switch (event)
    {
        case NX_PTP_CLIENT_EVENT_MASTER:
        {
            ...
            nx_ptp_client_master_info_get((NX_PTP_CLIENT_MASTER *)event_data, &address, &port_identity,
                                          &port_identity_length, &priority1, &priority2, &clock_class,
                                          &clock_accuracy, &clock_variance, &grandmaster_identity,
                                          &grandmaster_identity_length, &steps_removed, &time_source);
            ASSERT_TRUE(address.nxd_ip_address.v4 == IP_ADDRESS(1, 2, 3, 5));
            ASSERT_TRUE(address.nxd_ip_version == NX_IP_VERSION_V4);
            ASSERT_TRUE(port_identity_length == NX_PTP_CLOCK_PORT_IDENTITY_SIZE);
            ASSERT_SUCCESS(memcmp(port_identity, "\x46\xe7\xc8\xff\xfe\x71\x61\xa1\x00\x01", port_identity_length));
            ASSERT_TRUE(priority1 == 128);
            ASSERT_TRUE(priority2 == 128);
            ASSERT_TRUE(clock_class == 127);
            ASSERT_TRUE(clock_accuracy == 0xfe);
            ASSERT_TRUE(clock_variance == 0x7060);
            ASSERT_TRUE(grandmaster_identity_length == NX_PTP_CLOCK_IDENTITY_SIZE);
            ASSERT_SUCCESS(memcmp(grandmaster_identity, "\x46\xe7\xc8\xff\xfe\x71\x61\xa1\x00\x00", port_identity_length));
            ASSERT_TRUE(steps_removed == 0);
            ASSERT_TRUE(time_source == 0xa0);
            break;
        }
...
NX_PTP_TIME ts_diff = {1, 1, 1};
    ...
    /* Set timestamp to all zero.  */
    status = nx_ptp_client_time_set(&ptp_client, &ts_zero);
    ASSERT_SUCCESS(status);
    ...
    /* Set timestamp to all zero when PTP already started.  */
    status = nx_ptp_client_time_set(&ptp_client, &ts_zero);
    ASSERT_TRUE(status == NX_PTP_CLIENT_ALREADY_STARTED);
