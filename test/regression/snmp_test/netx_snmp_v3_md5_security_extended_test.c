...
#else
void    netx_snmp_v3_md5_security_extended_test_application_define(void *first_unused_memory)
#endif
{
...
CHAR    *pointer;
    ...
    status = nx_snmp_agent_md5_key_create_extended(&my_agent, (UCHAR *)("authpassword"), 12, &my_authentication_key);
    ...
    status = nx_snmp_agent_md5_key_create_extended(&my_agent, (UCHAR *)("privpassword"), 12, &my_privacy_key);
...
UINT status;
    ...
    printf("NetX Test:   SNMP V3 with MD5 Security Extended Test...................");
    tx_thread_sleep(20);
...
USHORT      request_id = 1;
    ...
    for (i = 0; i < QUERY_COUNT; i++ )
    {
        ...
        if (i == 1)
        {
            ...
            if ((agent_packet -> nx_packet_length <= 136) || (agent_packet -> nx_packet_length >= 140))
            {
                error_counter++;
                break;
            }
...
UINT    i;
UINT    status;
UINT    object_requested_length;
UINT    object_name_length;


    /* Get the object requested length.  */
    _nx_utility_string_length_check(object_requested, &object_requested_length, NX_SNMP_MAX_OCTET_STRING);
    ...
    while (mib2_mib[i].object_name)
    {
        ...
        /* Get the object name length.  */
        _nx_utility_string_length_check(mib2_mib[i].object_name, &object_name_length, NX_SNMP_MAX_OCTET_STRING);

        /* See if we have found the matching entry.  */
        status =  nx_snmp_object_compare_extended(object_requested, object_requested_length, mib2_mib[i].object_name, object_name_length);
...
UINT    object_value_length;
    ...
    nx_snmp_object_copy_extended(mib2_mib[i].object_name, object_name_length, object_requested, NX_SNMP_MAX_OCTET_STRING + 1);
    ...
    if (mib2_mib[i].object_get_callback)
    {
        ...
        if (object_data -> nx_snmp_object_data_type == NX_SNMP_END_OF_MIB_VIEW)
        {
            ...
            /* Get the object name length.  */
            _nx_utility_string_length_check(mib2_mib[i].object_value_ptr, &object_value_length, NX_SNMP_MAX_OCTET_STRING);

            /* Copy the name supplied in the mib table.  */
            nx_snmp_object_copy_extended(mib2_mib[i].object_value_ptr, object_value_length, object_requested, NX_SNMP_MAX_OCTET_STRING + 1);
        }
    }
    else
    {
...
UINT    object_name_length;
    ...
    /* Get the object requested length.  */
    _nx_utility_string_length_check(object_requested, &object_requested_length, NX_MAX_STRING_LENGTH);
    ...
    while (mib2_mib[i].object_name)
    {
        ...
        /* Get the object name length.  */
        _nx_utility_string_length_check(mib2_mib[i].object_name, &object_name_length, NX_MAX_STRING_LENGTH);
...
#endif
{
    ...
    printf("NetX Test:   SNMP V3 with MD5 Security Extended Test...................N/A\n"); 
