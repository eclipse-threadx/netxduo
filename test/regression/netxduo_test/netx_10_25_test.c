...
/* Procedure
   ...
   3. When receives packet, Server sends a packet to client immediately.
   4. Check the packet server sending to client,if it acknowledges packet of client.  */
...
static UINT    my_packet_process_10_25(NX_IP *ip_ptr, NX_PACKET *packet_ptr);
static void    my_tcp_packet_receive_10_25(NX_IP *ip_ptr, NX_PACKET *packet_ptr);
...
#else
void    netx_10_25_application_define(void *first_unused_memory)
#endif
{
...
NX_PACKET  *my_packet;
    ...
    printf("NetX Test:   TCP Spec 10.25 Test.......................................");
    ...
    ip_0.nx_ip_tcp_packet_receive = my_tcp_packet_receive_10_25;
    packet_process_callback = my_packet_process_10_25;
...
NX_PACKET  *my_packet;
    ...
    status = _nx_tcp_client_socket_connect(&client_socket, IP_ADDRESS(1, 2, 3, 5), 12, NX_IP_PERIODIC_RATE);
    ...
    for(i = 0;i < 20;i++)
    {
        data_10_25[i] = rand() % 256;
    }

    status = nx_packet_data_append(my_packet, data_10_25, 20, &pool_0, NX_IP_PERIODIC_RATE);
    ...
    /* Return error.  */
    if((error_counter) || (data_counter != 1) || (ack_counter != 1))
    {
        printf("ERROR!\n");
        test_control_return(1);
    }
...
static void    my_tcp_packet_receive_10_25(NX_IP *ip_ptr, NX_PACKET *packet_ptr)
{
    if( (server_socket.nx_tcp_socket_state == NX_TCP_ESTABLISHED) && (packet_ptr->nx_packet_length == 40) && (!memcmp(packet_ptr -> nx_packet_prepend_ptr + 20, data_10_25, 20)))
    {
        data_counter ++;
...
NX_TCP_HEADER   *tcp_header_ptr;
    ...
    if( server_socket.nx_tcp_socket_state == NX_TCP_ESTABLISHED)
    {
        ...
        /* Check whether piggyback acknowledgment with a segment being transmitted.  */
        if((tcp_header_ptr ->nx_tcp_header_word_3 & NX_TCP_ACK_BIT) && (tcp_header_ptr -> nx_tcp_acknowledgment_number == client_socket.nx_tcp_socket_tx_sequence))
            ack_counter++;
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Test:   TCP Spec 10.25 Test.......................................N/A\n"); 
