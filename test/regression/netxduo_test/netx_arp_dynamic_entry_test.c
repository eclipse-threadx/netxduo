...
#else
void    netx_arp_dynamic_entry_test_application_define(void *first_unused_memory)
#endif
{
...
ULONG       invalid_messages;
    ...
    printf("NetX Test:   ARP Dynamic Entry Processing Test.........................");
    ...
    /* Set a invalid dynamic ARP entry, the destination address isnot directly accessible .  */
    status =  nx_arp_dynamic_entry_set(&ip_0, IP_ADDRESS(2, 2, 3, 5), 0x0011, 0x22334458);
    ...
    /* Get nothing from ARP information.  */
    status =  nx_arp_info_get(&ip_0, NX_NULL, NX_NULL, NX_NULL, NX_NULL, NX_NULL, NX_NULL, NX_NULL, NX_NULL);
    ...
    if ((status) || (requests_received) || (requests_sent) || (responses_sent) || (responses_received) || (dynamic_entries != 1) || (static_entries) || (aged_entries) || (invalid_messages))
        error_counter++;

#ifdef __PRODUCT_NETXDUO__
    /* Test the API arp entry delete. */
    status = nx_arp_entry_delete(&ip_0, IP_ADDRESS(1,2,3,5));
    if(status != NX_SUCCESS)
        error_counter++;
...
#endif
{
    ...
    printf("NetX Test:   ARP Dynamic Entry Processing Test.........................N/A\n"); 
