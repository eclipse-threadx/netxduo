/* This NetX test concentrates on the chained packet of TCP transmission.  */
/* Window size is 1200 bytes. Payload size of pool is 512. Host A sends 2000 bytes. */
...
static UCHAR   send_buff[3000];
static UCHAR   recv_buff[3000];
...
#else
void    netx_tcp_chained_packet_test_application_define(void *first_unused_memory)
#endif
{
...
NX_PACKET   *my_packet;
    ...
    printf("NetX Test:   TCP Chained Packet Test...................................");
    ...
    status = nx_packet_data_append(my_packet, send_buff, sizeof(send_buff), 
                                   &pool_0, 2 * NX_IP_PERIODIC_RATE);
    if(status)
        error_counter++;
...
ULONG           bytes_copied;
    ...
    status =  nx_tcp_socket_create(&ip_1, &server_socket, "Server Socket", 
                                   NX_IP_NORMAL, NX_FRAGMENT_OKAY, NX_IP_TIME_TO_LIVE, 1202,
                                   NX_NULL, NX_NULL);
    if (status)
        error_counter++;
    ...
    while (nx_tcp_socket_receive(&server_socket, &packet_ptr, 3 * NX_IP_PERIODIC_RATE) == NX_SUCCESS)
    {

        /* Retrieve data. */
        nx_packet_data_retrieve(packet_ptr, recv_buff + total_length, &bytes_copied);
        total_length += bytes_copied;
    ...
    /* Check received data. */
    if (total_length != sizeof(send_buff))
        error_counter++;
...
#endif
{
    ...
    printf("NetX Test:   TCP Chained Packet Test...................................N/A\n");
