/* This NetX test concentrates on the forward fragment operation(fragment failure:_nx_packet_allocate failure in nx_ip_fragment_forward_packet).  */
...
#else
void    netx_forward_udp_fragment_test2_application_define(void *first_unused_memory)
#endif
{
...
UINT    status;
    ...
    status = nx_ip_create(&ip_2, "NetX IP Instance 1", IP_ADDRESS(2, 2, 3, 5), 0xFFFFFF00UL, &pool_0, _nx_ram_network_driver_512, pointer, 2048, 2);
    pointer =  pointer + 2048;
    if (status)
        error_counter++;
...
UINT        free_port;
    ...
    printf("NetX Test:   Forward UDP Fragment Test2................................");
    ...
    /* Allocate a packet.  */
    status =  nx_packet_data_append(my_packet, &message[0], 800, &pool_0, TX_WAIT_FOREVER);
...
ULONG       packets_sent, bytes_sent, packets_received, bytes_received, packets_queued, receive_packets_dropped, checksum_errors;
    ...
    status =  nx_udp_socket_receive(&socket_2, &my_packet, 2 * NX_IP_PERIODIC_RATE);
    ...
    if ((packets_sent != 0) || (bytes_sent != 0) || (packets_received != 1) || (bytes_received != 800))
    {
        error_counter++;
    }
#endif
...
NX_PACKET   *tmp_packet[10];
    ...
    /* Allocate the all packets.  */
    for (i = 0; i < avalibale_count; i++)
    {
    ...
    /* Call the _nx_ipv4_packet_receive function directly receive this packet(forward module).  */
    _nx_ipv4_packet_receive(&ip_0, packet_ptr);

    /* Check the forward fragment operation(_nx_ip_fragment_forward_packet function).  */ 
#ifndef NX_DISABLE_IP_INFO
    if ((ip_0.nx_ip_fragment_failures != 1) || (ip_0.nx_ip_send_packets_dropped != 1) || (ip_0.nx_ip_transmit_resource_errors != 1))  
        error_counter++;   
#endif

    /* Check the packet available count(fragment failure, the source packet should be release).  */
...
#endif
{
    printf("NetX Test:   Forward UDP Fragment Test2................................N/A\n");
