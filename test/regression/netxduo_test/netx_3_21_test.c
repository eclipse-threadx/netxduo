...
/*  Procedure
    1.Connect.
    2.Set server socket to LAST_ACK state.
    3.Client sends a packet to server.
    4.Set the packet to an OTW packet in the callback function.
    5.Client should receive an ACK with proper SEQ and ACK numbers, and server socket should remain in LAST_ACK state.
    6.Client sends a packet to server.
    7.Set the packet to an unacceptable packet in the callback function.
    8.Client should receive an ACK with proper SEQ and ACK numbers, and server socket should remain in LAST_ACK state.  */
...
static void    my_tcp_packet_receive_3_21(NX_IP *ip_ptr, NX_PACKET *packet_ptr);
static void    my_tcp_packet_receive_3_21_2(NX_IP *ip_ptr, NX_PACKET *packet_ptr);
...
#else
void           netx_3_21_application_define(void *first_unused_memory)
#endif
{
CHAR       *pointer;
UINT       status;
...
ULONG      actual_status;
    ...
    printf("NetX Test:   TCP Spec 3.21 Test........................................");
    ...
    advanced_packet_process_callback = my_packet_process_3_21;

    /* Disconnect the server socket.  */
    status = nx_tcp_socket_disconnect(&server_socket, NX_NO_WAIT);

    if(server_socket.nx_tcp_socket_state != NX_TCP_LAST_ACK)
        error_counter++;

    /* Set the receive function to my_tcp_packet_receive_3_21.  */
    ip_0.nx_ip_tcp_packet_receive = my_tcp_packet_receive_3_21;
...
NX_PACKET   *my_packet1, *my_packet2;
    ...
    /* Replace the TCP receive function with my_tcp_packet_receive_3_21 for ip_1.  */
    ip_1.nx_ip_tcp_packet_receive = my_tcp_packet_receive_3_21_2;
    ...
    if((error_counter == 0) && (fin_counter == 1) && (ack_counter == 2) && (data_packet_counter == 2))
    {
        printf("SUCCESS!\n");
...
#error "NetX Product undefined."
    ...
    else
    {
        ...
        {
            client_socket.nx_tcp_socket_rx_sequence++;
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Test:   TCP Spec 3.21 Test........................................N/A\n");
