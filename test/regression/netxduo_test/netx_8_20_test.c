...
/* Procedure
1. Connection.
2. Drop syn+ack packet.
3. server sends ack packet.
4. the state of client socket should be the SYN_SENT.  */
...
static void    my_tcp_packet_receive_8_20(NX_IP *ip_ptr, NX_PACKET *packet_ptr);
...
#else
void           netx_8_20_application_define(void *first_unused_memory)
#endif
{
CHAR       *pointer;
UINT       status;
...
UINT       status;
    ...
    advanced_packet_process_callback = my_packet_process_8_20;
    ...
    /* Deal the packet with my routing.  */
    ip_0.nx_ip_tcp_packet_receive = my_tcp_packet_receive_8_20;

    /* Resume the thread ntest_1.  */
    tx_thread_resume(&ntest_1);

    /* Check whether the client socket state has changed.  */
    if((ack_counter != 1) || (client_socket.nx_tcp_socket_state != NX_TCP_SYN_SENT))
        error_counter++;
    ...
    /* Check for error.  */
    if(error_counter || (syn_ack_counter != 1) || (ack_counter != 1))
    {
        printf("ERROR!\n");
...
ULONG          actual_status;
    ...
    printf("NetX Test:   TCP Spec 8.20 Test........................................");
...
NX_TCP_HEADER    *tcp_header_ptr;
    ...
    {
        /* Drop the SYN+ACK packet in order to postpone the connection. */
...
NX_TCP_HEADER    *tcp_header_ptr;
    ...
    if(!(tcp_header_ptr -> nx_tcp_header_word_3 & NX_TCP_RST_BIT) && (tcp_header_ptr -> nx_tcp_header_word_3 & NX_TCP_ACK_BIT) 
       && !(tcp_header_ptr -> nx_tcp_header_word_3 & NX_TCP_SYN_BIT) && (tcp_header_ptr -> nx_tcp_acknowledgment_number == client_socket.nx_tcp_socket_tx_sequence))
    {
        ack_counter++;
...
#endif
{
    ...
    printf("NetX Test:   TCP Spec 8.20 Test........................................N/A\n"); 
