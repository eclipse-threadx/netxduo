/* This NetX test concentrates on the TCP Socket connect reset operation under interface deatch(tested transmit queue count and transmit suspension list).  */
...
#else
void    netx_tcp_transmit_under_interface_detach_test_application_define(void *first_unused_memory)
#endif
{
...
NX_PACKET   *my_packet;
    ...
    printf("NetX Test:   TCP Transmit Under Interface Detach Test..................");
    ...
    /* Create a temp socket checked during interface detach. */
    status += nx_tcp_socket_create(&ip_0, &test_socket, "Test Socket", 
                                   NX_IP_NORMAL, NX_FRAGMENT_OKAY, NX_IP_TIME_TO_LIVE, 20,
                                   NX_NULL, NX_NULL);
...
UINT        status;
    ...
    /* Check the TCP socket transmit suspended count.  */      
    if (client_socket.nx_tcp_socket_transmit_sent_count != 0)       
        error_counter++;
    if (client_socket.nx_tcp_socket_transmit_suspension_list != NX_NULL)       
        error_counter++;
}    
...
#endif
{
    printf("NetX Test:   TCP Transmit Under Interface Detach Test..................N/A\n");
