/* This NetX test concentrates on the ARP static entry operation.  */
...
#else
void    netx_arp_static_entry_test_application_define(void *first_unused_memory)
#endif
{
...
ULONG       invalid_messages;
    ...
    printf("NetX Test:   ARP Static Entry Processing Test..........................");
    ...
    /* Delete an not existing static entry.  */
    status =  nx_arp_static_entry_delete(&ip_0, IP_ADDRESS(1, 2, 3, 5), 0x0011, 0x22334457);
    ...
    status =  nx_arp_static_entry_create(&ip_0, IP_ADDRESS(1, 2, 3, 5), 0x0011, 0x22334457);
    if (status)
    {
    ...
    /* Set a invalid static ARP entry, the destination address isnot directly accessible .  */
    status =  nx_arp_static_entry_create(&ip_0, IP_ADDRESS(2, 2, 3, 5), 0x0011, 0x22334458);
    ...
    /* Find the IP address for inexistence entry.  */
    status = nx_arp_ip_address_find(&ip_0, &ip_address, 0x0011, 0x22334458);
    ...
    /* Find the hardware address for inexistence entry.  */
    ...
    if ((requests_sent) || (requests_received) || (responses_sent) || (responses_received) || (static_entries != 1) || (aged_entries) || (invalid_messages))
    {
        error_counter++;
    }
#endif
    ...
    if ((status) || (dynamic_entries))
        error_counter++;

    /* Delete the static entry with wrong MAC.  */
    status =  nx_arp_static_entry_delete(&ip_0, IP_ADDRESS(1, 2, 3, 5), 0, 0x22334457);
    ...
    /* Get ARP information again.  */
    status =  nx_arp_info_get(&ip_0, &requests_sent, &requests_received, &responses_sent, &responses_received, &dynamic_entries, &static_entries, &aged_entries, &invalid_messages);

#ifndef NX_DISABLE_ARP_INFO
    if ((requests_sent) || (requests_received) || (responses_sent) || (responses_received) || (static_entries) || (aged_entries) || (invalid_messages))
    {
        error_counter++;
    }
#endif
    ...
    /* Set a static ARP entry with lsw 0 .  */
    status =  nx_arp_static_entry_create(&ip_0, IP_ADDRESS(1, 2, 3, 15), 0x0011, 0x00000000);
    if (status != NX_SUCCESS)
    {
    ...
    /* Delete the static entry with wrong MAC.  */
    status =  nx_arp_static_entry_delete(&ip_0, IP_ADDRESS(1, 2, 3, 15), 0x0011, 0x00000001);

    /* Check for error.  */
    if (status != NX_ENTRY_NOT_FOUND)
        error_counter++;

    /* Delete the static entry with correct MAC.  */
    status =  nx_arp_static_entry_delete(&ip_0, IP_ADDRESS(1, 2, 3, 15), 0x0011, 0x00000000);
...
#endif
{
    ...
    printf("NetX Test:   ARP Static Entry Processing Test..........................N/A\n"); 
