...
#else
void    netx_ip_fragmentation_wrong_destination_address_test_application_define(void *first_unused_memory)
#endif
{
...
UINT        packet_length;
    ...
    printf("NetX Test:   IP Fragmentation Wrong Destination Address Test...........");
    ...
    /* Let Socket 0 send all packet. test _nx_ip_fragment_assembly()
       Hit the false condition: ((search_header -> nx_ip_header_word_2 & NX_IP_PROTOCOL_MASK) ==
...
ULONG            checksum;
    ...
    if ((ip_ptr == &ip_0) && (packet_ptr -> nx_packet_length > 28))
    {
        ...
        /* Copy the first fragmentation packet.  */
        if (packet_counter == 2)
        {
            ...
            /* Modify the destination address to 255.255.255.255. */
...
#endif
{
    ...
    printf("NetX Test:   IP Fragmentation Wrong Destination Address Test...........N/A\n");
