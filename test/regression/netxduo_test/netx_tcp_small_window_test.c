/* This NetX test concentrates on the small window TCP operation.  */
...
#else
void    netx_tcp_small_window_test_application_define(void *first_unused_memory)
#endif
{
...
ULONG       end_time;
    ...
    printf("NetX Test:   TCP Small Window Size Test................................");
    ...
    status = nx_packet_data_append(my_packet, message, sizeof(message), &pool_0, NX_WAIT_FOREVER);

    /* Check status.  */
    if (status != NX_SUCCESS)
        error_counter++;

    /* Delay the packet sent out. */
    advanced_packet_process_callback = my_packet_process;

    /* Send the packet out!  */
    status =  nx_tcp_socket_send(&client_socket, my_packet, 1);

    advanced_packet_process_callback = NX_NULL;

    /* The timeout is one tick only, so only part of whole packet can be sent.  */
    if (status)
    {
    ...
    /* Send the packet out!  */
    status =  nx_tcp_socket_send(&client_socket, my_packet, 10);
...
ULONG           loop;
    ...
    expected_length = sizeof(message);
    loop = 6;
#else
    expected_length = 28;
    loop = 5;
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Test:   TCP Small Window Size Test................................N/A\n"); 
