/* 1.21 TCP MUST send an ACK after receiving a FIN in FIN_WAIT_1 state.  */

/* Procedure
1. Establish a connection by the "three-way handshake" procedure.
2. Server sends a FIN packet and becomes FIN_WAIT_1 state.
3. Delay the FIN packet and keep the server state to FIN_WAIT_1.
4. Client sends a FIN packet.
5. Check the packet is a FIN one.
...
static void    my_tcp_packet_receive_1_21_01(NX_IP *ip_ptr, NX_PACKET *packet_ptr);
static void    my_tcp_packet_receive_1_21_01_2(NX_IP *ip_ptr, NX_PACKET *packet_ptr);
...
#else
void           netx_1_21_01_application_define(void *first_unused_memory)
#endif
{
CHAR       *pointer;
UINT       status;
...
ULONG      actual_status;
    ...
    printf("NetX Test:   TCP Spec 1.21.01 Test.....................................");
    ...
    /* Let the driver delay the FIN packet.  */
    advanced_packet_process_callback = my_packet_process_1_21_01;

    /*Let the server socket check the FIN packet.  */
    ip_0.nx_ip_tcp_packet_receive = my_tcp_packet_receive_1_21_01;
...
UINT       status;
    ...
    /* Let client check the ACK packet.  */
    ip_1.nx_ip_tcp_packet_receive = my_tcp_packet_receive_1_21_01_2;
    ...
    if((error_counter) || (fin_counter != 1) || (ack_counter != 1))
    {
        printf("ERROR!\n");
        test_control_return(1);
    }
...
void           my_tcp_packet_receive_1_21_01(NX_IP *ip_ptr, NX_PACKET *packet_ptr)
{
NX_TCP_HEADER               *tcp_header_ptr;

    /* Check whether the state of server is in FIN_WAIT_1.  */
...
void           my_tcp_packet_receive_1_21_01_2(NX_IP *ip_ptr, NX_PACKET *packet_ptr)
{
NX_TCP_HEADER               *tcp_header_ptr;

    /* Check whether the packet is a ACK packet which responsing the FIN one.  */
    if(fin_counter == 1)
    {
        tcp_header_ptr = (NX_TCP_HEADER *)packet_ptr -> nx_packet_prepend_ptr;
        NX_CHANGE_ULONG_ENDIAN(tcp_header_ptr -> nx_tcp_header_word_3);
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Test:   TCP Spec 1.21.01 Test.....................................N/A\n"); 
