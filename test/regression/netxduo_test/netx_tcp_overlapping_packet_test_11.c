...
#else
void           netx_tcp_overlapping_packet_test_11_application_define(void *first_unused_memory)
#endif
{
    CHAR    *pointer;
    UINT    status;
...
ULONG      bytes_available;
    ...
    printf("NetX Test:   TCP Overlapping Packet Test 11............................");
    ...
    memcpy(my_packet1 -> nx_packet_prepend_ptr, &msg[60], 20);
    ...
    memcpy(my_packet4 -> nx_packet_prepend_ptr, &msg[0], 30);
    my_packet4 -> nx_packet_length = 30;
    my_packet4 -> nx_packet_append_ptr = my_packet4 -> nx_packet_prepend_ptr + 30;
    ...
    status =  nx_tcp_socket_bytes_available(&server_socket, &bytes_available);
    if (status || (bytes_available != 0))
        error_counter++;   

    /* Update the seq number and send the 2th one.  */
    client_socket.nx_tcp_socket_tx_sequence = seq1 - 40;
    status += nx_tcp_socket_send(&client_socket, my_packet2, NX_IP_PERIODIC_RATE);

    /* Restore the Seq number for packet 3.  */
    client_socket.nx_tcp_socket_tx_sequence = seq1 - 20;
    status += nx_tcp_socket_send(&client_socket, my_packet3, NX_IP_PERIODIC_RATE);

    /* Restore the seq number for packet 4.  */
    client_socket.nx_tcp_socket_tx_sequence = seq1 - 60;
...
static void    thread_1_entry(ULONG thread_input)
{
    ...
    /* let thread0 run, so thread0 can send out out-of-order packets.   */
...
#endif
{
    printf("NetX Test:   TCP Overlapping Packet Test 11............................N/A\n");
