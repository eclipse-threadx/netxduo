...
#else
void    netx_tcp_listen_test_application_define(void *first_unused_memory)
#endif
{
...
NX_PACKET   *my_packet;
    ...
    printf("NetX Test:   TCP Listen Test...........................................");
    ...
    status =  nx_tcp_server_socket_listen(&ip_0, 12, &client_socket, 5, NX_NULL);
    if (status != NX_ALREADY_BOUND)
        error_counter++;

#ifdef __PRODUCT_NETXDUO__
    /* Should return some error message since server socket is accepting. */
...
static void    thread_1_entry(ULONG thread_input)
{
    ...
    /* Create a socket.  */
    status =  nx_tcp_socket_create(&ip_1, &server_socket_1, "Server Socket", 
                                   NX_IP_NORMAL, NX_FRAGMENT_OKAY, NX_IP_TIME_TO_LIVE, 100,
                                   NX_NULL, NX_NULL);
    if (status)
        error_counter++;
    ...
    /* Create a socket.  */
    status =  nx_tcp_socket_create(&ip_1, &server_socket_2, "Server Socket", 
                                   NX_IP_NORMAL, NX_FRAGMENT_OKAY, NX_IP_TIME_TO_LIVE, 100,
                                   NX_NULL, NX_NULL);
    if (status)
        error_counter++;
    ...
    /* server_socket listen again. */
    status =  nx_tcp_server_socket_listen(&ip_1, 12, &server_socket, 5, NX_NULL);
    if (status != NX_NOT_CLOSED)
        error_counter++;

    /* server_socket_1 listen at port 13. */
    status =  nx_tcp_server_socket_listen(&ip_1, 13, &server_socket_1, 5, NX_NULL);
    if (status)
        error_counter++;

    /* server_socket_2 listen at port 13. Should return duplicate error */
    status =  nx_tcp_server_socket_listen(&ip_1, 13, &server_socket_2, 5, NX_NULL);
    ...
    /* Unlisten on the server port 13.  */
    status =  nx_tcp_server_socket_unlisten(&ip_1, 13);
    if (status)
        error_counter++;
...
#endif
{
    ...
    printf("NetX Test:   TCP Listen Test...........................................N/A\n"); 
