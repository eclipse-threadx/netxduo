/* 8.29.02 TCP, in FINWAIT-1 or FINWAIT-2 state,MUST ignore a RST segment with OTW SEQ number.  */

/* Procedure
   1.Connection.
   2.Server disconnect without waiting.
   3.Delay the FIN packet.
   4.When Server is on the FINWAIT-1 state, Client sends a RST which contains a OTW SEQ to Server.
   5.Check whether the state has been changed.
   6.When Server is on the FINWAIT-2 state, Client sends a RST which contains a OTW SEQ to Server.
...
static void    my_tcp_packet_receive_8_29_02(NX_IP *ip_ptr, NX_PACKET *packet_ptr);
...
#else
void           netx_8_29_02_application_define(void *first_unused_memory)
#endif
{
CHAR       *pointer;
UINT       status;
...
ULONG      actual_status;
    ...
    printf("NetX Test:   TCP Spec 8.29.02 Test.....................................");
    ...
    advanced_packet_process_callback = my_packet_process_8_29_02;
    ...
    ip_0.nx_ip_tcp_packet_receive = my_tcp_packet_receive_8_29_02;

    tx_thread_suspend(&ntest_0);

    if((rst_counter != 1) || (server_socket.nx_tcp_socket_state != NX_TCP_FIN_WAIT_1))
        error_counter++;
    ...
    if((rst_counter != 2) || (server_socket.nx_tcp_socket_state != NX_TCP_FIN_WAIT_2))
        error_counter++;
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Test:   TCP Spec 8.29.02 Test.....................................N/A\n");
