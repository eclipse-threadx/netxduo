 ...
 * The transmitting data are placed in 4 packets, the packet pool has only one packet available.
 * While the payload size of packet is smaller than the window size, TCP won't send all data until packet is available. */
...
#else
void    netx_tcp_send_fail_test2_application_define(void *first_unused_memory)
#endif
{
...
UINT    status;
    ...
    /* Create a packet pool with 5 packets.  */
    pointer = (CHAR *)(((ALIGN_TYPE)pointer + NX_PACKET_ALIGNMENT - 1) & ~(NX_PACKET_ALIGNMENT - 1));
    status =  nx_packet_pool_create(&pool_0, "NetX Main Packet Pool", 512, pointer, 
                                    ((512 + sizeof(NX_PACKET) + NX_PACKET_ALIGNMENT - 1) & ~(NX_PACKET_ALIGNMENT - 1)) * 6);
    pointer = pointer + (512 + NX_PACKET_ALIGNMENT + sizeof(NX_PACKET)) * 6;

    /* Create another packet pool. */
    status +=  nx_packet_pool_create(&pool_1, "NetX Main Packet Pool", 1536, pointer, 10240);
    ...
    status = nx_ip_create(&ip_0, "NetX IP Instance 0", IP_ADDRESS(1, 2, 3, 4), 0xFFFFFF00UL, &pool_1, _nx_ram_network_driver_1500,
                    pointer, 2048, 1);
...
NX_PACKET   *my_packet;
    ...
    printf("NetX Test:   TCP Send Fail Test 2......................................");
    ...
    /* Verify only one packet available. */
    if (pool_0.nx_packet_pool_available != 2)
        error_counter++;

    /* Send the packet out!  */
    status =  nx_tcp_socket_send(&client_socket, my_packet, NX_IP_PERIODIC_RATE << 1);
    if (status != NX_NO_PACKET)
        error_counter++;

    /* Verify partial data are sent. */
    if (my_packet -> nx_packet_length == sizeof(send_buff))
        error_counter++;
...
NX_PACKET      *waste_ptr;
    ...
    status =  nx_tcp_socket_create(&ip_1, &server_socket, "Server Socket", 
                                   NX_IP_NORMAL, NX_FRAGMENT_OKAY, NX_IP_TIME_TO_LIVE, 800,
                                   NX_NULL, NX_NULL);
    if (status)
        error_counter++;
    ...
    else
    {
        ...
        /* Verify only partial data are received. */
        if(packet_ptr -> nx_packet_length != server_socket.nx_tcp_socket_rx_window_default)
            error_counter++;

        nx_packet_release(packet_ptr);
    }

    /* Waste a packet from pool 0. */
    status =  nx_packet_allocate(&pool_0, &waste_ptr, NX_TCP_PACKET, NX_NO_WAIT);
    if (status)
        error_counter++;
...
#endif
{
    ...
    printf("NetX Test:   TCP Send Fail Test 2......................................N/A\n");
