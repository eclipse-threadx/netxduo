/* 102.18 The TCP output routine never sends more than cwnd if cwnd less than the receiver's advertised window.  */

/*  Procedure
    1.Connect
    2.Client sends a packet with 100 length to server
    3.Server should receive two packet, one with 88 data and another with 12 data
      (client's MSS is 88, advertised window is 100)  */
...
#else
void           netx_102_18_application_define(void *first_unused_memory)
#endif
{
CHAR       *pointer;
UINT       status;
...
char         *msg = MSG;
    ...
    client_socket.nx_tcp_socket_tx_window_congestion = server_socket.nx_tcp_socket_rx_window_last_sent - 10;
    ...
    status = nx_packet_data_append(my_packet, msg, congestion_window+20, &pool_0, NX_IP_PERIODIC_RATE);
    ...
    if((error_counter) || (data_packet_counter != 2))
    {
        printf("ERROR!\n");
...
NX_PACKET    *rcv_packet_ptr;
    ...
    printf("NetX Test:   TCP Spec 102.18 Test......................................");
    ...
    while(!nx_tcp_socket_receive(&server_socket, &rcv_packet_ptr, NX_IP_PERIODIC_RATE))
    {
        /* Check the length of the packet received.  */
        if(rcv_packet_ptr->nx_packet_length <= congestion_window)
            data_packet_counter++;
        else 
            error_counter++;
    }
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Test:   TCP Spec 102.18 Test......................................N/A\n"); 
