...
static void    my_tcp_packet_receive_10_24_01(NX_IP *ip_ptr, NX_PACKET *packet_ptr);
...
#else
void    netx_10_24_01_application_define(void *first_unused_memory)
#endif
{
...
UINT       status;
    ...
    advanced_packet_process_callback = my_packet_process_10_24_01;
    ip_0.nx_ip_tcp_packet_receive = my_tcp_packet_receive_10_24_01;
    ...
    /* Sleep two seconds to wait for the retransmission of the FIN packet. */
    tx_thread_sleep(2 * NX_IP_PERIODIC_RATE);

    if((client_socket.nx_tcp_socket_state != NX_TCP_CLOSE_WAIT) || (fin_counter != 2))
        error_counter++;
...
ULONG      actual_status;
    ...
    printf("NetX Test:   TCP Spec 10.24.01 Test....................................");
    ...
    status = nx_tcp_socket_disconnect(&server_socket, NX_IP_PERIODIC_RATE * 2);
...
NX_TCP_HEADER   *tcp_header_ptr;
    ...
    /* Drop the ACK packet in order to let the server socket retransmit a FIN packet. */
    if ((tcp_header_ptr -> nx_tcp_header_word_3 & NX_TCP_ACK_BIT) && (fin_counter == 1))
    {
...
void    my_tcp_packet_receive_10_24_01(NX_IP *ip_ptr, NX_PACKET *packet_ptr)
{
...
NX_TCP_HEADER   *tcp_header_ptr;
    ...
    else if (client_socket.nx_tcp_socket_state == NX_TCP_CLOSE_WAIT && fin_counter == 1)
    {
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Test:   TCP Spec 10.24.01 Test....................................N/A\n"); 
