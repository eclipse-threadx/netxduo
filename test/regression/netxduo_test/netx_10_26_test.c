...
/* Procedure
   1. Connection.
   2. Client sends a packet, then Server replies with an ACK packet
   3. Record client's tx_sequence.
   4. Server sends a same ACK packet and check whether receives a reply. Check client's tx_sequence dosen't change.  */
...
static ULONG                   tx_sequence;
static ULONG                   acknowledgment_number;
...
static void    my_tcp_packet_receive_10_26(NX_IP *ip_ptr, NX_PACKET *packet_ptr);
...
#else
void    netx_10_26_application_define(void *first_unused_memory)
#endif
{
...
NX_PACKET   *my_packet;
    ...
    ip_0.nx_ip_tcp_packet_receive = my_tcp_packet_receive_10_26;
    ...
    for(i = 0;i < 20;i++)
    {
        data_10_26[i] = rand() % 256;
    }

    status = nx_packet_data_append(my_packet, data_10_26, 20, &pool_0, NX_IP_PERIODIC_RATE);
    ...
    if(tx_sequence != client_socket.nx_tcp_socket_tx_sequence)
        error_counter++;
    ...
    /* Return error.  */
    if((error_counter) || (ack_counter != 2))
    {
        printf("ERROR!\n");
...
NX_PACKET  *my_packet;
    ...
    printf("NetX Test:   TCP Spec 10.26 Test.......................................");
    ...
    /* Let the thread sleep to wait for the the delay confirmation */
    tx_thread_sleep(NX_IP_PERIODIC_RATE);

    /* Send the duplicate ACK packet.  */
...
NX_TCP_HEADER   *tcp_header_ptr;
    ...
    if (client_socket.nx_tcp_socket_state == NX_TCP_ESTABLISHED)
    {
        ...
        /* Check whether the packet is a ACK one.  */
        if ((tcp_header_ptr -> nx_tcp_header_word_3 & NX_TCP_ACK_BIT) && 
            (ack_counter == 0))
        {
            ack_counter++;
            tx_sequence = client_socket.nx_tcp_socket_tx_sequence;
            acknowledgment_number = tcp_header_ptr -> nx_tcp_acknowledgment_number;
        }

        /* Check whether the packet is a duplicate ACK. */
...
#endif
{
    ...
    printf("NetX Test:   TCP Spec 10.26 Test.......................................N/A\n"); 
