/* 15.21 If a retransmitted packet differs from the original packet in the acknowledgement field, the same IP identification field MUST NOT be used.  */

/*  Procedure
    1.Connection successfully
    2.Record the 1st coming IP_ID to ip_id_15_21
    3.Drop this packet and wait for the retransmitted packet
    4.Modify retransmitted packet's Acknowledgement field
    5.Check the retransmitted packet's IP_ID ?= ip_id_15_21   */
...
#else
void           netx_15_21_application_define(void *first_unused_memory)
#endif
{
CHAR       *pointer;
UINT       status;
...
ULONG            actual_status;
NX_PACKET        *my_packet;
NX_TCP_HEADER    *retransmit_header;


    /* Print out test information banner.  */
    printf("NetX Test:   TCP Spec 15.21 Test.......................................");
    ...
    advanced_packet_process_callback = my_packet_process_15_21;
    ...
    if(server_socket.nx_tcp_socket_transmit_sent_head)
    {
        retransmit_header = (NX_TCP_HEADER *)(server_socket.nx_tcp_socket_transmit_sent_head -> nx_packet_prepend_ptr);

        NX_CHANGE_ULONG_ENDIAN(retransmit_header -> nx_tcp_acknowledgment_number);

        /* Modify the packet's acknowledgement field*/
        retransmit_header -> nx_tcp_acknowledgment_number --;
...
NX_PACKET    *rcv_packet_ptr;
    ...
    if(error_counter || (drop_counter != 1) || (data_packet_counter != 1) || (is_different != NX_TRUE))
    {
        printf("ERROR!\n");
        test_control_return(1);
    }
...
#error "NetX Product undefined."
    ...
    if ((packet_ptr -> nx_packet_length - 40 == 20) && (!memcmp(packet_ptr -> nx_packet_prepend_ptr + 40, MSG, 20)))
    {
        if (drop_counter == 0)
        {
            ip_id_15_21 = (ip_header_ptr -> nx_ip_header_word_1 & (~NX_LOWER_16_MASK));
        ...
        {
            if(ip_id_15_21 != (ip_header_ptr -> nx_ip_header_word_1 & (~NX_LOWER_16_MASK)))
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Test:   TCP Spec 15.21 Test.......................................N/A\n"); 
