/* 3.6 TCP, in LISTEN state MUST send a RST after receiving a segment that is carrying an unacceptable ACK and remain in the same state.  */

/* Procedure
   1. Connection.
   2. Server calls disconnect command.
   3. Client sends a segment that is carrying an unacceptable ACK.
   4. Server checks whether the packet contains ACK or not.
   5. Server checks whether a RST packet is send.  */
...
static void    my_tcp_packet_receive_3_06(NX_IP *ip_ptr, NX_PACKET *packet_ptr);
static void    my_tcp_packet_receive_3_06_2(NX_IP *ip_ptr, NX_PACKET *packet_ptr);
...
#else
void           netx_3_06_application_define(void *first_unused_memory)
#endif
{
CHAR       *pointer;
UINT       status;
...
ULONG      actual_status;
    ...
    advanced_packet_process_callback = my_packet_process_3_06;
    ...
    ip_0.nx_ip_tcp_packet_receive = my_tcp_packet_receive_3_06;
...
NX_PACKET       *my_packet;
    ...
    printf("NetX Test:   TCP Spec 3.06 Test........................................");
    ...
    ip_1.nx_ip_tcp_packet_receive = my_tcp_packet_receive_3_06_2;
    ...
    status = nx_packet_data_append(my_packet, MSG, 10, &pool_0, NX_IP_PERIODIC_RATE);
    ...
    if((error_counter) || (data_packet_counter != 1) || (rst_counter != 2))
    {
        printf("ERROR!\n");
        test_control_return(1);
    }
...
NX_TCP_HEADER       *tcp_header_ptr;
    ...
    {
        if((packet_ptr -> nx_packet_length == 30) && (!memcmp(packet_ptr -> nx_packet_prepend_ptr + 20, MSG, 10)))
            data_packet_counter++;
...
ULONG           source_ip, dest_ip;
    ...
    /* Modify the acknowledgment number of the packet to an unacceptable data and recalculate the checksum */
    if((tcp_header_ptr -> nx_tcp_header_word_3 & NX_TCP_ACK_BIT) && !(tcp_header_ptr -> nx_tcp_header_word_3 & NX_TCP_SYN_BIT) && !(tcp_header_ptr -> nx_tcp_header_word_3 & NX_TCP_RST_BIT))
    {
        NX_CHANGE_ULONG_ENDIAN(tcp_header_ptr ->nx_tcp_acknowledgment_number);

        /* Modify the acknowledgment number */
        tcp_header_ptr ->nx_tcp_acknowledgment_number += 10;
...
#endif
{
    ...
    printf("NetX Test:   TCP Spec 3.06 Test........................................N/A\n");
