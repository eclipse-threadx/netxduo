/* This case tests:
1. TCP connection failed with HTTP Proxy server.
2. Processing CONNECT response with FIN set. */
#include    "tx_api.h"
#include    "nx_api.h"

extern void test_control_return(UINT);

#if !defined(NX_DISABLE_IPV4) && defined(NX_ENABLE_HTTP_PROXY) && !defined(NX_DISABLE_EXTENDED_NOTIFY_SUPPORT) && defined(__PRODUCT_NETXDUO__)
...
{
0x48, 0x54, 0x54, 0x50, 0x2f, 0x31, 0x2e, 0x30, 0x20, 0x32, 0x30, 0x30, 0x20, 0x43, 0x6f, 0x6e,  /* HTTP/1.0 200 Con */
0x6e, 0x65, 0x63, 0x74, 0x69, 0x6f, 0x6e, 0x20, 0x65, 0x73, 0x74, 0x61, 0x62, 0x6c, 0x69, 0x73,  /* nection establis */
0x68, 0x65, 0x64, 0x0d, 0x0a, 0x50, 0x72, 0x6f, 0x78, 0x79, 0x2d, 0x61, 0x67, 0x65, 0x6e, 0x74,  /* hed..Proxy-agent */
0x3a, 0x20, 0x74, 0x69, 0x6e, 0x79, 0x70, 0x72, 0x6f, 0x78, 0x79, 0x2f, 0x31, 0x2e, 0x38, 0x2e,  /* : tinyproxy/1.8. */
0x34, 0x0d, 0x0a, 0x0d, 0x0a,                                                                    /* 4.... */
};
...
static UCHAR connect_403[] =
{
0x48, 0x54, 0x54, 0x50, 0x2f, 0x31, 0x2e, 0x30, 0x20, 0x34, 0x30, 0x33, 0x20, 0x41, 0x63, 0x63,  /* HTTP/1.0 403 Acc */
0x65, 0x73, 0x73, 0x20, 0x76, 0x69, 0x6f, 0x6c, 0x61, 0x74, 0x69, 0x6f, 0x6e, 0x0d, 0x0a, 0x53,  /* ess violation..S */
0x65, 0x72, 0x76, 0x65, 0x72, 0x3a, 0x20, 0x74, 0x69, 0x6e, 0x79, 0x70, 0x72, 0x6f, 0x78, 0x79,  /* erver: tinyproxy */
0x2f, 0x31, 0x2e, 0x38, 0x2e, 0x34, 0x0d, 0x0a, 0x43, 0x6f, 0x6e, 0x74, 0x65, 0x6e, 0x74, 0x2d,  /* /1.8.4..Content- */
0x54, 0x79, 0x70, 0x65, 0x3a, 0x20, 0x74, 0x65, 0x78, 0x74, 0x2f, 0x68, 0x74, 0x6d, 0x6c, 0x0d,  /* Type: text/html. */
0x0a, 0x43, 0x6f, 0x6e, 0x6e, 0x65, 0x63, 0x74, 0x69, 0x6f, 0x6e, 0x3a, 0x20, 0x63, 0x6c, 0x6f,  /* .Connection: clo */
0x73, 0x65, 0x0d, 0x0a, 0x0d, 0x0a,                                                              /* se.... */
};
...
#else
void    netx_http_proxy_data_fin_test_application_define(void *first_unused_memory)
#endif
{
CHAR    *pointer;
UINT    status;
...
static UINT connected = NX_FALSE;
static UINT disconnected = NX_FALSE;
static void test_establish_notify(NX_TCP_SOCKET *socekt_ptr)
{
    connected = NX_TRUE;
}

static void test_disconnect_callback(NX_TCP_SOCKET *socekt_ptr)
{
    disconnected = NX_TRUE;
}
...
NXD_ADDRESS     server_ip_address;
    ...
    nx_http_proxy_client_enable(&client_ip, &proxy_server_address, HTTP_PROXY_PORT, "user", sizeof("user") - 1, "password", sizeof("password") - 1);
    ...
    status = nx_tcp_socket_create(&client_ip, &test_client, "Test Client Socket", 
                                  ...
                                  NX_NULL, test_disconnect_callback);
    if(status)
        error_counter++;

    nx_tcp_socket_establish_notify(&test_client, test_establish_notify);
    ...
    /* Resume the HTTP Proxy server.  */
    tx_thread_resume(&proxy_thread);

    status = nxd_tcp_client_socket_connect(&test_client, &server_ip_address, TEST_SERVER_PORT, NX_IP_PERIODIC_RATE);

    /* Error response with FIN set.  */
    if (!status || (connected == NX_TRUE) || (disconnected == NX_TRUE))
    {
        error_counter++;
        test_client_stop = 1;
...
NXD_ADDRESS server_ip_address;
    ...
    /* Suspend the thread to make client connection timeout.  */
    tx_thread_suspend(&proxy_thread);
    ...
    /* Set the TCP receive function.  */
    client_ip.nx_ip_tcp_packet_receive = my_tcp_packet_receive;

    /* Send error response to test client and with FIN set.  */
    packet_ptr -> nx_packet_append_ptr = packet_ptr -> nx_packet_prepend_ptr;
    packet_ptr -> nx_packet_length = 0;
    nx_packet_data_append(packet_ptr, connect_403, sizeof(connect_403), &proxy_pool, NX_IP_PERIODIC_RATE);
    status = nx_tcp_socket_send(&agent_server, packet_ptr, NX_IP_PERIODIC_RATE);
    if(status)
        error_counter++;

    /* Disconnet.  */
    nx_tcp_socket_disconnect(&agent_server, NX_NO_WAIT);
...
NX_PACKET       *packet_ptr;
    ...
    printf("NetX Test:   HTTP Proxy Data Fin Test..................................");
...
ULONG          *source_ip, *dest_ip;
    ...
    /* Set FIN bit.  */
    tcp_header_ptr -> nx_tcp_header_word_3 = tcp_header_ptr -> nx_tcp_header_word_3 | NX_TCP_FIN_BIT;
    ...
    dest_ip = &test_client.nx_tcp_socket_connect_ip.nxd_ip_address.v4;
    source_ip = &test_client.nx_tcp_socket_connect_interface -> nx_interface_ip_address;
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Test:   HTTP Proxy Data Fin Test..................................N/A\n"); 
