/* 104.17 When next ACK arrives that acknowledges previously unacknowledged data, set cwnd to ssthresh (the value set in step 2). This is termed `'deflating' the window.  */
...
static void    my_tcp_packet_receive_104_17(NX_IP *ip_ptr, NX_PACKET *packet_ptr);
...
#else
void           netx_104_17_application_define(void *first_unused_memory)
#endif
{
CHAR           *pointer;
UINT           status;
...
NX_PACKET    *my_packet4;
    ...
    printf("NetX Test:   TCP Spec 104.17 Test......................................");
    ...
    advanced_packet_process_callback = my_packet_process_104_17;
    ip_0.nx_ip_tcp_packet_receive = my_tcp_packet_receive_104_17;
    ...
    ssthresh = server_socket.nx_tcp_socket_tx_outstanding_bytes / 2;
    if(ssthresh < 2 * server_socket.nx_tcp_socket_connect_mss)
        ssthresh = 2 * server_socket.nx_tcp_socket_connect_mss;
...
NX_PACKET    *packet_ptr;
    ...
    while(!(status = nx_tcp_socket_receive(&client_socket, &packet_ptr, NX_IP_PERIODIC_RATE)))
    {
    ...
    if(error_counter || (data_packet_counter != 4) || (duplicate_ack_counter != 3) || (ack_counter != 1))
    {
        printf("ERROR!\n");
        test_control_return(1);
    }
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Test:   TCP Spec 104.17 Test......................................N/A\n"); 
