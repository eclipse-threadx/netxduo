/* This NetX test concentrates on sending to and receiving from multiple UDP ports.  */
...
#else
void    netx_udp_multiple_ports_test_application_define(void *first_unused_memory)
#endif
{
...
ULONG       packets_sent, bytes_sent, packets_received, bytes_received, packets_queued, receive_packets_dropped, checksum_errors;
    ...
    printf("NetX Test:   UDP Multiple Ports Test...................................");
    ...
    /* Send 1000 packets to four ports.  */
    thread_0_counter =  0;
    while ((thread_0_counter < 1000) && (error_counter == 0))
    {
        ...
        /* Send the UDP packet.  */
        status =  nx_udp_socket_send(&socket_0, my_packet, IP_ADDRESS(1, 2, 3, 5), 0xE9);
        ...
        /* Send the UDP packet.  */
        status =  nx_udp_socket_send(&socket_0, my_packet, IP_ADDRESS(1, 2, 3, 5), 0xC9);
        ...
        /* Send the UDP packet.  */
        status =  nx_udp_socket_send(&socket_0, my_packet, IP_ADDRESS(1, 2, 3, 5), 0xA9);
    ...
    if ((packets_sent != 1000*4) || (bytes_sent != 1000*4*28))
    {
        error_counter++;
    }
#endif
    /* Check status.  */
    if ((error_counter) || (status) || (thread_0_counter != 1000) || (thread_1_counter != 1000) || 
...
#ifndef NX_DISABLE_TCP_INFO
    if ((udp_packets_sent != 1000*4) || (udp_bytes_sent != 1000*4*28))
    {
        error_counter++;
    }
#endif
    /* Check status.  */
    if ((error_counter) || (status) || (thread_0_counter != 1000) || (thread_1_counter != 1000) || 
        (udp_packets_received) || (udp_bytes_received) || (udp_invalid_packets) || (udp_receive_packets_dropped) || (udp_checksum_errors) || (notify_calls != 1000))
    {
...
NX_PACKET   *my_packet;
    ...
    status = nx_udp_socket_create(&ip_1, &socket_2, "Socket 2", NX_IP_NORMAL, NX_FRAGMENT_OKAY, 0x80, 5);
    ...
    /* Bind the UDP socket to the IP port.  */
    status =  nx_udp_socket_bind(&socket_2, 0xA9, TX_WAIT_FOREVER);
    ...
    /* Create another UDP socket.  */
    status = nx_udp_socket_create(&ip_1, &socket_3, "Socket 3", NX_IP_NORMAL, NX_FRAGMENT_OKAY, 0x80, 5);
    ...
    /* Bind the UDP socket to the IP port.  */
    status =  nx_udp_socket_bind(&socket_3, 0xC9, TX_WAIT_FOREVER);
    ...
    /* Create another UDP socket.  */
    status = nx_udp_socket_create(&ip_1, &socket_4, "Socket 4", NX_IP_NORMAL, NX_FRAGMENT_OKAY, 0x80, 5);
    ...
    /* Bind the UDP socket to the IP port.  */
    status =  nx_udp_socket_bind(&socket_4, 0xE9, TX_WAIT_FOREVER);
    ...
    /* Get 1000 packets for each socket.  */
    thread_1_counter =  0;
    while (thread_1_counter < 1000)
    {

        /* Receive a UDP packet.  */
        status =  nx_udp_socket_receive(&socket_4, &my_packet, TX_WAIT_FOREVER);
        ...
        /* Check status.  */
        if ((status) || (ip_address != IP_ADDRESS(1,2,3,4)) || (port != 0x88))
            break;
        ...
        /* Receive another UDP packet.  */
        status =  nx_udp_socket_receive(&socket_3, &my_packet, TX_WAIT_FOREVER);
        ...
        /* Receive another UDP packet.  */
        status =  nx_udp_socket_receive(&socket_2, &my_packet, TX_WAIT_FOREVER);
...
#endif
{
    ...
    printf("NetX Test:   UDP Multiple Ports Test...................................N/A\n"); 
