...
#else
void    netx_tcp_client_packet_leak_test_application_define(void *first_unused_memory)
#endif
{
...
UINT    status;
    ...
    status = nx_ip_create(&ip_0, "NetX IP Instance 0", IP_ADDRESS(1, 2, 3, 4), 0xFFFF0000UL, &pool_0, _nx_ram_network_driver_1500,
                    pointer, 2048, 1);
    pointer =  pointer + 2048;
...
ULONG   tcp_transmit_window;
    ...
    /* Connect to server again without unbind. */
    nx_tcp_client_socket_connect(&client_socket, IP_ADDRESS(1, 2, 3, 5), 12, 5 * NX_IP_PERIODIC_RATE);
    nx_tcp_socket_disconnect(&client_socket, 5 * NX_IP_PERIODIC_RATE);
...
NX_PACKET *packet_ptr;
    ...
    memcpy(packet_ptr -> nx_packet_prepend_ptr, "ABCDEFGHIJKLMNOPQRSTUVWXYZ  ", 28);
