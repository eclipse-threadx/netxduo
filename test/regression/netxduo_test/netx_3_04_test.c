/*  3.04:TCP, in ESTABLISHED state, 
         MUST return ACK with proper SEQ and ACK numbers after recv a seg with OTW SEQ or unacc ACK number, 
         and remain in same state If the connection is in a synchronized state, 
         any unacceptable segment (out of window sequence number or unacceptable acknowledgment number) 
...
/*  Procedure
    1.Connect
    2.Client sends a packet with OTW SEQ and acceptable ACK number to server
    3.Client should receive an ACK with proper SEQ and ACK numbers
    4.Client sends a packet with proper OTW SEQ and unacceptable ACK number to server
    5.Client should receive an ACK with proper SEQ and ACK numbers  */
...
static void    my_tcp_packet_receive_3_04(NX_IP *ip_ptr, NX_PACKET *packet_ptr);
static void    my_tcp_packet_receive_3_04_2(NX_IP *ip_ptr, NX_PACKET *packet_ptr);
...
#else
void           netx_3_04_application_define(void *first_unused_memory)
#endif
{
CHAR       *pointer;
UINT       status;
...
ULONG      actual_status;
    ...
    ip_0.nx_ip_tcp_packet_receive = my_tcp_packet_receive_3_04;
...
NX_PACKET       *my_packet2;
    ...
    advanced_packet_process_callback = my_packet_process_3_04;
    ip_1.nx_ip_tcp_packet_receive = my_tcp_packet_receive_3_04_2;
...
NX_TCP_HEADER   *header_ptr;
    ...
    if((packet_ptr -> nx_packet_length - 20 == 20) && (!memcmp(packet_ptr -> nx_packet_prepend_ptr + 20, MSG, 20)))
    {
       ...
       if(((header_ptr -> nx_tcp_sequence_number < server_socket.nx_tcp_socket_rx_sequence) ||
          ...
          (header_ptr ->nx_tcp_acknowledgment_number == server_socket.nx_tcp_socket_tx_sequence) )
          data_packet_counter ++;
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Test:   TCP Spec 3.04 Test........................................N/A\n"); 
