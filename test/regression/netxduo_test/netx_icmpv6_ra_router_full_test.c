...
NX_PACKET              *packet_ptr;
    ...
    router_address.nxd_ip_address.v6[3] = 0x00000002;

    /* Set the linklocal address.  */
    if (nxd_ipv6_address_set(&ip_0, 0, &ipv6_address, 10, &address_index))
        error_counter++;

    /* Sleep 5 seconds for DAD. */
    tx_thread_sleep(5 * NX_IP_PERIODIC_RATE);

    /* Loop to add router until full. */
    while (nxd_ipv6_default_router_add(&ip_0, &router_address, 3600, 0) == NX_SUCCESS)
    {
