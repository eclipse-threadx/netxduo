/* 16.22 TCP SHOULD increase exponentially the interval between successive zero window probes.  */
...
#else
void           netx_16_22_application_define(void *first_unused_memory)
#endif
{
    printf("NetX Test:   TCP Spec 16.22 Test.......................................N/A\n");
...
static void    my_tcp_packet_receive_16_22(NX_IP *ip_ptr, NX_PACKET *packet_ptr);
...
NX_PACKET    *my_packet;
    ...
    printf("NetX Test:   TCP Spec 16.22 Test.......................................");
    ...
    /* Modify the timeout_shift transmit parameter of the client aocket. */
    ...
    ip_0.nx_ip_tcp_packet_receive = my_tcp_packet_receive_16_22;
    ...
    /* Sleep 3 seconds ticks to wait for the successive zero window probes.*/
...
NX_PACKET    *packet_ptr;
    ...
    if(server_socket.nx_tcp_socket_receive_queue_head)
    {
        ...
        {
            if(memcmp(packet_ptr -> nx_packet_prepend_ptr, (void*)MSG, packet_ptr -> nx_packet_length) || (packet_ptr -> nx_packet_length != 20))
                error_counter++;
...
NX_TCP_HEADER   *header_ptr;
    ...
    if((header_ptr -> nx_tcp_header_word_3 & NX_TCP_ACK_BIT) && !(header_ptr -> nx_tcp_header_word_3 & NX_LOWER_16_MASK))
    {
        if(ack_counter == 0) 
            ack_counter++;
        else
        {
            /* Check if TCP increase exponentially the interval between successive zero window probes.  */
            if(client_socket.nx_tcp_socket_timeout == 
               client_socket.nx_tcp_socket_timeout_rate << (ack_counter * client_socket.nx_tcp_socket_timeout_shift))
                ack_counter++;
