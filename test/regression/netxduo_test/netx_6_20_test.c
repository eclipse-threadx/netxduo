/* 6.20 If a RECEIVE call arrives on SYN-SENT state, TCP MAY queue the request and for processing after entering ESTABLISHED state.  */

/* Procedure
1. Client sends a SYN packet and becomes SYN_SENT state.
2. Client calls the receive command.
3. Check the status, test error.
4. After entering ESTABLISHED state, check if client receives the packet.*/
...
static UINT    my_packet_process_6_20(NX_IP *ip_ptr, NX_PACKET *packet_ptr, UINT *operation_ptr, UINT *delay_ptr);
...
#else
void           netx_6_20_application_define(void *first_unused_memory)
#endif
{
CHAR       *pointer;
UINT       status;
...
NX_PACKET *packet_ptr;
    ...
    printf("NetX Test:   TCP Spec 6.20 Test........................................");
    ...
    advanced_packet_process_callback = my_packet_process_6_20;
    ...
    /* Call a receive function!  */
    rcv_status = nx_tcp_socket_receive(&client_socket, &packet_ptr, 5 * NX_IP_PERIODIC_RATE);

    status = nx_tcp_socket_state_wait(&client_socket, NX_TCP_ESTABLISHED, NX_IP_PERIODIC_RATE);
    
    /* Check for error.  */
    if(status)
        error_counter++;

    if(rcv_status == NX_SUCCESS)
    {
        /* Check received data and the data length. */
    ...
    else if((rcv_status != NX_SUCCESS) || (data_packet_counter != 1))
    {
        printf("WARNING!\n");
...
#endif
{
    ...
    printf("NetX Test:   TCP Spec 6.20 Test........................................N/A\n"); 
