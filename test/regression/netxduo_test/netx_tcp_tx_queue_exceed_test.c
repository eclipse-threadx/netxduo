/* This NetX test concentrates on the TCP TX queue exceed.  */
...
#else
void    netx_tcp_tx_queue_exceed_test_application_define(void *first_unused_memory)
#endif
{
...
UINT    status;
    ...
    status =  nx_packet_pool_create(&pool_0, "NetX Main Packet Pool", 512, pool_area, sizeof(pool_area));
...
UINT        i;
    ...
    printf("NetX Test:   TCP TX Queue Exceed Test..................................");
    ...
    /* Drop ACK from server to client. */
    ip_0.nx_ip_tcp_packet_receive = tcp_packet_receive;

    for (i = 0; i < NX_TCP_MAXIMUM_TX_QUEUE + 2; i++)
    {
        ...
        status = nx_packet_data_append(my_packet, "A", 1, &pool_0, 2 * NX_NO_WAIT);
        if(status)
            error_counter++;

        /* Send the packet out!  */
        if (i == (NX_TCP_MAXIMUM_TX_QUEUE + 1))
        {
        ...
        if (i < NX_TCP_MAXIMUM_TX_QUEUE)
        {

            /* For first NX_TCP_MAXIMUM_TX_QUEUE packets, send should be always success. */
            if (status)
            {
                error_counter++;
    ...
    /* Recover TCP packet process from server to client. */
...
ULONG           actual_status;
    ...
    /* Wait until client thread finishes testing. */
...
static VOID    tcp_packet_receive(NX_IP *ip_ptr, NX_PACKET *packet_ptr)
{
    ...
    /* Drop packet from server to client. */
...
#endif
{
    ...
    printf("NetX Test:   TCP TX Queue Exceed Test..................................N/A\n"); 
