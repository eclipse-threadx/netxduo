/* 1.01 TCP MUST send a SYN,ACK in response to a SYN in LISTEN state.  */

/* Procedure
   1. Check the state of server is LISTEN.
   2. Client sends a SYN packet.
   3. Check the packet is a SYN one.
   4. Server replies a packet.
   5. Check the packet is a SYN + ACK one.  */
...
static void    my_tcp_packet_receive_1_01(NX_IP *ip_ptr, NX_PACKET *packet_ptr);
static void    my_tcp_packet_receive_1_01_2(NX_IP *ip_ptr, NX_PACKET *packet_ptr);
...
#else
void           netx_1_01_application_define(void *first_unused_memory)
#endif
{
    CHAR       *pointer;
    UINT       status;
...
static void    ntest_0_entry(ULONG thread_input)
{
    ...
    printf("NetX Test:   TCP Spec 1.01 Test........................................");
    ...
    /* Check the state of server is LISTEN.  */
    if (server_socket.nx_tcp_socket_state != NX_TCP_LISTEN_STATE)
        error_counter++;

    /* Let client checks the packet.  */
    ip_0.nx_ip_tcp_packet_receive = my_tcp_packet_receive_1_01;
...
static void    ntest_1_entry(ULONG thread_input)
{
    ...
    /* Let client checks the packet.  */
     ip_1.nx_ip_tcp_packet_receive = my_tcp_packet_receive_1_01_2;
    ...
    if(error_counter||syn_counter!=1||syn_ack_counter!=1)
    {
        printf("ERROR!\n");
        test_control_return(1);
    }
...
void           my_tcp_packet_receive_1_01(NX_IP *ip_ptr, NX_PACKET *packet_ptr)
{
    ...
    /* If receives other packet, it is error.  */
    else
        error_counter++;
...
void           my_tcp_packet_receive_1_01_2(NX_IP *ip_ptr, NX_PACKET *packet_ptr)
{
    NX_TCP_HEADER               *tcp_header_ptr;
    ...
    /* Check whether the packet is a SYN+ACK packet which responsing the SYN one.  */
    if(syn_counter == 1)
    {
    ...
    /* Let client receives the SYN + ACK packet.  */
    _nx_tcp_packet_receive(ip_ptr, packet_ptr); 
}
#else
...
#endif
{
    ...
    printf("NetX Test:   TCP Spec 1.01 Test........................................N/A\n"); 
