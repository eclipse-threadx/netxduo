/* 5.25 If a SEND call arrives on SYN-RCVD state, TCP MAY queue the request and send it after entering ESTABLISHED state.  */
...
static UINT    my_packet_process_5_25(NX_IP *ip_ptr, NX_PACKET *packet_ptr, UINT *operation_ptr, UINT *delay_ptr);
...
#else
void           netx_5_25_application_define(void *first_unused_memory)
#endif
{
...
NX_PACKET  *my_packet;
    ...
    printf("NetX Test:   TCP Spec 5.25 Test........................................");
    ...
    /* Let driver delay SYN+ACK packet. */
    advanced_packet_process_callback = my_packet_process_5_25;

    /* Accept a client socket connection.  */
    nx_tcp_server_socket_accept(&server_socket, NX_NO_WAIT);

    tx_thread_suspend(&ntest_0);

    /*  Send packet in SYN_RECEIVED status.  */
    if (server_socket.nx_tcp_socket_state == NX_TCP_SYN_RECEIVED &&
        client_socket.nx_tcp_socket_state == NX_TCP_SYN_SENT)
    {
        /* Create a packet.  */
        ...
        /* Send the packet out!  */
        nx_tcp_socket_send(&server_socket, my_packet, NX_IP_PERIODIC_RATE);
    }
    
    status = nx_tcp_socket_state_wait(&server_socket, NX_TCP_ESTABLISHED, 5 * NX_IP_PERIODIC_RATE);

    /* Check for error.  */
    if(status)
        error_counter++;

    /* Sleep to let client thread run. */
...
NX_PACKET       *rcv_packet_ptr;
    ...
    if(!status)
    {
        ...
        /* Check received data. */
        if(memcmp(rcv_packet_ptr -> nx_packet_prepend_ptr, "01012345678920123456", 20))
            error_counter++;
    ...
    /* Let client thread disconnect first. */
    ...
    if(error_counter || (syn_ack_counter != 1))
    {
        printf("ERROR!\n");
        test_control_return(1);
    }
...
#endif
{
    ...
    printf("NetX Test:   TCP Spec 5.25 Test........................................N/A\n"); 
