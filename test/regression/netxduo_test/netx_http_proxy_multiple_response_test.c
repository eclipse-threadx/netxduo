/* This case tests HTTP Proxy response in multiple packets. */
#include    "tx_api.h"
#include    "nx_api.h"
...
#else
void    netx_http_proxy_multiple_response_test_application_define(void *first_unused_memory)
#endif
{
CHAR    *pointer;
UINT    status;
...
UINT       i, status;
ULONG      actual_status;
NX_PACKET  *packet_ptr, *packet_ptr_1, *packet_ptr_2;
NXD_ADDRESS server_ip_address;
    ...
    for (i = 0; i < TEST_LOOP; i++)
    {
        ...
        nx_packet_data_append(packet_ptr, connect_200, 5, &proxy_pool, NX_IP_PERIODIC_RATE);
        status = nx_tcp_socket_send(&agent_server, packet_ptr, NX_IP_PERIODIC_RATE);
        if(status)
            error_counter++;
        
        status = nx_packet_allocate(&client_pool, &packet_ptr_1, NX_TCP_PACKET, NX_IP_PERIODIC_RATE);
        if(status)
            error_counter++;
        nx_packet_data_append(packet_ptr_1, connect_200 + 5, 15, &proxy_pool, NX_IP_PERIODIC_RATE);
        status = nx_tcp_socket_send(&agent_server, packet_ptr_1, NX_IP_PERIODIC_RATE);
        if(status)
            error_counter++;
        
        status = nx_packet_allocate(&client_pool, &packet_ptr_2, NX_TCP_PACKET, NX_IP_PERIODIC_RATE);
        if(status)
            error_counter++;
        nx_packet_data_append(packet_ptr_2, connect_200 + 20, sizeof(connect_200) - 20, &proxy_pool, NX_IP_PERIODIC_RATE);
        status = nx_tcp_socket_send(&agent_server, packet_ptr_2, NX_IP_PERIODIC_RATE);
        if(status)
            error_counter++;
...
NX_PACKET       *packet_ptr;
    ...
    printf("NetX Test:   HTTP Proxy Multiple Response Test.........................");
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Test:   HTTP Proxy Multiple Response Test.........................N/A\n"); 
