/* This NetX test concentrates on the ARP static delete all entries operation.  */
...
#else
void    netx_arp_static_entries_delete_test_application_define(void *first_unused_memory)
#endif
{
...
ULONG       invalid_messages;
    ...
    printf("NetX Test:   ARP Static Delete All Entries Processing Test.............");
    ...
    status +=  nx_arp_static_entry_create(&ip_0, IP_ADDRESS(1, 2, 3, 6), 0x0011, 0x22334458);
    status +=  nx_arp_static_entry_create(&ip_0, IP_ADDRESS(1, 2, 3, 7), 0x0011, 0x22334459);
    status +=  nx_arp_static_entry_create(&ip_0, IP_ADDRESS(1, 2, 3, 39), 0x0011, 0x2233445a);
    status +=  nx_arp_static_entry_create(&ip_0, IP_ADDRESS(1, 2, 3, 37), 0x0011, 0x2233445b);
    status +=  nx_arp_static_entry_create(&ip_0, IP_ADDRESS(1, 2, 3, 69), 0x0000, 0x2233445c);
    ...
    /* Find the IP address.  */
    status = nx_arp_ip_address_find(&ip_0, &ip_address, 0x0000, 0x2233445c);
    if ((status) || (ip_address != IP_ADDRESS(1, 2, 3, 69)))
    {
    ...
    /* Find the hardware address.  */
    status = nx_arp_hardware_address_find(&ip_0, IP_ADDRESS(1, 2, 3, 69), &physical_msw, &physical_lsw);
    if ((status) || (physical_msw != 0x0000) || (physical_lsw != 0x2233445c))
    {
    ...
    if ((status) || (requests_received)|| (responses_sent) || (responses_received) || (dynamic_entries) || (static_entries) || (aged_entries) || (invalid_messages))
        error_counter++;
...
#endif
{
    ...
    printf("NetX Test:   ARP Static Delete All Entries Processing Test.............N/A\n"); 
