...
/* Procedure
   ...
   2.Server responses SYN+ACK to Client.
   3.Delay the SYN+ACK packet.
   4.When Server is on the SYN_RECEIVED state, Client sends a RST which contains a OTW SEQ to Server.
   5.Check whether the state has been changed.
   6.When Server is on the ESTABLISHED state, Client sends a RST which contains a OTW SEQ to Server.
   7.Check whether the state has been changed.*/
...
static void    my_tcp_packet_receive_8_29_01(NX_IP *ip_ptr, NX_PACKET *packet_ptr);
...
#else
void           netx_8_29_01_application_define(void *first_unused_memory)
#endif
{
CHAR       *pointer;
UINT       status;
...
ULONG      actual_status;
    ...
    printf("NetX Test:   TCP Spec 8.29.01 Test.....................................");
    ...
    advanced_packet_process_callback = my_packet_process_8_29_01;
    ...
    /* Check Server socket's state.  */
    if((rst_counter != 1) || (server_socket.nx_tcp_socket_state != NX_TCP_SYN_RECEIVED))
        error_counter++;

    status = nx_tcp_socket_state_wait(&server_socket, NX_TCP_ESTABLISHED, 2 * NX_IP_PERIODIC_RATE);
    ...
    /* Check Server socket's state.  */
    if((rst_counter != 2) || (server_socket.nx_tcp_socket_state != NX_TCP_ESTABLISHED))
        error_counter++;
...
NX_TCP_HEADER    header_ptr;
    ...
    /* Send RST  */
    header_ptr.nx_tcp_header_word_3 = NX_TCP_ACK_BIT;
    header_ptr.nx_tcp_acknowledgment_number = server_socket.nx_tcp_socket_rx_sequence - 10;
    ...
    status = nx_tcp_socket_state_wait(&client_socket, NX_TCP_ESTABLISHED, 2 * NX_IP_PERIODIC_RATE);
    ...
    /* Send RST  */
    header_ptr.nx_tcp_header_word_3 = NX_TCP_ACK_BIT;
    header_ptr.nx_tcp_acknowledgment_number = server_socket.nx_tcp_socket_rx_sequence - 20;
    header_ptr.nx_tcp_sequence_number       = server_socket.nx_tcp_socket_tx_sequence;
    _nx_tcp_packet_send_rst(&client_socket, &header_ptr);
    ...
    if((error_counter) || (syn_ack_counter != 1) || (rst_counter != 2))
    {
        printf("ERROR!\n");
        test_control_return(1);
    }
    ...
    if(packet_ptr -> nx_packet_length >= 40)
    {
        ...
        if((tcp_header_ptr -> nx_tcp_header_word_3 & NX_TCP_SYN_BIT) && (tcp_header_ptr -> nx_tcp_header_word_3 & NX_TCP_ACK_BIT) && !(tcp_header_ptr -> nx_tcp_header_word_3 & NX_TCP_RST_BIT))
        {
            ...
            *delay_ptr = 200;
            ...
            ip_0.nx_ip_tcp_packet_receive = my_tcp_packet_receive_8_29_01;
...
NX_TCP_HEADER        *tcp_header_ptr;
    ...
    if(((server_socket.nx_tcp_socket_state == NX_TCP_SYN_RECEIVED) && (rst_counter == 0)) || ((server_socket.nx_tcp_socket_state == NX_TCP_ESTABLISHED) && (rst_counter == 1)))
    {
        /* Check if the packet is a RST one with OTW SEQ number.  */
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Test:   TCP Spec 8.29.01 Test.....................................N/A\n"); 
