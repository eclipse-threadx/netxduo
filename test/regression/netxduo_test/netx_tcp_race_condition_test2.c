/* This NetX test concentrates when one thread is sending data, another thread disconnect the socket.  */
...
static NX_PACKET *test_packet[20];
static UINT       test_packet_count;
...
#else
void    netx_tcp_race_condition_test2_application_define(void *first_unused_memory)
#endif
{
...
NX_PACKET   *my_packet;
    ...
    printf("NetX Test:   TCP Race Condition Test 2.................................");
    ...
    status = nx_packet_data_append(my_packet, send_buffer, 200, &pool_0, 2 * NX_IP_PERIODIC_RATE);
    if(status)
        error_counter++;

    for (test_packet_count = 0; test_packet_count < 20; test_packet_count++)
    {
        status = nx_packet_allocate(&pool_0, &test_packet[test_packet_count], NX_TCP_PACKET, NX_IP_PERIODIC_RATE);
        if (status)
        {
            break;
        }
    ...
    status = nx_packet_data_append(my_packet, send_buffer, 100, &pool_0, 2 * NX_IP_PERIODIC_RATE);
    if(status)
        error_counter++;
    ...
    status = nx_packet_data_append(my_packet, send_buffer, 20, &pool_0, 2 * NX_IP_PERIODIC_RATE);
    if(status)
        error_counter++;
...
#endif
{
    ...
    printf("NetX Test:   TCP Race Condition Test 2.................................N/A\n"); 
