...
static ULONG                   server_reply_counter = 0;
...
#else
void    netx_ftp_establish_data_connection_08_test_application_define(void *first_unused_memory)
#endif
{
...
UINT        status;
    ...
    if(ftp_client.nx_ftp_client_data_socket.nx_tcp_socket_state != NX_TCP_CLOSED)
        error_counter++;
...
UINT    status;
    ...
    printf("NetX Test:   FTP Server Close Data Connection 08 Test..................");
    ...
    if((error_counter) && ( server_reply_counter != 1 ))
    {
        printf("ERROR!\n");
        test_control_return(1);
    }
...
void           my_ftp_packet_receive_client(NX_IP *ip_ptr, NX_PACKET *packet_ptr)
{
    ...
    if((tcp_header_ptr -> nx_tcp_header_word_3 & NX_TCP_ACK_BIT) && (*message >= '0') && (*(message) <= '9'))
    {
        if((!memcmp(message,"250",3)) || (!memcmp(message,"226",3)) )
            server_reply_counter++;
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Test:   FTP Server Close Data Connection 08 Test..................N/A\n"); 
