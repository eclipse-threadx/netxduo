/* Test steps:
   1. Client sends STOR in passive mode;
   2. Client connects data socket and sends data;
   3. Client disconnects data socket;
   4. Client receives the response of STOR;
  */
#include    "tx_api.h"
#include    "fx_api.h" 
#include    "nx_api.h"
...
#else
void    netx_ftp_pasv_stor_test_application_define(void *first_unused_memory)
#endif
{
...
UINT         data_port;
    ...
    /* This sends TYPE commands.  */
    status =  nx_packet_allocate(&client_pool, &my_packet, NX_TCP_PACKET, NX_IP_PERIODIC_RATE);
    memcpy(my_packet -> nx_packet_prepend_ptr, "TYPE I\r\n", 8);
    my_packet -> nx_packet_length =  8;
    ...
    /* Check response for PASV.  */
    status = nx_tcp_socket_receive(&(ftp_client.nx_ftp_client_control_socket), &recv_packet_ptr, NX_IP_PERIODIC_RATE);
    if (status != NX_SUCCESS) 
    {
        error_counter++;
    }
    ...
    {
        buffer_ptr = recv_packet_ptr -> nx_packet_prepend_ptr;
        if ((recv_packet_ptr -> nx_packet_length < 3) || (buffer_ptr[0] != '2'))
        {
            error_counter++;
        }
    ...
    /* This sends PASV commands.  */
    status =  nx_packet_allocate(&client_pool, &my_packet, NX_TCP_PACKET, NX_IP_PERIODIC_RATE);
    memcpy(my_packet -> nx_packet_prepend_ptr, "PASV\r\n", 6);
    my_packet -> nx_packet_length =  6;
    my_packet -> nx_packet_append_ptr =  my_packet -> nx_packet_prepend_ptr + my_packet -> nx_packet_length;
    status = nx_tcp_socket_send(&(ftp_client.nx_ftp_client_control_socket), my_packet, NX_IP_PERIODIC_RATE);
    if (status != NX_SUCCESS) 
    {
    ...
    {
        buffer_ptr = recv_packet_ptr -> nx_packet_prepend_ptr;
        if ((recv_packet_ptr -> nx_packet_length < 3) || (buffer_ptr[0] != '2') || (buffer_ptr[1] != '2') || (buffer_ptr[2] != '7'))
        {
            error_counter++;
        }
        nx_packet_release(recv_packet_ptr);
    ...
    ftp_client.nx_ftp_client_data_socket.nx_tcp_socket_connect_ip.nxd_ip_version = NX_IP_VERSION_V4;
    ftp_client.nx_ftp_client_data_socket.nx_tcp_socket_connect_ip.nxd_ip_address.v4 = FTP_SERVER_ADDRESS;
#else
    ftp_client.nx_ftp_client_data_socket.nx_tcp_socket_connect_ip = FTP_SERVER_ADDRESS;
#endif /* __PRODUCT_NETXDUO__  */
    data_port = ftp_server.nx_ftp_server_data_port - 1;

    /* This sends STOR commands.  */
    status =  nx_packet_allocate(&client_pool, &my_packet, NX_TCP_PACKET, NX_IP_PERIODIC_RATE);
    memcpy(my_packet -> nx_packet_prepend_ptr, "STOR test.txt\r\n", 15);
    my_packet -> nx_packet_length =  15;
    my_packet -> nx_packet_append_ptr =  my_packet -> nx_packet_prepend_ptr + my_packet -> nx_packet_length;
    status = nx_tcp_socket_send(&(ftp_client.nx_ftp_client_control_socket), my_packet, NX_IP_PERIODIC_RATE);
    if (status != NX_SUCCESS) 
    {
    ...
    status =  nx_tcp_client_socket_bind((&ftp_client.nx_ftp_client_data_socket), NX_ANY_PORT, NX_IP_PERIODIC_RATE);
    /* Check for an error.  */
    if (status != NX_SUCCESS) 
    {
        error_counter++;
...
#ifdef __PRODUCT_NETXDUO__
    status =  nxd_tcp_client_socket_connect(&(ftp_client.nx_ftp_client_data_socket),
                                            &(ftp_client.nx_ftp_client_data_socket.nx_tcp_socket_connect_ip),
                                            data_port, NX_IP_PERIODIC_RATE);
#else
    status =  nx_tcp_client_socket_connect(&(ftp_client.nx_ftp_client_data_socket), 
                                           ftp_client.nx_ftp_client_data_socket.nx_tcp_socket_connect_ip,
                                           data_port, NX_IP_PERIODIC_RATE); 
#endif /* __PRODUCT_NETXDUO__  */
    ...
    status =  nx_tcp_socket_send(&(ftp_client.nx_ftp_client_data_socket), my_packet, NX_IP_PERIODIC_RATE);
    if (status != NX_SUCCESS) 
    {
        error_counter++;
    ...
    _nx_ftp_client_data_socket_cleanup(&ftp_client, 50);

    /* Check response for STOR.  */
    status = nx_tcp_socket_receive(&(ftp_client.nx_ftp_client_control_socket), &recv_packet_ptr, 5 * NX_IP_PERIODIC_RATE);
    if (status != NX_SUCCESS) 
    {
        error_counter++;
    }
    ...
    {
        buffer_ptr = recv_packet_ptr -> nx_packet_prepend_ptr;
        if ((recv_packet_ptr -> nx_packet_length < 3) || (buffer_ptr[0] != '1'))
        {
            error_counter++;
        }
        nx_packet_release(recv_packet_ptr);
    ...
    /* Check if file is stored in server.  */
...
void    thread_server_entry(ULONG thread_input)
{
    ...
    printf("NetX Test:   FTP PASV STOR Test........................................");
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Test:   FTP PASV STOR Test........................................N/A\n"); 
