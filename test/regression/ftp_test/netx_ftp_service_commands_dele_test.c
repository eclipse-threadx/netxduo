...
#else
void    netx_ftp_service_commands_dele_test_application_define(void *first_unused_memory)
#endif
{
...
NX_PACKET   *my_packet;
    ...
    status = nx_ftp_client_file_delete(&ftp_client,"test.txt",NX_IP_PERIODIC_RATE);
    if (status)
        error_counter++;
    ...
    status = nx_ftp_client_file_delete(&ftp_client,"/test1.txt",NX_IP_PERIODIC_RATE);
    if (status)
        error_counter++;
    ...
    status =  nx_ftp_client_file_open(&ftp_client, "/test1.txt", NX_FTP_OPEN_FOR_READ, NX_IP_PERIODIC_RATE);
    if (status == NX_SUCCESS) 
        error_counter++;
...
UINT    status;
    ...
    printf("NetX Test:   FTP Service Command DELE Test ............................");
    ...
    if((error_counter) || ( dele_counter != 2 ) || (fail_counter != 2))
    {
        printf("ERROR!\n");
        test_control_return(1);
    }
...
UCHAR            *message;
    ...
    if((tcp_header_ptr -> nx_tcp_header_word_3 & NX_TCP_ACK_BIT) && *(message) == 'D')
    {
        if(!memcmp(message,"DELE test.txt",13))
            dele_counter++;
        else if(!memcmp(message,"DELE /test1.txt",15))
        {
            dele_counter++;
...
UCHAR            *message;
    ...
    {
        if(!memcmp(message,"550 File Open Fail",18))
            fail_counter++;
    }
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Test:   FTP Service Command DELE Test ............................N/A\n"); 
