...
static UINT                    type_counter = 0;;
static UINT                    sp_counter = 0;
static UINT                    client_counter = 0;
...
#else
void    netx_ftp_user_data_type_application_define(void *first_unused_memory)
#endif
{
...
UINT    status;
    ...
    printf("NetX Test:   FTP User Data Type Test...................................");
    ...
    if((error_counter) || (type_counter != 1) || (client_counter != 1) || (sp_counter != 1))
    {
        printf("ERROR!\n");
        test_control_return(1);
    }
...
UCHAR            *message;
    ...
    if((tcp_header_ptr -> nx_tcp_header_word_3 & NX_TCP_ACK_BIT) && (*message == 'T'))
    {
        /* Check if The parameters are separated by a <SP>  */
        if((!memcmp(message,"TYPE",4)) && (*(message + 4) == ' '))
            sp_counter++;

        /* Check if the default Type is "I' (which means the ASCII) */
        if(*(message + 5) == 'I')
        {
            type_counter++;
            client_counter++;
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Test:   FTP User Data Type Test...................................N/A\n"); 
