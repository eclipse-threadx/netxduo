/* This test case focus on a bug that nx_ftp_client_request_packet will be released multiple times in NX_FTP_QUIT command process. */
#include    "tx_api.h"
#include    "fx_api.h" 
#include    "nx_api.h"
...
#else
void    netx_ftp_server_dangling_pinter_test_application_define(void *first_unused_memory)
#endif
{
...
UINT        i;
    ...
    for (i = 0; i <= NX_FTP_MAX_CLIENTS; i++)
    {
        ...
        if (i != NX_FTP_MAX_CLIENTS)
        {
            status =  send_data("TYPE I\n");
            if (status) 
                error_counter++;

            status =  send_data("STOR 1.txt\n");
            if (status) 
                error_counter++;

            status =  send_data("RNFR 1.txt\n");
            if (status) 
                error_counter++;
        }

        status =  send_data("QUIT\n");
        if (status) 
            error_counter++;
...
UINT    status;
    ...
    printf("NetX Test:   FTP Server Dangling Pointer Test..........................");
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Test:   FTP Server Dangling Pointer Test..........................N/A\n");
