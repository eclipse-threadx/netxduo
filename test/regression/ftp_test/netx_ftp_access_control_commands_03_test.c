...
static ULONG                   logg_in_counter = 0;
...
#else
void    netx_ftp_access_control_commands_03_test_application_define(void *first_unused_memory)
#endif
{
...
ULONG       control_port;
    ...
    control_port = ftp_client.nx_ftp_client_control_socket.nx_tcp_socket_port;
    ...
    memcpy(my_packet -> nx_packet_prepend_ptr, "USER name1", 10);

    *(my_packet -> nx_packet_prepend_ptr + 10) = 13;
    *(my_packet -> nx_packet_prepend_ptr + 11) = 10;

    /* Adjust the write pointer.  */
    my_packet -> nx_packet_length =  12;
    my_packet -> nx_packet_append_ptr =  my_packet -> nx_packet_prepend_ptr + 12;

    status =  nx_tcp_socket_send(&(ftp_client.nx_ftp_client_control_socket), my_packet, NX_IP_PERIODIC_RATE);
    if (status) 
        error_counter++;

    status =  nx_tcp_socket_receive(&(ftp_client.nx_ftp_client_control_socket), &my_packet, NX_IP_PERIODIC_RATE);    
    if (status) 
        error_counter++;
    else
    {
        if(memcmp(my_packet->nx_packet_prepend_ptr,"33",2))
            error_counter++;
        else
        {
            /* Allocate a FTP packet.  */
            ...
            memcpy(my_packet -> nx_packet_prepend_ptr, "PASS password1", 14);
            *(my_packet -> nx_packet_prepend_ptr + 14) = 13;
            *(my_packet -> nx_packet_prepend_ptr + 15) = 10;

            /* Adjust the write pointer.  */
            my_packet -> nx_packet_length =  16;
            my_packet -> nx_packet_append_ptr =  my_packet -> nx_packet_prepend_ptr + 16;

            status =  nx_tcp_socket_send(&(ftp_client.nx_ftp_client_control_socket), my_packet, NX_IP_PERIODIC_RATE);
            if (status) 
                error_counter++;

            /* Check if the transfer parameters has been changed */
            if(ftp_client.nx_ftp_client_control_socket.nx_tcp_socket_port != control_port)
                error_counter++;
    ...
    /* Allocate a FTP packet.  */
    status =  nx_packet_allocate(&client_pool, &data_packet, NX_TCP_PACKET, NX_IP_PERIODIC_RATE);
    if (status) 
        error_counter++;

    /* Write ABCs into the packet payload!  */
    memcpy(data_packet -> nx_packet_prepend_ptr, "ABCDEFGHIJKLMNOPQRSTUVWXYZ  ", 28);

    /* Adjust the write pointer.  */
    data_packet -> nx_packet_length =  28;
    data_packet -> nx_packet_append_ptr =  data_packet -> nx_packet_prepend_ptr + 28;

    /* Write the packet to the file test.txt.  */
    status =  nx_ftp_client_file_write(&ftp_client, data_packet, NX_IP_PERIODIC_RATE);
    if (status)
        error_counter++;

    data_port = ftp_client.nx_ftp_client_data_port;
    ...
    memcpy(my_packet -> nx_packet_prepend_ptr, "USER name2", 10);
    ...
    *(my_packet -> nx_packet_prepend_ptr + 11) = 10;
    ...
    else
    {
        ...
        else
        {
            ...
            memcpy(my_packet -> nx_packet_prepend_ptr, "PASS password2", 14);
            *(my_packet -> nx_packet_prepend_ptr + 14) = 13;
            *(my_packet -> nx_packet_prepend_ptr + 15) = 10;
            ...
            /* Check if the transfer parameters has been changed */
            if((ftp_client.nx_ftp_client_data_port != data_port) && (ftp_client.nx_ftp_client_control_socket.nx_tcp_socket_port != control_port))
                error_counter++;
        }
    }
...
CHAR    type;
    ...
    printf("NetX Test:   FTP Server Access Control Commands User 03 Test...........");
    ...
    /* Check if the transfer parameter has been changed. */
    type = ftp_server.nx_ftp_server_client_list[0].nx_ftp_client_request_transfer_type;

    tx_thread_sleep(2 * NX_IP_PERIODIC_RATE);

    /* Check if it has flushed the old user, passwd */
    if((memcmp(ftp_server.nx_ftp_server_client_list[0].nx_ftp_client_request_username,"name1",5)) || (memcmp(ftp_server.nx_ftp_server_client_list[0].nx_ftp_client_request_password,"password1",9)))
        error_counter++;

    /* Check if the transfer parameter has been changed. */
    if(type != ftp_server.nx_ftp_server_client_list[0].nx_ftp_client_request_transfer_type)
        error_counter++;
    ...
    /* Check if it has flushed the old user, passwd */
    if((memcmp(ftp_server.nx_ftp_server_client_list[0].nx_ftp_client_request_username,"name2",5)) || (memcmp(ftp_server.nx_ftp_server_client_list[0].nx_ftp_client_request_password,"password2",9)))
        error_counter++;
    ...
    if((error_counter) && ( logg_in_counter != 3 ))
    {
        printf("ERROR!\n");
        test_control_return(1);
    }
...
UCHAR            *message;
    ...
    if(!memcmp(message,"230 Logged in",13))
        logg_in_counter++;

    if(logg_in_counter == 3)
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Test:   FTP Server Access Control Commands User 03 Test...........N/A\n"); 
