...
extern VOID  _nx_ftp_server_command_present(NX_TCP_SOCKET *control_socket_ptr);

#ifdef CTEST
VOID test_application_define(void *first_unused_memory)
#else
void    netx_ftp_server_abnormal_packet_test_application_define(void *first_unused_memory)
#endif
{
...
static UCHAR valid_buffer[4];
static UCHAR *valid_data_ptr = NX_NULL;
static VOID tcp_receive_callback(NX_TCP_SOCKET *tcp_socket)
{
NX_PACKET *packet_ptr = tcp_socket -> nx_tcp_socket_receive_queue_head;
UCHAR *buffer_ptr;

    _nx_ftp_server_command_present(tcp_socket);
    ...
    if (buffer_ptr[20] == 'C')
    {
        ...
        /* Move payload to the end of the packet.  */
        memmove(packet_ptr -> nx_packet_data_end - packet_ptr -> nx_packet_length, buffer_ptr, packet_ptr -> nx_packet_length);
        packet_ptr -> nx_packet_prepend_ptr = packet_ptr -> nx_packet_data_end - packet_ptr -> nx_packet_length;
        packet_ptr -> nx_packet_append_ptr = packet_ptr -> nx_packet_data_end;
        valid_data_ptr = packet_ptr -> nx_packet_data_end;
        memcpy(valid_buffer, valid_data_ptr, sizeof(valid_buffer));
    }
}
...
UCHAR       *buffer_ptr;
    ...
    /* Allocate packet.  */
    status = _nx_ftp_client_packet_allocate(&ftp_client, &my_packet, NX_IP_PERIODIC_RATE);
    if (status)
        error_counter++;

    ftp_server.nx_ftp_server_client_list[0].nx_ftp_client_request_control_socket.nx_tcp_receive_callback = tcp_receive_callback;
    ...
    /* Check if the buffer was modified.  */
    if (valid_data_ptr)
    {
        if (memcmp(valid_buffer, valid_data_ptr, sizeof(valid_buffer)) != 0)
            error_counter++;
    }
...
UINT    status;
    ...
    printf("NetX Test:   FTP Server Abnormal Packet Test...........................");
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Test:   FTP Server Abnormal Packet Test...........................N/A\n"); 
