...
#else
void    netx_ftp_service_commands_rename_test_application_define(void *first_unused_memory)
#endif
{
...
NX_PACKET   *my_packet;
    ...
    status = nx_ftp_client_file_rename(&ftp_client,"test.txt","test1.txt",NX_IP_PERIODIC_RATE);
    if (status)
        error_counter++;
...
UINT    status;
    ...
    printf("NetX Test:   FTP Service Command Rename Test ..........................");
    ...
    if((error_counter) || ( rename_counter != 2 ) || (fail_counter != 1))
    {
        printf("ERROR!\n");
        test_control_return(1);
    }
...
UCHAR            *message;
    ...
    if((tcp_header_ptr -> nx_tcp_header_word_3 & NX_TCP_PSH_BIT ) && (tcp_header_ptr -> nx_tcp_header_word_3 & NX_TCP_ACK_BIT) )
    {
        if(*(message) == 'R')
        {
            if(!memcmp(message,"RNFR test.txt",13))
            {
                if(rename_counter == 0)
                    rename_counter++;
            }
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Test:   FTP Service Command Rename Test ..........................N/A\n"); 
