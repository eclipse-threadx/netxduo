...
static ULONG                   fail_counter = 0;
static ULONG                   nlist_counter = 0;
static ULONG                   type_counter = 0;
...
#else
void    netx_ftp_service_commands_nlist_test_application_define(void *first_unused_memory)
#endif
{
...
UCHAR   buffer_list[26];
    ...
    status = nx_ftp_client_directory_create(&ftp_client,"/abc",NX_IP_PERIODIC_RATE/2);
    if (status)
        error_counter++;

    status = nx_ftp_client_directory_create(&ftp_client,"def",NX_IP_PERIODIC_RATE/2);
    if (status)
        error_counter++;
    ...
    /* Create a File.  */
    status =  nx_ftp_client_file_open(&ftp_client, "/abc/test2.txt", NX_FTP_OPEN_FOR_WRITE, NX_IP_PERIODIC_RATE);
    if (status) 
        error_counter++;
    ...
    status = nx_ftp_client_directory_create(&ftp_client,"cpp",NX_IP_PERIODIC_RATE/2);
    if (status)
        error_counter++;
    ...
    status = nx_ftp_client_directory_listing_get(&ftp_client,"/abc",&my_packet,NX_IP_PERIODIC_RATE/2);
    if (status)
        error_counter++;
    else if(memcmp(buffer_list,my_packet->nx_packet_prepend_ptr,15))
            error_counter++;

    status = nx_ftp_client_directory_listing_continue(&ftp_client,&my_packet,NX_IP_PERIODIC_RATE/2);
    if (status != NX_FTP_END_OF_LISTING)
        error_counter++;

    memcpy(buffer_list,"abc",3);
    buffer_list[3] = 13;
    buffer_list[4] = 10;
    memcpy(buffer_list + 5,"def",3);
    buffer_list[8] = 13;
    buffer_list[9] = 10;
    memcpy(buffer_list + 10,"test1.txt",9);
    buffer_list[19] = 13;
    buffer_list[20] = 10;
    memcpy(buffer_list + 21,"cpp",3);
    buffer_list[24] = 13;
    buffer_list[25] = 10;

   status = nx_ftp_client_directory_listing_get(&ftp_client,"",&my_packet,NX_IP_PERIODIC_RATE/2);
    if (status)
        error_counter++;
    else if(memcmp(my_packet->nx_packet_prepend_ptr,buffer_list,25))
            error_counter++;
    ...
    status = nx_ftp_client_directory_listing_continue(&ftp_client,&my_packet,NX_IP_PERIODIC_RATE/2);
    if (status != NX_FTP_EXPECTED_2XX_CODE)
        error_counter++;
...
UINT    status;
    ...
    printf("NetX Test:   FTP Service Command NLIST Test ...........................");
    ...
    if((error_counter) || ( nlist_counter != 1 ) || (fail_counter != 1) || (type_counter >= 1))
    {
        printf("ERROR!\n");
        test_control_return(1);
    }
...
UCHAR            *message;
    ...
    if((tcp_header_ptr -> nx_tcp_header_word_3 & NX_TCP_PSH_BIT ) && (tcp_header_ptr -> nx_tcp_header_word_3 & NX_TCP_ACK_BIT) )
    {
        if(*message == 'T')
        {
            if(*(message + 5) == 'A' || *(message + 5) == 'E')
                type_counter++;
        }
        else if(*message == 'N')
        {
            if (!memcmp(message,"NLST /abc",9))
                nlist_counter++;
...
UCHAR            *message;
    ...
    if((tcp_header_ptr -> nx_tcp_header_word_3 & NX_TCP_ACK_BIT) && *(message) == '5')
    {
        if(!memcmp(message,"550 List bad Directory",18))
        {
            fail_counter++;
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Test:   FTP Service Command NLIST Test ...........................N/A\n"); 
