...
static ULONG                   control_disconnect_counter = 0;
static ULONG                   server_disconnect_counter = 0;
...
#else
void    netx_ftp_establish_data_connection_05_test_application_define(void *first_unused_memory)
#endif
{
...
UINT        status;
    ...
    /* Make a situation that the control connection is closed legally */
    nx_tcp_socket_disconnect(&(ftp_client.nx_ftp_client_control_socket), NX_IP_PERIODIC_RATE);
    if (status)
        error_counter++;
    ...
    if(ftp_client.nx_ftp_client_control_socket.nx_tcp_socket_state == NX_TCP_CLOSED)
        control_disconnect_counter++;

    if(ftp_client.nx_ftp_client_data_socket.nx_tcp_socket_state != NX_TCP_ESTABLISHED)
...
UINT    status;
    ...
    printf("NetX Test:   FTP Server Close Data Connection 05 Test..................");
    ...
    if((error_counter) || (control_disconnect_counter != 1) || (server_disconnect_counter != 1))
    {
        printf("ERROR!\n");
        test_control_return(1);
    }
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Test:   FTP Server Close Data Connection 05 Test..................N/A\n"); 
