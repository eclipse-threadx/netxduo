/* No.4 Files are transferred only via the data connection. 
    rfc959,  page 9, 3. DATA TRANSFER FUNCTIONS */
#include    "tx_api.h"
#include    "fx_api.h" 
#include    "nx_api.h"
...
#else
void    netx_ftp_data_connection_test_application_define(void *first_unused_memory)
#endif
{
...
UINT        status;
    ...
    /* If after closing the client data socket we can also successfully transfer FTP commands, then we can make sure that the The control connection is used for the transfer of commands.*/
    status = nx_tcp_socket_disconnect(&(ftp_client.nx_ftp_client_data_socket), NX_IP_PERIODIC_RATE);
    if(status)
        error_counter++;

    ftp_client.nx_ftp_client_data_socket.nx_tcp_socket_state = NX_TCP_CLOSED;
    ...
    /* Delete the socket.  */
    status = nx_tcp_socket_delete(&ftp_client.nx_ftp_client_data_socket);

    /* Check for error.  */
    if(status)
        error_counter++;

    /* Disconnect the data socket connection we can find that we cannot successfully transfer the files. */
    /* Read the file.  */
    status =  nx_ftp_client_file_read(&ftp_client, &my_packet, NX_IP_PERIODIC_RATE);
...
void    thread_server_entry(ULONG thread_input)
{
    ...
    printf("NetX Test:   FTP Data Connection Test..................................");
    ...
    if((error_counter != 0) || (data_control_counter != 2))
    {
        printf("ERROR!\n");
        test_control_return(1);
    }
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Test:   FTP Data Connection Test..................................N/A\n"); 
