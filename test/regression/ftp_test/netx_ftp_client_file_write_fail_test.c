/* This is a small demo of NetX FTP on the high-performance NetX TCP/IP stack.  This demo 
   ...
   to the server and a directory listing get using passive file transfer (PASV).  */
...
static UCHAR                   zero_data_buffer[4096];  
...
static  char stor_150_response[30] = {
0x31, 0x35,                                     /* C.....15 */
0x30, 0x20, 0x4f, 0x70, 0x65, 0x6e, 0x69, 0x6e, /* 0 Openin */
...
static  int stor_150_response_size = 30;
...
#else
void    netx_ftp_client_file_write_fail_test_application_define(void *first_unused_memory)
#endif
{
...
UCHAR   *pointer;
    ...
    status =  nx_packet_pool_create(&client_pool, "NetX Client Packet Pool", PACKET_PAYLOAD, pointer, 20*PACKET_PAYLOAD);
    ...
    pointer =  pointer + 20*PACKET_PAYLOAD;
...
UINT        status;
    ...
    status =  nx_ftp_client_connect(&ftp_client, FTP_SERVER_ADDRESS, "User", "password", 5 * NX_IP_PERIODIC_RATE);
    ...
    status = nx_ftp_client_directory_default_set(&ftp_client, "\\Users\\Test", 2 * NX_IP_PERIODIC_RATE);
    if (status != NX_SUCCESS) 
    {
        error_counter++;
    }
    ...
    status = nx_ftp_client_directory_listing_get(&ftp_client, "", &my_packet, 3 * NX_IP_PERIODIC_RATE);
    if (status != NX_SUCCESS) 
    {
        error_counter++;
    }
    ...
    do
    {
        status =  nx_ftp_client_directory_listing_continue(&ftp_client, &recv_packet_ptr, 2 * NX_IP_PERIODIC_RATE);
        if (status == NX_SUCCESS)
        {
    ...
    /* Request a file downloaded. This sends PASV and STOR commands and opens another data socket.  */
    status =  nx_ftp_client_file_open(&ftp_client, "socket_tcp_client_recv2.pl", NX_FTP_OPEN_FOR_WRITE, NX_IP_PERIODIC_RATE);
    if (status != NX_SUCCESS) 
    {
        error_counter++;
    }
    ...
    do 
    {
        ...
        else
        {
            ...
            /* Append data larger than TCP window size to trigger a failure during file write.  */
            status = nx_packet_data_append(my_packet, zero_data_buffer, sizeof(zero_data_buffer), &client_pool, NX_NO_WAIT);
        }

        status =  nx_ftp_client_file_write(&ftp_client, my_packet, NX_NO_WAIT);
        packets_sent++;
    ...
    } while ((status == NX_SUCCESS) && (packets_sent < 3));
...
UINT         i;
    ...
    printf("NetX Test:   FTP File Write Fail Test.................................."); 
    ...
    if (status)
    {
     ...
     /* this is the second PASV request. */
    status =  nx_tcp_socket_receive(&server_socket, &my_packet, 10 * NX_IP_PERIODIC_RATE);
    ...
    status =  nx_tcp_server_socket_accept(&server_socket_passive,  NX_IP_PERIODIC_RATE);
    ...
    if (status)
    {
     ...
     /* this is responding to the STOR command. */
    status =  nx_tcp_socket_receive(&server_socket, &my_packet, 10 * NX_IP_PERIODIC_RATE);
    ...
    if (status)
    {
     ...
     /* this is the file data packet */
    status =  nx_tcp_socket_receive(&server_socket_passive, &my_packet, 10 * NX_IP_PERIODIC_RATE);
    ...
    else
    {
     ...
     /* this is the second data packet */
    status =  nx_tcp_socket_receive(&server_socket_passive, &my_packet, 10 * NX_IP_PERIODIC_RATE);
    ...
    status = nx_ftp_response_packet_send(&server_socket, 80, 8);
    ...
    /* Disconnect and delete the TCP socket.  */
    status = nx_tcp_socket_disconnect(&server_socket, 100);
    ...
    if (client_pool.nx_packet_pool_invalid_releases != 0)
    {
        error_counter++;
    }
...
/* Download data responses */
    ...
    ftp_response[7].ftp_response_pkt_data = &stor_150_response[0];
    ftp_response[7].ftp_response_pkt_size = stor_150_response_size  ;     
    
    ftp_response[8].ftp_response_pkt_data = &download_complete_226_response[0];
    ftp_response[8].ftp_response_pkt_size = download_complete_226_response_size  ;  
        
    ftp_response[9].ftp_response_pkt_data = &quit_221_response[0];
    ftp_response[9].ftp_response_pkt_size = quit_221_response_size  ; 
}
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Test:   FTP File Write Fail Test..................................N/A\n");
