...
static ULONG                   mkd_counter = 0;
static ULONG                   rmd_counter = 0;
static ULONG                   dir_fail_counter = 0;
...
#else
void    netx_ftp_service_commands_m_d_dir_test_application_define(void *first_unused_memory)
#endif
{
...
NX_PACKET   *my_packet;
    ...
    /*To ensure the making dir successfully*/
    status = nx_ftp_client_directory_default_set(&ftp_client, "/abc" ,NX_IP_PERIODIC_RATE/2);
    if (status)
        error_counter++;
    ...
    status = nx_ftp_client_directory_default_set(&ftp_client, "/def" ,NX_IP_PERIODIC_RATE/2);
    if (status)
        error_counter++;

    client_ip.nx_ip_tcp_packet_receive = my_ftp_packet_receive_client;
    status = nx_ftp_client_directory_delete(&ftp_client,"/abc",NX_IP_PERIODIC_RATE/2);
    if (status != NX_FTP_EXPECTED_2XX_CODE)
        error_counter++;

    status = nx_ftp_client_directory_default_set(&ftp_client, "/" ,NX_IP_PERIODIC_RATE/2);
    if (status)
        error_counter++;

    status = nx_ftp_client_directory_delete(&ftp_client,"def",NX_IP_PERIODIC_RATE/2);
    if (status)
        error_counter++;

    /*Check the response when deleting a dir deleted.*/
    status = nx_ftp_client_file_delete(&ftp_client, "/abc/test.txt",NX_IP_PERIODIC_RATE/2);
    if (status)
        error_counter++;
    ...
    /*Check if the two dir have been successfully deleted. */
    status = nx_ftp_client_directory_default_set(&ftp_client, "/abc" ,NX_IP_PERIODIC_RATE/2);
...
UINT    status;
    ...
    printf("NetX Test:   FTP Server Service Commands make dele dir Test............");
    ...
    if((error_counter) || ( rmd_counter != 2 ) || (mkd_counter != 2) || (dir_fail_counter != 5))
    {
        printf("ERROR!\n");
        test_control_return(1);
    }
...
void           my_ftp_packet_receive_server(NX_IP *ip_ptr, NX_PACKET *packet_ptr)
{
    ...
    if(!memcmp(message,"MKD /abc ",8) || !memcmp(message,"MKD def ",7))
        mkd_counter++;
    else if(!memcmp(message,"RMD /abc ",8))
        rmd_counter++;
    else if (!memcmp(message,"RMD def ",7))
    {
        rmd_counter++;
...
UCHAR            *message;
    ...
    if((tcp_header_ptr -> nx_tcp_header_word_3 & NX_TCP_ACK_BIT) && *(message) == '5')
    {
        ...
        if(!memcmp(message,"550 Delete Directory Fail",25) || !memcmp(message,"550 Change Dir Fail",19))
            dir_fail_counter++;
    }
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Test:   FTP Server Service Commands make dele dir Test............N/A\n"); 
