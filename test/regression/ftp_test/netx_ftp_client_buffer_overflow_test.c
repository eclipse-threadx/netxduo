...
#if !defined(NX_DISABLE_IPV4) && !defined(NX_DISABLE_LOOPBACK_INTERFACE)

#define     DEMO_STACK_SIZE         2048
#define     PACKET_PAYLOAD          1536
...
#define FTP_SERVER_ADDRESS  IP_ADDRESS(127,0,0,1)
...
static  UINT   nx_ftp_response_packet_send(NX_TCP_SOCKET *server_socket, ULONG packet_type, UCHAR *data, UINT data_size);
...
NX_PACKET   *my_packet;
    ...
    printf("NetX Test:   FTP Client Buffer Overflow Test...........................");
    ...
    status = nx_tcp_socket_create(&client_ip, &server_socket, "Socket Server", NX_IP_NORMAL, NX_FRAGMENT_OKAY, 
    ...
    status =  nx_tcp_server_socket_listen(&client_ip, SERVER_PORT, &server_socket, 5, NX_NULL);
    ...
    if (nx_ftp_response_packet_send(&server_socket, (client_pool.nx_packet_pool_payload_size - welcome_220_response_1_size) & 0xFFFFFFFC,
                                    welcome_220_response_1, welcome_220_response_1_size))
    {
        error_counter++;
    }

    if (nx_ftp_response_packet_send(&server_socket, (client_pool.nx_packet_pool_payload_size - welcome_220_response_2_size) & 0xFFFFFFFC,
                                    welcome_220_response_2, welcome_220_response_2_size))
    {
        error_counter++;
    }
    ...
    if (nx_ftp_response_packet_send(&server_socket, (client_pool.nx_packet_pool_payload_size - password_request_331_size) & 0xFFFFFFFC,
                                    password_request_331, password_request_331_size))
    {
        error_counter++;
    }
    ...
    if (nx_ftp_response_packet_send(&server_socket, (client_pool.nx_packet_pool_payload_size - password_request_331_size) & 0xFFFFFFFC,
                                    logged_in_230_response, logged_in_230_response_size))
    {
        error_counter++;
    }
    ...
    while (client_pool.nx_packet_pool_available)
    {
        if (nx_packet_allocate(&client_pool, &my_packet, 0, NX_NO_WAIT) ||
            (my_packet -> nx_packet_pool_owner != &client_pool))
        {
...
NX_PACKET   *response_packet;
    ...
    status =  nx_packet_allocate(&client_pool, &response_packet, packet_type, TX_WAIT_FOREVER);
    ...
    status =  nx_tcp_socket_send(server_socket, response_packet, NX_IP_PERIODIC_RATE);
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Test:   FTP Client Buffer Overflow Test...........................N/A\n"); 
