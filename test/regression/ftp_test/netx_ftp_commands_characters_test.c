...
static ULONG                   up_counter = 0;
static ULONG                   low_counter = 0;
static ULONG                   command_success_counter = 0;
...
#else
void    netx_ftp_commands_characters_test_application_define(void *first_unused_memory)
#endif
{
...
NX_PACKET   *my_packet;
    ...
    memcpy(my_packet -> nx_packet_prepend_ptr, "Mkd /def", 8);

    *(my_packet -> nx_packet_prepend_ptr + 8) = 13;
    *(my_packet -> nx_packet_prepend_ptr + 9) = 10;

    /* Adjust the write pointer.  */
    my_packet -> nx_packet_length =  10;
    my_packet -> nx_packet_append_ptr =  my_packet -> nx_packet_prepend_ptr + 10;
...
UINT    status;
    ...
    printf("NetX Test:   FTP Commands Characters Test..............................");
    ...
    if((error_counter) || (up_counter != 1) || (low_counter != 1) || (command_success_counter != 2))
    {
        printf("ERROR!\n");
        test_control_return(1);
    }
...
void           my_ftp_packet_receive_server(NX_IP *ip_ptr, NX_PACKET *packet_ptr)
{
    ...
    if((tcp_header_ptr -> nx_tcp_header_word_3 & NX_TCP_ACK_BIT) && *message == 'M' )
    {

        if(!memcmp(message,"MKD /abc ",8))
            up_counter++;
        else if(!memcmp(message,"Mkd /def ",8))
        {
            low_counter++;
...
UCHAR            *message;
    ...
    if((packet_ptr -> nx_packet_length > 23) && (!memcmp(message,"200",3)))
        command_success_counter++;
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Test:   FTP Commands Characters Test..............................N/A\n"); 
