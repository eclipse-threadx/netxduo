...
static ULONG                   cdup_counter = 0;
static ULONG                   success_counter = 0;
...
#else
void    netx_ftp_access_control_commands_04_test_application_define(void *first_unused_memory)
#endif
{
...
NX_PACKET   *my_packet;
    ...
    nx_ftp_client_directory_create(&ftp_client,"/abc", NX_IP_PERIODIC_RATE);
    if (status)
        error_counter++;

    status = nx_ftp_client_directory_default_set(&ftp_client, "/abc" , NX_IP_PERIODIC_RATE);
    if (status)
        error_counter++;
    ...
    memcpy(my_packet -> nx_packet_prepend_ptr, "CDUP ", 5);

    *(my_packet -> nx_packet_prepend_ptr + 5) = 13;
    *(my_packet -> nx_packet_prepend_ptr + 6) = 10;

    /* Adjust the write pointer.  */
    my_packet -> nx_packet_length =  7;
    my_packet -> nx_packet_append_ptr =  my_packet -> nx_packet_prepend_ptr + 7;
    ...
    nx_ftp_client_directory_delete(&ftp_client,"/abc",NX_IP_PERIODIC_RATE);
    if (status)
        error_counter++;
...
UINT    status;
    ...
    printf("NetX Test:   FTP Server Access Control Commands CDUP Test..............");
    ...
    if((error_counter) && ( success_counter != 2 ) && (cdup_counter != 1))
    {
        printf("ERROR!\n");
        test_control_return(1);
    }
...
void           my_ftp_packet_receive_server(NX_IP *ip_ptr, NX_PACKET *packet_ptr)
{
    ...
    if((tcp_header_ptr -> nx_tcp_header_word_3 & NX_TCP_ACK_BIT) && (*message == 'C'))
    {

        if(!memcmp(message,"CDUP ",5))
        {
            cdup_counter++;
...
UCHAR            *message;
    ...
    if(!memcmp(message,"250 \"/abc\" set successfully",26))
        success_counter++;

    else if(!memcmp(message,"250 \"/\" set successfully",23))
    {
        success_counter++;
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Test:   FTP Server Access Control Commands CDUP Test..............N/A\n"); 
