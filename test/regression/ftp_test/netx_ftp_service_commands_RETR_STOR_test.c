...
#define FTP_CLIENT_2_ADDRESS  IP_ADDRESS(1,2,3,6)
...
#else
void    netx_ftp_service_commands_RETR_STOR_test_application_define(void *first_unused_memory)
#endif
{
UINT    status;
UCHAR   *pointer;
    ...
    /* Create an IP instance for the FTP client.  */
    status = nx_ip_create(&client_ip_2, "NetX Client IP Instance", FTP_CLIENT_2_ADDRESS, 0xFFFFFF00UL, 
                          &client_pool, _nx_ram_network_driver_512, pointer, 2048, 1);
    pointer = pointer + 2048;
    if (status)
        error_counter++;
    ...
    /* Enable ARP and supply ARP cache memory for the FTP Client IP.  */
    status = nx_arp_enable(&client_ip_2, (void *) pointer, 1024);
    pointer = pointer + 1024;
    if (status)
        error_counter++;

    /* Enable TCP for client IP instance.  */
    status = nx_tcp_enable(&client_ip_2);
    if (status)
        error_counter++;
...
UINT    status;
    ...
    printf("NetX Test:   FTP Service Command RETR STOR rest .......................");
    ...
    if((error_counter != 0) || (retr_counter != 1) || (stor_counter != 1))
    {
        printf("ERROR!\n");
        test_control_return(1);
    }
...
UCHAR            *message;
    ...
    if((tcp_header_ptr -> nx_tcp_header_word_3 & NX_TCP_ACK_BIT) && ((*(message) == 'R') || *(message) == 'S'))
    {
        if(!memcmp(message,"STOR test.txt",13))
            stor_counter++;

        else if(!memcmp(message,"RETR test.txt",13))
        {
            retr_counter++;
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Test:   FTP Service Command RETR STOR rest .......................N/A\n"); 
