...
0xf0, 0xbc, 0x00, 0x00, 0x84, 0x00, 0x00, 0x00, /* ........ */
...
NX_MDNS_SERVICE service;
    ...
    printf("NetX Test:   MDNS One Shot Query Test..................................");
    ...
    if(nx_mdns_service_one_shot_query(&mdns_0, NX_NULL, "_http._tcp", NX_NULL, &service, 5 * NX_IP_PERIODIC_RATE))
        error_counter++;

    /* Sleep 5 seconds and check whether more queries are sent. */
    tx_thread_sleep(5 * NX_IP_PERIODIC_RATE);
...
UINT       status;
    ...
    /* Do not reply response for the first two queries. */
    if(query_received < 3)
        return NX_TRUE;

    /* Only one query is expected. */
    if(query_received > 3)
    {
        error_counter++;
        return NX_TRUE;
    ...
    else
    {
        ...
        /* Inject a response packet for the third query. */
...
#endif
{
    printf("NetX Test:   MDNS One Shot Query Test..................................N/A\n"); 
