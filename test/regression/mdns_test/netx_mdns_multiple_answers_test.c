...
UINT       status;
    ...
    status = nx_packet_pool_create(&pool_0, "NetX Main Packet Pool", 1536, pointer, 8192);
...
NXD_ADDRESS address;
    ...
    printf("NetX Test:   MDNS Multiple Answers Test................................");
    ...
    /* Create two services. */
    status = nx_mdns_service_add(&mdns_0, (CHAR *)"ARMMDNSTest", (CHAR *)"_http._tcp", NX_NULL, "paper=A4;version=01", 100, 0, 0, 80, NX_MDNS_RR_SET_UNIQUE, 0);
    status += nx_mdns_service_add(&mdns_0, (CHAR *)"Simple Web Server", (CHAR *)"_http._tcp", NX_NULL, "paper=A4;version=01", 100, 0, 0, 80, NX_MDNS_RR_SET_UNIQUE, 0);
    if(status)
        error_counter++;
    ...
    /* Sleep one second and check received answers. */
    tx_thread_sleep(100);
    if(answer_received != 2)
        error_counter++;
...
UCHAR *pointer;
    ...
    if(((*pointer << 8) + *(pointer + 1)) != (NX_MDNS_RESPONSE_FLAG | NX_MDNS_AA_FLAG))
    {
        ...
        /* It is not a response packet. */
        return NX_TRUE;
    }

    /* Get the answer RRs. */
    pointer = packet_ptr -> nx_packet_prepend_ptr + 34;
    answer_received = (*pointer << 8) + *(pointer + 1);
...
#endif
{
    ...
    printf("NetX Test:   MDNS Multiple Answers Test................................N/A\n"); 
