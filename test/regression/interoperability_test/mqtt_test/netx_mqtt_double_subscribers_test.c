...
#define  CLIENT_0_ID_STRING         "mysecondtestclient"
...
static ULONG                        client_0_memory[2000 / sizeof(ULONG)];

/* Declare the MQTT thread stack space. */
static ULONG                        mqtt_client_stack[MQTT_CLIENT_STACK_SIZE / sizeof(ULONG)];
static ULONG                        mqtt_client_0_stack[MQTT_CLIENT_STACK_SIZE / sizeof(ULONG)];
...
static UCHAR metadata_buffer_0[5*4096];
...
UCHAR remote_issuer_buffer_0[2000];
...
UINT tls_setup_method_0(NXD_MQTT_CLIENT* client_ptr, NX_SECURE_TLS_SESSION* tls_session,
        NX_SECURE_X509_CERT* certificate, NX_SECURE_X509_CERT* trusted_certificate)
{
...
static UCHAR tls_packet_buffer_0[4096];
...
INT         test_result = 0;
    ...
    /* Create the second MQTT client instance. */
    status = nxd_mqtt_client_create(&mqtt_client_0, "my_client_0", CLIENT_0_ID_STRING, strlen(CLIENT_0_ID_STRING),
                                    &ip_0, &pool_0, (VOID*)mqtt_client_0_stack, sizeof(mqtt_client_0_stack), 
                                    MQTT_THREAD_PRIORTY,
                                    (UCHAR*)client_0_memory, sizeof(client_0_memory));
    exit_if_fail(NX_SUCCESS == status, TLS_TEST_UNKNOWN_TYPE_ERROR);
    
    /* Create a tls session. */
    status = nx_secure_tls_session_create(&(mqtt_client.nxd_mqtt_tls_session),
                                          &nx_crypto_tls_ciphers,
                                          metadata_buffer,
                                          sizeof(metadata_buffer));
    status += nx_secure_tls_session_packet_buffer_set(&(mqtt_client.nxd_mqtt_tls_session), tls_packet_buffer, sizeof(tls_packet_buffer));
    exit_if_fail(NX_SUCCESS == status, TLS_TEST_UNKNOWN_TYPE_ERROR);

    /* Create a tls session for the second MQTT client instance. */
    status = nx_secure_tls_session_create(&(mqtt_client_0.nxd_mqtt_tls_session),
                                          &nx_crypto_tls_ciphers,
                                          metadata_buffer_0,
                                          sizeof(metadata_buffer_0));
    status += nx_secure_tls_session_packet_buffer_set(&(mqtt_client_0.nxd_mqtt_tls_session), tls_packet_buffer_0, sizeof(tls_packet_buffer_0));
    exit_if_fail(NX_SUCCESS == status, TLS_TEST_UNKNOWN_TYPE_ERROR);
    ...
    /* So as the second MQTT client instance. */
    status = nxd_mqtt_client_secure_connect(&mqtt_client_0, &server_ip, MQTT_PORT, 
                                            tls_setup_method_0,
                                            MQTT_KEEP_ALIVE_TIMER, 1, NX_WAIT_FOREVER);
    exit_if_fail(NX_SUCCESS == status, TLS_TEST_UNKNOWN_TYPE_ERROR);

    status = nxd_mqtt_client_subscribe(&mqtt_client_0, TOPIC_NAME, strlen(TOPIC_NAME), QOS0);
    add_error_counter_if_fail(NX_SUCCESS == status, test_result);
    ...
    /* Though the publisher put the semaphore, Wait for a moment. */
    ...
    /* So as the second MQTT client instance. */
    status = nxd_mqtt_client_message_get(&mqtt_client_0, topic_buffer, sizeof(topic_buffer), &topic_length, 
                                         message_buffer, sizeof(message_buffer), &message_length);
    printf("func: %s, line %d, status = %d.\n", __func__, __LINE__, status);
    add_error_counter_if_fail(NX_SUCCESS == status, test_result);
    ...
    add_error_counter_if_fail(NX_SUCCESS == status, test_result);
    status = nxd_mqtt_client_unsubscribe(&mqtt_client_0, TOPIC_NAME, strlen(TOPIC_NAME));
    add_error_counter_if_fail(NX_SUCCESS == status, test_result);

    /* Disconnect from the broker. */
    status = nxd_mqtt_client_disconnect(&mqtt_client);
    add_error_counter_if_fail(NX_SUCCESS == status, test_result);
    status = nxd_mqtt_client_disconnect(&mqtt_client_0);
    add_error_counter_if_fail(NX_SUCCESS == status, test_result);

    /* Delete the client instance, release all the resources. */
    status = nxd_mqtt_client_delete(&mqtt_client);
    add_error_counter_if_fail(NX_SUCCESS == status, test_result);
    status = nxd_mqtt_client_delete(&mqtt_client_0);
    add_error_counter_if_fail(NX_SUCCESS == status, test_result);
