...
NX_PARAMETER_NOT_USED(thread_input);
    ...
    status = nx_secure_dtls_ecc_initialize(&dtls_session, nx_crypto_ecc_supported_groups,
                                           nx_crypto_ecc_supported_groups_size,
                                           nx_crypto_ecc_curves);
    show_error_message_if_fail( NX_SUCCESS == status);

    // Initialize the CA certificate - it does not have a private RSA key
    status = nx_secure_x509_certificate_initialize(&device_issuer_certificate, ECCA2_der, ECCA2_der_len, NX_NULL, 0, NX_NULL, 0, NX_SECURE_X509_KEY_TYPE_NONE);
    show_error_message_if_fail( NX_SUCCESS == status);
