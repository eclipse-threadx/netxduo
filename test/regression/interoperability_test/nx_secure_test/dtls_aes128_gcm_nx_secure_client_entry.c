...
#if !defined(NX_SECURE_TLS_CLIENT_DISABLED) && defined(NX_SECURE_ENABLE_DTLS) && defined(NX_SECURE_ENABLE_AEAD_CIPHER)
...
static VOID ciphersuites_setup(NX_SECURE_TLS_CRYPTO *tls_ciphers)
{
UINT i;
UINT status;
UINT count;
    ...
    i = 0;
    while (TLS_RSA_WITH_AES_128_GCM_SHA256 !=
           (UINT)_nx_crypto_ciphersuite_lookup_table[i].nx_secure_tls_ciphersuite)
    {
        i++;
    }
    tls_ciphers -> nx_secure_tls_ciphersuite_lookup_table = &_nx_crypto_ciphersuite_lookup_table[i];
...
NX_PARAMETER_NOT_USED(thread_input);
    ...
    ciphersuites_setup(&tls_ciphers_client);
