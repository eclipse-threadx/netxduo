...
#else /* !defined(NX_SECURE_TLS_CLIENT_DISABLED) && defined(NX_SECURE_ENABLE_DTLS) */
...
NX_PARAMETER_NOT_USED(thread_input);
    ...
    status = nx_secure_x509_certificate_initialize(&certificate, test_client_der, test_client_der_len, NX_NULL, 0, test_client_key_der, test_client_key_der_len, NX_SECURE_X509_KEY_TYPE_RSA_PKCS1_DER);
    show_error_message_if_fail( NX_SUCCESS == status);
    status = nx_secure_dtls_session_local_certificate_add(&dtls_session, &certificate, 1);
    show_error_message_if_fail( NX_SUCCESS == status);
    ...
    show_error_message_if_fail( NX_SUCCESS == status);
    status = nx_secure_dtls_session_trusted_certificate_add(&dtls_session, &device_issuer_certificate, 2);
    show_error_message_if_fail( NX_SUCCESS == status);
    ...
    exit_if_fail('h' == ((CHAR*)receive_buffer)[0], TLS_TEST_UNKNOWN_TYPE_ERROR);
    exit_if_fail('e' == ((CHAR*)receive_buffer)[1], TLS_TEST_UNKNOWN_TYPE_ERROR);
    exit_if_fail('l' == ((CHAR*)receive_buffer)[2], TLS_TEST_UNKNOWN_TYPE_ERROR);
    exit_if_fail('l' == ((CHAR*)receive_buffer)[3], TLS_TEST_UNKNOWN_TYPE_ERROR);
    exit_if_fail('o' == ((CHAR*)receive_buffer)[4], TLS_TEST_UNKNOWN_TYPE_ERROR);
