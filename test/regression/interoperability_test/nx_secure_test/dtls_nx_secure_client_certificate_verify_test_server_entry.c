...
#if !defined(NX_SECURE_TLS_SERVER_DISABLED) && defined(NX_SECURE_ENABLE_CLIENT_CERTIFICATE_VERIFY) && defined(NX_SECURE_ENABLE_DTLS)
...
#else /* !NX_SECURE_TLS_SERVER_DISABLED && NX_SECURE_ENABLE_CLIENT_CERTIFICATE_VERIFY && NX_SECURE_ENABLE_DTLS */

    exit(TLS_TEST_NOT_AVAILABLE);

#endif /* !NX_SECURE_TLS_SERVER_DISABLED && NX_SECURE_ENABLE_CLIENT_CERTIFICATE_VERIFY && NX_SECURE_ENABLE_DTLS */
...
void server_thread_entry(ULONG thread_input)
{
    ...
    nx_secure_dtls_server_x509_client_verify_configure(&dtls_server, 2, remote_cert_buffer, sizeof(remote_cert_buffer));

    memset(&certificate, 0, sizeof(certificate));
    status = nx_secure_x509_certificate_initialize(&certificate,
                                                   test_server_der, test_server_der_len,
                                                   NX_NULL, 0, test_server_key_der,
                                                   test_server_key_der_len, NX_SECURE_X509_KEY_TYPE_RSA_PKCS1_DER);
    show_error_message_if_fail( NX_SUCCESS == status);
