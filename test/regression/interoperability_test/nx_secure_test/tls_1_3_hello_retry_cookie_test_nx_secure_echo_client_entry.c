...
static ULONG client_callback(NX_SECURE_TLS_SESSION *tls_session, NX_SECURE_TLS_HELLO_EXTENSION *extensions, UINT num_extensions)
{
...
NX_PARAMETER_NOT_USED(thread_input);
    ...
    /* Reset the client callback function. */
    tls_session.nx_secure_tls_session_client_callback = NX_NULL;
    ...
    /* End the TLS session. This is required to properly shut down the TLS connection. */
    status = nx_secure_tls_session_end(&tls_session, NX_NO_WAIT);
    //exit_if_fail(NX_SUCCESS == status, TLS_TEST_UNKNOWN_TYPE_ERROR);

    /* Close the TCP connection. */
    status =  nx_tcp_socket_disconnect(&tcp_socket, NX_NO_WAIT);
    //exit_if_fail(NX_SUCCESS == status, TLS_TEST_UNKNOWN_TYPE_ERROR);
