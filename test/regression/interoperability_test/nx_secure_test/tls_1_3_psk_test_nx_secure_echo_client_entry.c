...
#if !defined(NX_SECURE_TLS_CLIENT_DISABLED) && defined(NX_SECURE_ENABLE_ECC_CIPHERSUITE) && (NX_SECURE_TLS_TLS_1_3_ENABLED) && defined(NX_SECURE_ENABLE_PSK_CIPHERSUITES)
...
ULONG       bytes;
UCHAR psk_test[] = {0x11, 0x22, 0x33, 0x44, 0x55, 0x66};
NX_PARAMETER_NOT_USED(thread_input);
    ...
    status =  nx_tcp_client_socket_bind(&tcp_socket, LOCAL_CLIENT_PORT, NX_WAIT_FOREVER);
    exit_if_fail(NX_SUCCESS == status, TLS_TEST_UNKNOWN_TYPE_ERROR);
    ...
    /* Add psk.  */
    status = nx_secure_tls_client_psk_set(&tls_session, psk_test, sizeof(psk_test), "psk_test", 8, "", 0);
    exit_if_fail(NX_SUCCESS == status, TLS_TEST_UNKNOWN_TYPE_ERROR);
    status = nx_secure_tls_psk_add(&tls_session, psk_test, sizeof(psk_test), "psk_test", 8, "", 0);
    exit_if_fail(NX_SUCCESS == status, TLS_TEST_UNKNOWN_TYPE_ERROR);
    ...
    status = nx_tcp_client_socket_connect(&tcp_socket, REMOTE_IP_ADDRESS_NUMBER, REMOTE_SERVER_PORT, NX_WAIT_FOREVER);
    exit_if_fail(NX_SUCCESS == status, TLS_TEST_UNKNOWN_TYPE_ERROR);
