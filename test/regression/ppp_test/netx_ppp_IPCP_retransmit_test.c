...
#else
void    netx_ppp_IPCP_retransmit_test_application_define(void *first_unused_memory)
#endif
{
...
NX_PACKET   *my_packet;
    ...
    printf("NetX Test:   PPP IPCP Retransmit Test..................................");
    ...
    status =  nx_ip_status_check(&ip_0, NX_IP_ADDRESS_RESOLVED, &ip_status, 30 * NX_IP_PERIODIC_RATE);
...
NX_PACKET   *my_packet;
    ...
    /* Wait for the link to come up.  */
    status =  nx_ip_status_check(&ip_1, NX_IP_ADDRESS_RESOLVED, &ip_status, 30 * NX_IP_PERIODIC_RATE);
...
static UCHAR temp_data[8] = {0};
static UCHAR temp_index = 0;
static UINT i = 0;
static UINT packet_drop = NX_FALSE;
static UINT packet_end = NX_FALSE;

static UCHAR configure_request_ack[] = {0x7E, 0xFF, 0x7D, 0x23, 0xc0, 0x21, 0x7D, 0x22};

static void    ppp_0_serial_byte_output(UCHAR byte)
{

    /* Discard LCP Configure Request ACK before NX_PPP_IPCP_CONFIGURE_REQUEST_SENT_STATE. */
    if (ppp_0.nx_ppp_ipcp_state < NX_PPP_IPCP_CONFIGURE_REQUEST_SENT_STATE)
    {
        ...
        if (temp_index < 8)
        {
            temp_data[temp_index++] = byte;
        }
        else if (memcmp(temp_data, configure_request_ack, 8) != 0)
        {
            packet_drop = NX_FALSE;
        }
        ...
        else
        {
            for (;i < temp_index; i++)
            {
                nx_ppp_byte_receive(&ppp_1, temp_data[i]);
            }
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Test:   PPP IPCP retransmit Test..................................N/A\n"); 
