...
/* Define the PPP_REGRESSION_TEST option to run in regression test environment. */
...
#else
void    netx_ppp_PAP_bad_username_test_application_define(void *first_unused_memory)
#endif
{
...
UINT    status;
    ...
    status = nx_ip_create(&ip_0, "NetX IP Instance 0", IP_ADDRESS(0, 0, 0, 0), 0xFFFFF000UL, 
    ...
    status += nx_ip_create(&ip_1, "NetX IP Instance 1", IP_ADDRESS(0, 0, 0, 0), 0xFFFFF000UL, 
                           &pool_0, nx_ppp_driver, pointer, 2048, 1);
    pointer =  pointer + 2048;
...
ULONG       ip_status;
    ...
    printf("NetX Test:   PPP PAP Bad Username Test.................................");
    if (error_counter)
    {
    ...
    /* Check status.  */
    if ((status) || (nak_counter != 1) || (login_attempts != 2) ||
    ...
    /* Delete PPP 0, PAP failed, we don't need to finish IPCP handshake..  */
...
static UINT generate_login(CHAR *name, CHAR *password)
{
    ...
    if (login_attempts == 0)
    {
         /* Input a wrong username and good password, called "yname" and "mypassword".  */
         name[0] = 'y';  /* This username will not match the one in verify login; PAP should fail. */
         name[1] = 'n';
         name[2] = 'a';
         name[3] = 'm';
         name[4] = 'e';
    ...
    else /* if (login_attempts >= 1)*/
    {
        ...
        /* Try a corrected username, called "myname" and "mypassword".  */
        name[0] = 'm';
        name[1] = 'y';
        name[2] = 'n';
        name[3] = 'a';
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Test:   PPP PAP Bad Username Test.................................N/A\n"); 
