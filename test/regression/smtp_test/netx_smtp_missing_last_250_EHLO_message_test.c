...
/* This uses one packet for the server's EHLO response to client. Otherwise it sends
   two packets for the EHLO response. */
...
extern char response_250_ehlo_first_pkt[145];
extern int response_250_ehlo_first_size;

extern char response_250_ehlo_notlast_pkt[10];
extern int response_250_ehlo_notlast_size;
...
#else
void    netx_smtp_missing_last_250_EHLO_message_test_application_define(void *first_unused_memory)
#endif
{
...
UINT        wait_timeout = 300;
    ...
    printf("NetX Test:   SMTP Missing Last Packet EHLO Message Test................");
    ...
    /* Check for errors. This mail should not succeed! */
    if (status == NX_SUCCESS)
    {
         error_counter++;
    }
...
ULONG      actual_status;
    ...
    for (i = 0; i < MSG_COUNT; i++ )
    {
        ...
        else
        {
            ...
            status =  nx_tcp_socket_receive(&server_socket, &my_packet, 50 * NX_IP_PERIODIC_RATE);
        ...
        /* Check if this if the first EHLO packet. */
        if (i == 1)
        {

            /* It is. Wait a beat and send the second one. */
            i++;
            tx_thread_sleep(2);
...
static void     smtp_test_initialize()
{
    ...
    smtp_test[1].smtp_test_pkt_data = &response_250_ehlo_first_pkt[0];
    smtp_test[1].smtp_test_pkt_size = response_250_ehlo_first_size;  
    smtp_test[2].smtp_test_pkt_data = &response_250_ehlo_notlast_pkt[0];
    smtp_test[2].smtp_test_pkt_size = response_250_ehlo_notlast_size;  
...
#endif
{
    ...
    printf("NetX Test:   SMTP Missing Last Packet EHLO Message Test................N/A\n"); 
