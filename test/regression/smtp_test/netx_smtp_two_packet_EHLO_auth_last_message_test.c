/* This NetX tests the SMTP client for handling a server 250 reply to EHLO which has multiple
   ...
   Note that the Client authentication type must be PLAIN for the sequence to succeed. */
...
#define     SERVER_IPADR            IP_ADDRESS(10, 2, 41, 1)
#define     CLIENT_IPADR            IP_ADDRESS(10, 2, 41, 10)
...
static NXD_ADDRESS address_client;
...
/* These next two are the first and second packets of the EHLO message */
extern char response_250_ehlo_first_no_auth_pkt[145];
extern int response_250_ehlo_first_no_auth_size;

extern char response_250_ehlo_last_with_auth_pkt[42];
extern int response_250_ehlo_last_with_auth_size;

extern char response_334_pkt[6];
extern int response_334_size;
...
#else
void    netx_smtp_two_packet_EHLO_auth_last_message_test_application_define(void *first_unused_memory)
#endif
{
...
CHAR    *pointer;
    ...
    server_ip_address.nxd_ip_address.v6[1] = 0x00010001;
    server_ip_address.nxd_ip_address.v6[2] = 0x021122FF;
    server_ip_address.nxd_ip_address.v6[3] = 0xFE334456;

    address_client.nxd_ip_version = NX_IP_VERSION_V6;
    address_client.nxd_ip_address.v6[0] = 0x20010DB8;
    address_client.nxd_ip_address.v6[1] = 0x00010001;
    address_client.nxd_ip_address.v6[2] = 0x021122FF;
    address_client.nxd_ip_address.v6[3] = 0xFE334499;

    status = nxd_ipv6_address_set(&client_ip, 0, &server_ip_address, 64, NX_NULL);
    status = nxd_ipv6_address_set(&server_ip, 0, &address_client, 64, NX_NULL);
...
UINT        wait_timeout = 300;
    ...
    printf("NetX Test:   SMTP Two Packet EHLO Message Auth Last Test...............");


#ifdef FEATURE_NX_IPV6
    /* Sleep 5 seconds to finish DAD and let server set up.  */
...
ULONG      actual_status;
   ...
   tx_thread_sleep(5 * NX_IP_PERIODIC_RATE);
    ...
    for (i = 0; i < MSG_COUNT; i++ )
    {
        ...
        if (i == 0)
        {
            /* This is the greeting, we don't wait to hear from client...*/
        }
        else
        {
            ...
            if (status)
            {
                ...
                {
                    /* If this is not the EHLO message, this is an error. */
                    error_counter++;
                    continue;
                }
            }
            else if (i == 2)
            {
                ...
                /* The server should not receive a packet. The Client is not supposed to respond yet. */
                error_counter++;
                continue;
            }
            ...
            else if (i == 3)
            {
                ...
                /* The server should not receive another EHLO message. */
                if (!memcmp(my_packet -> nx_packet_prepend_ptr, "EHLO", 4))
                {
...
static void     smtp_test_initialize()
{
    ...
    smtp_test[1].smtp_test_pkt_data = &response_250_ehlo_first_no_auth_pkt[0];
    smtp_test[1].smtp_test_pkt_size = response_250_ehlo_first_no_auth_size;  
    smtp_test[2].smtp_test_pkt_data = &response_250_ehlo_last_with_auth_pkt[0];
    smtp_test[2].smtp_test_pkt_size = response_250_ehlo_last_with_auth_size;  
    smtp_test[3].smtp_test_pkt_data = &response_334_pkt[0];
    smtp_test[3].smtp_test_pkt_size = response_334_size;   
...
#endif
{
    ...
    printf("NetX Test:   SMTP Two Packet EHLO Message Auth Last Test...............N/A\n"); 
