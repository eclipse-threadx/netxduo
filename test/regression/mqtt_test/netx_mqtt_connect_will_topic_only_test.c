...
#else
void       netx_mqtt_client_connect_will_topic_only_application_define(void *first_unused_memory)
#endif
{
CHAR       *pointer;
UINT       status;
...
UINT i;
    ...
    printf("NetX Test:   MQTT Connect Will Topic Only Test ........................");
    ...
    /* Test again with no will message, no Will RETAIN flag, and QoS set to 0 */
    will_retain = 0;
    will_QoS = 0;
    ...
    for (i = 0; i < TEST_LOOP; i++)
    {
        ...
        status = nxd_mqtt_client_will_message_set(client_ptr, WILL_TOPIC, strlen(WILL_TOPIC), NX_NULL,
                                                  0, will_retain, will_QoS);
        if (status)
                SET_ERROR_COUNTER(&error_counter, __FILE__, __LINE__);
...
UINT        i;
    ...
    for (i = 0; i < TEST_LOOP; i++)
    {
        ...
        else
        {
            ...
            content[1] = (UCHAR)(sizeof(fixed_header) + strlen(CLIENT_ID) + strlen(WILL_TOPIC) + 2 + 2);
