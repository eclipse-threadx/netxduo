...
/* MQTT malformed PUBLISH test.  This test case validates that the MQTT client releases the
   packet carrying a malformed PUBLISH instead of leaking it, that the malformed message is not
   delivered to the application, and that the client keeps receiving afterwards.  The broker
   sends one PUBLISH for each of the three checks that make _nxd_mqtt_process_publish() return
   NXD_MQTT_INVALID_PACKET, then a well-formed one.  */
...
#else
void       netx_mqtt_malformed_publish_application_define(void *first_unused_memory)
#endif
{
CHAR       *pointer;
UINT       status;
...
/* This thread sets up MQTT client, connects, and checks what it receives from the broker. */
static void    ntest_0_entry(ULONG thread_input)
{
UINT        status;
...
UINT        i;
    ...
    printf("NetX Test:   MQTT Malformed Publish Test ..............................");
    ...
    for (i = 0; i < TEST_LOOP; i++)
    {
        ...
        /* The broker sends three malformed PUBLISH messages followed by a well-formed one.
           Only the well-formed one must reach the application.  */
        status = tx_event_flags_get(&mqtt_app_flag, DEMO_MESSAGE_EVENT, TX_OR_CLEAR, &events, 5 * NX_IP_PERIODIC_RATE);
        if (status)
            SET_ERROR_COUNTER(&error_counter, __FILE__, __LINE__);

        status = nxd_mqtt_client_message_get(client_ptr, topic, sizeof(topic), &topic_length,
                                             message, sizeof(message), &message_length);
        if (status)
            SET_ERROR_COUNTER(&error_counter, __FILE__, __LINE__);

        if ((topic_length != strlen(TOPIC)) || strncmp(TOPIC, (CHAR *)topic, topic_length) ||
            (message_length != strlen(MESSAGE)) || strncmp(MESSAGE, (CHAR *)message, message_length))
            SET_ERROR_COUNTER(&error_counter, __FILE__, __LINE__);

        /* Nothing else was queued: the malformed messages were dropped.  */
        status = nxd_mqtt_client_message_get(client_ptr, topic, sizeof(topic), &topic_length,
                                             message, sizeof(message), &message_length);
        if (status != NXD_MQTT_NO_MESSAGE)
            SET_ERROR_COUNTER(&error_counter, __FILE__, __LINE__);
    ...
    /* Check packet leak.  Before the fix, every malformed PUBLISH left one packet unreleased.  */
...
/* Remaining length of 1: below the 2 bytes of the topic length field.  */
static UCHAR publish_short_remaining_length[] = {0x30, 0x01, 0x00};

/* Topic length of 5 with only 2 bytes remaining after the topic length field.  */
static UCHAR publish_topic_past_end[] = {0x30, 0x04, 0x00, 0x05, 't', 'o'};

/* QoS 1 with a 2-byte topic and no room left for the packet identifier.  */
static UCHAR publish_qos1_no_packet_id[] = {0x32, 0x04, 0x00, 0x02, 't', 'o'};

/* Well-formed QoS 0 PUBLISH: fixed header, topic length, topic, payload.  */
static UCHAR publish_valid[] = {0x30, 0x0E, 0x00, 0x05, 't', 'o', 'p', 'i', 'c', 'm', 'e', 's', 's', 'a', 'g', 'e'};

/* Send one message from the broker, over TLS when a session is passed.  */
static void    server_send(VOID *tls_session_ptr, UCHAR *data, UINT length)
{
UINT       status;
NX_PACKET *packet_ptr;
#ifdef NX_SECURE_ENABLE
NX_SECURE_TLS_SESSION *tls_session = (NX_SECURE_TLS_SESSION *)tls_session_ptr;
#else
    NX_PARAMETER_NOT_USED(tls_session_ptr);
#endif
    ...
    memcpy(packet_ptr -> nx_packet_prepend_ptr, data, length);
    ...
    /* Keep each message in its own TCP segment, so each one is parsed on its own.  */
    tx_thread_sleep(1);
}
...
UINT                   i;
    ...
    for (i = 0; i < TEST_LOOP; i++)
    {
        ...
        if (i == 1)
        {
            tls_session = &tls_server_session;
        ...
        else
        {
            ...
            /* Response with CONNACK, SUCCESS */
            content[0] = 0x20;
            content[1] = 0x02;
            content[2] = 0;
            content[3] = 0;
            server_send(tls_session, content, 4);

            /* Three malformed PUBLISH messages, one per check in _nxd_mqtt_process_publish().  */
            server_send(tls_session, publish_short_remaining_length, sizeof(publish_short_remaining_length));
            server_send(tls_session, publish_topic_past_end, sizeof(publish_topic_past_end));
            server_send(tls_session, publish_qos1_no_packet_id, sizeof(publish_qos1_no_packet_id));

            /* Then a well-formed one, which the client must still deliver.  */
            server_send(tls_session, publish_valid, sizeof(publish_valid));
