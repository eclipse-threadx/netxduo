...
#else
void       netx_mqtt_client_connect_will_message_application_define(void *first_unused_memory)
#endif
{
CHAR       *pointer;
UINT       status;
...
UINT i;
    ...
    printf("NetX Test:   MQTT Connect Will Message Test ...........................");
...
UINT        i;
    ...
    for (i = 0; i < TEST_LOOP; i++)
    {
        ...
        else
        {
            ...
            index = sizeof(fixed_header) + 2 + strlen(CLIENT_ID);

            /* Append Will Topic and Will Mesage. */
            content[index] = (strlen(WILL_TOPIC) >> 8) & 0xFF;
            content[index + 1] = (strlen(WILL_TOPIC)) & 0xFF;
            memcpy(content + index + 2, WILL_TOPIC, strlen(WILL_TOPIC));

            index = index + 2 + strlen(WILL_TOPIC);

            content[index] = (strlen(WILL_MESSAGE) >> 8) & 0xFF;
            content[index + 1] = (strlen(WILL_MESSAGE)) & 0xFF;
            memcpy(content + index + 2, WILL_MESSAGE, strlen(WILL_MESSAGE));
            index = index + 2 + strlen(WILL_MESSAGE);

            content[1] = (UCHAR)(sizeof(fixed_header) + strlen(CLIENT_ID) + strlen(WILL_MESSAGE) + strlen(WILL_TOPIC) + 4);
            ...
            if (memcmp(packet_ptr->nx_packet_prepend_ptr, content, index))
                SET_ERROR_COUNTER(&error_counter, __FILE__, __LINE__);
