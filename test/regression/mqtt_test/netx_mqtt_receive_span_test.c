...
static void publish_mqtt_message(UINT i, UCHAR *data, UINT size);
...
UINT i;
    ...
    printf("NetX Test:   MQTT Receive Span Test....................................");
...
UINT        i;
    ...
    for (i = 0; i < TEST_LOOP; i++)
    {
        ...
        else
        {
            ...
            memcpy(byte + index, byte, index);

            /* 1. Send fixed_header field. */
            publish_mqtt_message(i, content, 1);

            /* 2. Send length field. */
            publish_mqtt_message(i, content + 1, 1);

            /* 3. Send another byte. */
            publish_mqtt_message(i, content + 2, 1);

            /* 4. Send remaining bytes of first message plus one bytes of second message. */
            publish_mqtt_message(i, content + 3, index - 2);

            /* 5. Send remaining bytes of second message. */
            publish_mqtt_message(i, content + index + 1, index - 1);
...
NX_PACKET *packet_ptr;
    ...
    memcpy(packet_ptr->nx_packet_prepend_ptr, data, size);
