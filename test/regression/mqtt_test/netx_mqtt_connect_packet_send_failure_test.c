...
#else
void       netx_mqtt_client_connect_packet_send_failure_application_define(void *first_unused_memory)
#endif
{
CHAR       *pointer;
UINT       status;
...
UINT i;
    ...
    printf("NetX Test:   MQTT Connect Packet Send Failure Test ........................................");
    ...
    for (i = 0; i < TEST_LOOP; i++)
    {
        ...
        if (i == 0)
        {
            /* Validate invalid length behavior. */
            client_ptr -> nxd_mqtt_client_will_topic = "hacker_topic\r\n";
            client_ptr -> nxd_mqtt_client_will_topic_length = 127 * 127 * 127 * 127;
            ...
            if (status != NXD_MQTT_CONNECT_FAILURE)
                SET_ERROR_COUNTER(&error_counter, __FILE__, __LINE__);
            /* Post test cleanup... */
            client_ptr -> nxd_mqtt_client_will_topic = "";
            client_ptr -> nxd_mqtt_client_will_topic_length = 0;
        }
