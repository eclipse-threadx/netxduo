...
#if defined(NXD_MQTT_OVER_WEBSOCKET)
...
#else
void       netx_mqtt_websocket_block_test_application_define(void *first_unused_memory)
#endif
{
CHAR       *pointer;
UINT       status;
...
UINT i;
    ...
    printf("NetX Test:   MQTT Websocket Block Test.................................");
    ...
    for (i = 0; i < TEST_LOOP; i++)
    {
        ...
        if (i < 2)
        {
            status = nxd_mqtt_client_connect(client_ptr, &server_address, NXD_MQTT_PORT,
                                             keepalive_value, cleansession_value, NX_WAIT_FOREVER);
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Test:   MQTT Websocket Block Test.................................N/A\n");
