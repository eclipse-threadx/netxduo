...
#else
void       netx_mqtt_client_unsubscribe_application_define(void *first_unused_memory)
#endif
{
CHAR       *pointer;
UINT       status;
...
UINT        i;
    ...
    printf("NetX Test:   MQTT Unsubscribe Test ....................................");
    ...
    for (i = 0; i < TEST_LOOP; i++)
    {
        ...
        /* Issue an unsubscribe command. */
        status = nxd_mqtt_client_unsubscribe(client_ptr, TOPIC1, strlen(TOPIC1));
        if (status)
            SET_ERROR_COUNTER(&error_counter, __FILE__, __LINE__);
    ...
    for (i = 0; i < TEST_LOOP; i++)
    {
        ...
        else
        {
            ...
            *byte++ = QoS & 0x3;
            ...
            /* Validate the MQTT subscribe request. */
            ...
            *byte++ = strlen(TOPIC1) & 0xFF;
            ...
            /* Validate the MQTT unsubscribe request. */
            if (memcmp(packet_ptr->nx_packet_prepend_ptr, content, packet_ptr->nx_packet_length))
                SET_ERROR_COUNTER(&error_counter, __FILE__, __LINE__);
