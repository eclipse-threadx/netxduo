...
#define WILL_TOPIC "topic"
#define WILL_MESSAGE "message"
static UINT will_QoS = 1;
static UINT will_retain = 1;
...
#else
void       netx_mqtt_client_connect_auth_will_application_define(void *first_unused_memory)
#endif
{
CHAR       *pointer;
UINT       status;
...
UINT i;
    ...
    printf("NetX Test:   MQTT Connect Authentication And Will Test.................");
    ...
    for (i = 0; i < TEST_LOOP; i++)
    {
        ...
        status = nxd_mqtt_client_login_set(client_ptr, USERNAME, strlen(USERNAME), PASSWORD, strlen(PASSWORD));
        if (status)
            SET_ERROR_COUNTER(&error_counter, __FILE__, __LINE__);

        /* Set will topic with Will topic with message. */
        status = nxd_mqtt_client_will_message_set(client_ptr, WILL_TOPIC, strlen(WILL_TOPIC), WILL_MESSAGE,
                                                  strlen(WILL_MESSAGE), will_retain, will_QoS);
        if (status)
            SET_ERROR_COUNTER(&error_counter, __FILE__, __LINE__);
...
UINT       i;
    ...
    for (i = 0; i < TEST_LOOP; i++)
    {
        ...
        else
        {
            ...
            if (will_retain)
                content[9] = content[9] | (1 << 5);

            /* Set the QoS value. */
            content[9] = content[9] | (will_QoS << 3);

            /* Set the will flag. */
            content[9] = content[9] | (1 << 2);
            ...
            *buf = (strlen(WILL_TOPIC) >> 8) & 0xFF;
            *(buf + 1) = (strlen(WILL_TOPIC)) & 0xFF;
            memcpy(buf + 2, WILL_TOPIC, strlen(WILL_TOPIC));

            buf += (2 + strlen(WILL_TOPIC));
            *buf = (strlen(WILL_MESSAGE) >> 8) & 0xFF;
            *(buf + 1) = (strlen(WILL_MESSAGE)) & 0xFF;
            memcpy(buf + 2, WILL_MESSAGE, strlen(WILL_MESSAGE));

            buf += (2 + strlen(WILL_MESSAGE));

            *buf = (strlen(USERNAME) >> 8) & 0xFF;
            *(buf + 1) = (strlen(USERNAME)) & 0xFF;
            memcpy(buf + 2, USERNAME, strlen(USERNAME));
            buf = buf + 2 + strlen(USERNAME);
            *buf = (strlen(PASSWORD) >> 8) & 0xFF;
            *(buf + 1) = (strlen(PASSWORD)) & 0xFF;
            memcpy(buf + 2, PASSWORD, strlen(PASSWORD));

            content[1] = (UCHAR)(sizeof(fixed_header) + strlen(CLIENT_ID) + 4 + strlen(USERNAME) + strlen(PASSWORD));
            /* Add will topic and will message. */
            content[1] += strlen(WILL_MESSAGE) + strlen(WILL_TOPIC) + 4;
