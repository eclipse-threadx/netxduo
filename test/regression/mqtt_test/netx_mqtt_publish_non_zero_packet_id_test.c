...
#else
void       netx_mqtt_client_publish_non_zero_packet_id_application_define(void *first_unused_memory)
#endif
{
CHAR       *pointer;
UINT       status;
...
UINT        i;
    ...
    printf("NetX Test:   MQTT Publish Non-Zero Packet ID Test .....................");
    ...
    for (i = 0; i < TEST_LOOP; i++)
    {
        ...
        QoS = 1;
        retain = 0;
        /* Issue a publish command. */
        status = nxd_mqtt_client_publish(client_ptr, TOPIC1, strlen(TOPIC1), MESSAGE1, strlen(MESSAGE1), retain, QoS, 1);
        if (status)
            SET_ERROR_COUNTER(&error_counter, __FILE__, __LINE__);

        /* Set packet id 0xFFFF */
        client_ptr->nxd_mqtt_client_packet_identifier = 0xffff;

        /* Issue a publish command. */
        status = nxd_mqtt_client_publish(client_ptr, TOPIC1, strlen(TOPIC1), MESSAGE2, strlen(MESSAGE2), retain, QoS, 1);
        if (status)
            SET_ERROR_COUNTER(&error_counter, __FILE__, __LINE__);

        /* Issue a publish command. */
        status = nxd_mqtt_client_publish(client_ptr, TOPIC1, strlen(TOPIC1), MESSAGE3, strlen(MESSAGE3), retain, QoS, 1);
        if (status)
            SET_ERROR_COUNTER(&error_counter, __FILE__, __LINE__);
...
//static UCHAR fixed_header[] = {0x80, 0x00,  /* MQTT Control Packet Type, remaining length */
//                               0x00, 0x00}; /* Packet identifier, MSB, and LSB */
    ...
    for (i = 0; i < TEST_LOOP; i++)
    {
        ...
        else
        {
            ...
            *byte++ = strlen(TOPIC1) & 0xFF;
            ...
            if (((content[0] & 6) >> 1) != 0)
            {
                ...
                *byte++ = (packet_id & 0xFF);
                ...
                if (packet_id == 0)
                {
                    printf("1st pubilsh: packet_id is zero.\n");
                    SET_ERROR_COUNTER(&error_counter, __FILE__, __LINE__);
                }
            }
            ...
            *byte++ = strlen(TOPIC1) & 0xFF;
            ...
            /* Fill in the message being pulished. */
            memcpy(byte, MESSAGE2, strlen(MESSAGE2));
            byte += strlen(MESSAGE2);
            ...
            *byte++ = strlen(TOPIC1) & 0xFF;
            ...
            /* Fill in the message being pulished. */
            memcpy(byte, MESSAGE3, strlen(MESSAGE3));
            byte += strlen(MESSAGE3);
