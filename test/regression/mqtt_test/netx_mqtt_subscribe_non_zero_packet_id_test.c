...
#else
void       netx_mqtt_client_subscribe_non_zero_packet_id_application_define(void *first_unused_memory)
#endif
{
CHAR       *pointer;
UINT       status;
...
UINT        i;
    ...
    printf("NetX Test:   MQTT Subscribe Non-Zero Packet ID Test ...................");
    ...
    memset(client_ptr, 0, sizeof(NXD_MQTT_CLIENT));
    ...
    for (i = 0; i < TEST_LOOP; i++)
    {
        ...
        QoS = 1;
        /* Issue the 1st subscribe command. */
        status = nxd_mqtt_client_subscribe(client_ptr, TOPIC1, strlen(TOPIC1), QoS);
        if (status)
            SET_ERROR_COUNTER(&error_counter, __FILE__, __LINE__);

        /* Force packet ID to be 0xFFFF */
        client_ptr->nxd_mqtt_client_packet_identifier = 0xffff;

        /* Issue the 2nd subscribe command. */
        status = nxd_mqtt_client_subscribe(client_ptr, TOPIC1, strlen(TOPIC1), QoS);
        if (status)
            SET_ERROR_COUNTER(&error_counter, __FILE__, __LINE__);

        /* Issue the 3rd subscribe command. */
        status = nxd_mqtt_client_subscribe(client_ptr, TOPIC1, strlen(TOPIC1), QoS);
        if (status)
            SET_ERROR_COUNTER(&error_counter, __FILE__, __LINE__);
...
UINT        i;
    ...
    for (i = 0; i < TEST_LOOP; i++)
    {
        ...
        else
        {
            ...
            /* construct the subscribe message. */
            byte = content;
            *byte++ = 0x82; /* Subscribe */
            byte++; /* Skip the length field. */
            packet_id = (packet_ptr->nx_packet_prepend_ptr[2] << 8) + (packet_ptr->nx_packet_prepend_ptr[3]);

            *byte++ = packet_ptr->nx_packet_prepend_ptr[2];/* Fill in packet ID MSB. */
            *byte++ = packet_ptr->nx_packet_prepend_ptr[3];/* Fill in packet ID LSB. */
            /* Fill in subscribe topic length MSB and LSB */
            *byte++ = (strlen(TOPIC1) >> 8) & 0xFF;
            *byte++ = strlen(TOPIC1) & 0xFF;

            /* Fill in the topic string. */
            memcpy(byte, TOPIC1, strlen(TOPIC1));
            /* Fill in the QoS byte. */
            byte += strlen(TOPIC1);
            *byte++ = QoS & 0x3;
