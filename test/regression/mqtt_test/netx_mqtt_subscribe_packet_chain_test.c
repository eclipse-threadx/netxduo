...
#else
void       netx_mqtt_client_subscribe_packet_chain_application_define(void *first_unused_memory)
#endif
{
CHAR       *pointer;
UINT       status;
...
UINT        i;
    ...
    printf("NetX Test:   MQTT Subscribe Packet Chain Test..........................");
...
NX_PACKET  *temp_packets[NUM_PACKETS];
    ...
    status = nxd_mqtt_client_subscribe(client_ptr, topic, TOPIC_LEN, QoS);
...
UINT        i;
    ...
    else
    {
        ...
        control_header = 0x82; /* Subscribe */
        len = 1 + 2 + 2 + 2 + TOPIC_LEN + 1; /* Control header 1 + remaining length 2 + packet ID 2 + topic length 2 + topic + QoS 1 */

        for (i = 0 ; i < TOPIC_LEN; i++)
        {
            if (packet_buffer[7 + i] != (i & 0xff))
            {
                error_counter++;
                break;
            }
        ...
        if ((packet_buffer[0] != control_header) || (packet_buffer[packet_len -1] != (QoS & 0x3)))
            error_counter++;
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Test:   MQTT Subscribe Packet Chain Test..........................N/A\n");
