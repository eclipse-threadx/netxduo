
/*  Test user-defined key path in processing ClientKeyExchange and sending CertificateVerify messages. */

#include   "tx_api.h"
#include   "nx_api.h"
#include   "nx_tcp.h"
#include   "nx_secure_tls_api.h"
#include   "tls_test_utility.h"
#include   "nx_crypto.h"
#include   "nx_crypto_rsa.h"

#define __LINUX__

extern void    test_control_return(UINT status);

#ifndef NX_SECURE_DISABLE_X509

/* Define the ThreadX and NetX object control blocks...  */
static TX_THREAD               ntest_0;
static NX_PACKET_POOL          pool_0;
static NX_IP                   ip_0;
static NX_TCP_SOCKET           tcp_socket;
static NX_SECURE_TLS_SESSION   tls_session;

static NX_SECURE_X509_CERT certificate;
static NX_SECURE_X509_CERT client_remote_certificate, client_remote_issuer;
static NX_SECURE_X509_CERT trusted_certificate;

static UCHAR client_remote_cert_buffer[2000];
static CHAR crypto_metadata[16000];


static void    ntest_0_entry(ULONG thread_input);

/* Test PKI (3-level). */
#include "test_ca_cert.c"
#define ca_cert_der test_ca_cert_der
#define ca_cert_der_len test_ca_cert_der_len

/*  Cryptographic routines. */
extern const NX_SECURE_TLS_CRYPTO nx_crypto_tls_ciphers;

static unsigned char ica_cert_der[] = {
  
};
static unsigned int ica_cert_der_len = 995;


/* Server certificate. */
#if 0
static unsigned char test_device_cert_der[] = {
  
};
static unsigned int test_device_cert_der_len = 982;

static unsigned char test_device_cert_key_der[] = {
  
};

static unsigned int test_device_cert_key_der_len = 1192;
#endif

static unsigned char test_device_cert_der[] = {
  
};
static unsigned int test_device_cert_der_len = 991;

static unsigned char test_device_cert_key_der[] = {
  
};
static unsigned int test_device_cert_key_der_len = 1191;





/*****************************************************/

extern NX_CRYPTO_METHOD crypto_method_none;
extern NX_CRYPTO_METHOD crypto_method_null;
extern NX_CRYPTO_METHOD crypto_method_aes_cbc_128;
extern NX_CRYPTO_METHOD crypto_method_aes_cbc_256;
extern NX_CRYPTO_METHOD crypto_method_hmac_sha1;
extern NX_CRYPTO_METHOD crypto_method_hmac_sha256;
extern NX_CRYPTO_METHOD crypto_method_hmac_md5;
extern NX_CRYPTO_METHOD crypto_method_rsa;
extern NX_CRYPTO_METHOD crypto_method_auth_psk;
extern NX_CRYPTO_METHOD crypto_method_public_null;
extern NX_CRYPTO_METHOD crypto_method_md5;
extern NX_CRYPTO_METHOD crypto_method_sha1;
extern NX_CRYPTO_METHOD crypto_method_sha256;
extern NX_CRYPTO_METHOD crypto_method_sha384;
extern NX_CRYPTO_METHOD crypto_method_sha512;
extern NX_CRYPTO_METHOD crypto_method_tls_prf_1;
extern NX_CRYPTO_METHOD crypto_method_tls_prf_sha256;
extern NX_CRYPTO_METHOD crypto_method_hkdf;
extern NX_CRYPTO_METHOD crypto_method_hmac;
extern NX_CRYPTO_METHOD crypto_method_ecdhe;



#define USER_KEY_TYPE (0x00011234)

/* Define an overloaded RSA method to test the user-defined key operation and then create a ciphersuite table that uses it. */
static UINT  _nx_crypto_method_rsa_init_user_key(struct NX_CRYPTO_METHOD_STRUCT *method,
                                 UCHAR *key, NX_CRYPTO_KEY_SIZE key_size_in_bits,
                                 VOID **handle,
                                 VOID *crypto_metadata,
                                 ULONG crypto_metadata_size)
{
NX_CRYPTO_RSA *ctx;

    NX_PARAMETER_NOT_USED(method);
    NX_PARAMETER_NOT_USED(key_size_in_bits);
    NX_PARAMETER_NOT_USED(handle);
    NX_PARAMETER_NOT_USED(crypto_metadata_size);
    NX_PARAMETER_NOT_USED(crypto_metadata);

    return(NX_CRYPTO_SUCCESS);
}

static UINT test_failed = 1;

static UINT  _nx_crypto_method_rsa_operation_user_key(UINT op,      /* Encrypt, Decrypt, Authenticate */
                                      VOID *handle, /* Crypto handler */
                                      struct NX_CRYPTO_METHOD_STRUCT *method,
                                      UCHAR *key,
                                      NX_CRYPTO_KEY_SIZE key_size_in_bits,
                                      UCHAR *input,
                                      ULONG input_length_in_byte,
                                      UCHAR *iv_ptr,
                                      UCHAR *output,
                                      ULONG output_length_in_byte,
                                      VOID *crypto_metadata,
                                      ULONG crypto_metadata_size,
                                      VOID *packet_ptr,
                                      VOID (*nx_crypto_hw_process_callback)(VOID *packet_ptr, UINT status))
{
NX_CRYPTO_RSA *ctx;

    NX_PARAMETER_NOT_USED(handle);
    NX_PARAMETER_NOT_USED(method);
    NX_PARAMETER_NOT_USED(iv_ptr);
    NX_PARAMETER_NOT_USED(output_length_in_byte);
    NX_PARAMETER_NOT_USED(crypto_metadata_size);
    NX_PARAMETER_NOT_USED(packet_ptr);
    NX_PARAMETER_NOT_USED(nx_crypto_hw_process_callback);

    if ((op != USER_KEY_TYPE) || (memcmp(key, test_device_cert_key_der, key_size_in_bits) != 0))
    {
        printf("input length: %d, expected: %d\n", key_size_in_bits, test_device_cert_key_der_len);
        printf("User-defined key failure!\n");
        test_control_return(1);
    }

    test_failed = 0;

    return(NX_CRYPTO_SUCCESS);
}

/*  Crypto method structure for our custom RSA operation. */
NX_CRYPTO_METHOD crypto_method_rsa_user_key =
{
    TLS_CIPHER_RSA,                           /* RSA crypto algorithm                  */
    0,                                        /* Key size in bits                      */
    0,                                        /* IV size in bits                       */
    0,                                        /* ICV size in bits, not used.           */
    0,                                        /* Block size in bytes.                  */
    sizeof(NX_CRYPTO_RSA),                    /* Metadata size in bytes                */
    _nx_crypto_method_rsa_init_user_key,      /* RSA initialization routine.           */
    NX_NULL,                                  /* RSA cleanup routine, not used.        */
    _nx_crypto_method_rsa_operation_user_key  /* RSA operation                         */
};


/* Lookup table used to map ciphersuites to cryptographic routines. */
NX_SECURE_TLS_CIPHERSUITE_INFO _nx_crypto_ciphersuite_lookup_table_user_key[] =
{
    /* Ciphersuite,                     public cipher,       public_auth,             session cipher & cipher mode,  iv size, key size,  hash method,              hash size, TLS PRF */
    {TLS_NULL_WITH_NULL_NULL,           &crypto_method_null, &crypto_method_null,     &crypto_method_null,           0,       0,         &crypto_method_null,      0,         &crypto_method_tls_prf_sha256},
//    {TLS_RSA_WITH_AES_128_CBC_SHA,      &crypto_method_rsa_user_key,  &crypto_method_rsa_user_key,      &crypto_method_aes_cbc_128,    16,      16,        &crypto_method_hmac_sha1, 20,        &crypto_method_tls_prf_sha256},
    {TLS_RSA_WITH_AES_128_CBC_SHA256,   &crypto_method_rsa_user_key,  &crypto_method_rsa_user_key,      &crypto_method_aes_cbc_128,    16,      16,        &crypto_method_hmac_sha256,     32,        &crypto_method_tls_prf_sha256},
};

const UINT _nx_crypto_ciphersuite_lookup_table_user_key_size = sizeof(_nx_crypto_ciphersuite_lookup_table_user_key) / sizeof(NX_SECURE_TLS_CIPHERSUITE_INFO);

/* Lookup table for X.509 digital certificates - they need a public-key algorithm and a hash routine for verification. */
NX_SECURE_X509_CRYPTO _nx_crypto_x509_cipher_lookup_table_user_key[] =
{
    /* OID identifier,                        public cipher,            hash method */
    {NX_SECURE_TLS_X509_TYPE_RSA_MD5,        &crypto_method_rsa_user_key,       &crypto_method_md5},
    {NX_SECURE_TLS_X509_TYPE_RSA_SHA_1,      &crypto_method_rsa_user_key,       &crypto_method_sha1},
    {NX_SECURE_TLS_X509_TYPE_RSA_SHA_256,    &crypto_method_rsa_user_key,       &crypto_method_sha256},
    {NX_SECURE_TLS_X509_TYPE_RSA_SHA_384,    &crypto_method_rsa_user_key,       &crypto_method_sha384},
    {NX_SECURE_TLS_X509_TYPE_RSA_SHA_512,    &crypto_method_rsa_user_key,       &crypto_method_sha512},
};

/* Define the object we can pass into TLS. */
static const NX_SECURE_TLS_CRYPTO nx_crypto_tls_user_key_ciphers =
{
    /* Ciphersuite lookup table and size. */
    _nx_crypto_ciphersuite_lookup_table_user_key,
    sizeof(_nx_crypto_ciphersuite_lookup_table_user_key) / sizeof(NX_SECURE_TLS_CIPHERSUITE_INFO),

#ifndef NX_SECURE_DISABLE_X509
    /* X.509 certificate cipher table and size. */
    _nx_crypto_x509_cipher_lookup_table_user_key,
    sizeof(_nx_crypto_x509_cipher_lookup_table_user_key) / sizeof(NX_SECURE_X509_CRYPTO),
#endif

    /* TLS version-specific methods. */
#if (NX_SECURE_TLS_TLS_1_0_ENABLED || NX_SECURE_TLS_TLS_1_1_ENABLED)
    &crypto_method_md5,
    &crypto_method_sha1,
    &crypto_method_tls_prf_1,
#endif

#if (NX_SECURE_TLS_TLS_1_2_ENABLED)
    &crypto_method_sha256,
    &crypto_method_tls_prf_sha256,
#endif

#if (NX_SECURE_TLS_TLS_1_3_ENABLED)
    &crypto_method_hkdf,
    &crypto_method_hmac,
    &crypto_method_ecdhe,
#endif
};
/* ************************************************** */



#define     DEMO_STACK_SIZE  4096 //  (3 * 1024 / sizeof(ULONG))

/* Define the IP thread's stack area.  */
#define IP_STACK_SIZE 4096 //(2 * 1024 / sizeof(ULONG))

/* Define packet pool for the demonstration.  */
#define NX_PACKET_POOL_BYTES  ((1536 + sizeof(NX_PACKET)) * 20)
#define NX_PACKET_POOL_SIZE (NX_PACKET_POOL_BYTES/sizeof(ULONG) + 64 / sizeof(ULONG))

/* Define the ARP cache area.  */
#define ARP_AREA_SIZE 1024 // (512 / sizeof(ULONG))

#define TOTAL_STACK_SPACE (2 * (DEMO_STACK_SIZE + IP_STACK_SIZE + NX_PACKET_POOL_SIZE + ARP_AREA_SIZE))

#ifndef __LINUX__
ULONG test_stack_area[TOTAL_STACK_SPACE + 2000];
#endif

/* Define the counters used in the demo application...  */
ULONG                   error_counter;


/* Define thread prototypes.  */


extern void    _nx_ram_network_driver_1500(struct NX_IP_DRIVER_STRUCT *driver_req);

/* Captured ClientKeyExchange message for use in tests below. */
char client_key_exchange_packet_bytes[] = {
  
};

UINT client_key_exchange_packet_bytes_size = sizeof(client_key_exchange_packet_bytes);

/* Define what the initial system looks like.  */
#ifdef CTEST
void test_application_define(void *first_unused_memory);
void test_application_define(void *first_unused_memory)
#else
void           nx_secure_tls_user_defined_key_test_application_define(void *first_unused_memory)
#endif
{
UINT       status;
CHAR       *pointer;
ULONG      actual_status;


    /* Print out test information banner.  */
    printf("NetX Secure Test:   TLS User-defined Key Type Test.....................");

#ifndef NX_SECURE_TLS_SERVER_DISABLED

    error_counter = 0;

    pointer = (CHAR *) first_unused_memory;

    /* Create the main thread.  */
    tx_thread_create(&ntest_0, "thread 0", ntest_0_entry, 0,
                     pointer, DEMO_STACK_SIZE,
                     3, 3, TX_NO_TIME_SLICE, TX_AUTO_START);


    /* Initialize the NetX system.  */
    nx_system_initialize();
      
    /* Create a packet pool.  */
    status = nx_packet_pool_create(&pool_0, "NetX Main Packet Pool", 1536, pointer, NX_PACKET_POOL_BYTES);
    pointer = pointer + NX_PACKET_POOL_BYTES;

    if(status)
    {
        printf("Error in packet pool create: 0x%x\n", status);
        error_counter++;
    }
    /* Create an IP instance.  */
    status = nx_ip_create(&ip_0, "NetX IP Instance 0", IP_ADDRESS(1, 2, 3, 4), 0xFFFFFF00UL, &pool_0, _nx_ram_network_driver_1500,
                          pointer, IP_STACK_SIZE, 1);
    pointer = pointer + IP_STACK_SIZE;

    if(status)
    {
        printf("Error in IP create: 0x%x\n", status);
        error_counter++;
    }

    /* Enable ARP and supply ARP cache memory for IP Instance 0.  */
    status = nx_arp_enable(&ip_0, (void *) pointer, ARP_AREA_SIZE);
    pointer = pointer + ARP_AREA_SIZE;

    /* Check ARP enable status.  */
    if(status)
    {
        printf("Error in ARP enable: 0x%x\n", status);
        error_counter++;
    }

    /* Enable TCP processing for both IP instances.  */
    status = nx_tcp_enable(&ip_0);

    /* Check TCP enable status.  */
    if(status)
    {
        printf("Error in TCP enable: 0x%x\n", status);
        error_counter++;
    }

    nx_secure_tls_initialize();
#else
    printf("N/A\n");
    test_control_return(3);
#endif
}


static void    ntest_0_entry(ULONG thread_input)
{
UINT       status;
ULONG      actual_status;
NX_PACKET *send_packet;
NX_PACKET *receive_packet;
UCHAR receive_buffer[100];
ULONG bytes;

    /* Create a socket.  */
    status = nx_tcp_socket_create(&ip_0, &tcp_socket, "Server Socket", 
                                  NX_IP_NORMAL, NX_FRAGMENT_OKAY, NX_IP_TIME_TO_LIVE * 100, 16*1024,
                                  NX_NULL, NX_NULL);

    /* Check for earlier error.  */
    if(error_counter)
    {
        printf("ERROR!\n");
        test_control_return(1);
    }


    /* Create a TLS session for our socket.  */
    status =  nx_secure_tls_session_create(&tls_session,
                                           &nx_crypto_tls_user_key_ciphers,
                                           crypto_metadata,
                                           sizeof(crypto_metadata));

    /* Check for error.  */
    if(status)
        error_counter++;


    /////////////////////////////////////////////////////////////////////////////////////////////////////////
    // Initialize our certificate
    nx_secure_x509_certificate_initialize(&certificate, test_device_cert_der, test_device_cert_der_len, NX_NULL, 0, test_device_cert_key_der, test_device_cert_key_der_len, USER_KEY_TYPE);
    nx_secure_tls_local_certificate_add(&tls_session, &certificate);

    // If we are testing client certificate verify, allocate remote certificate space.
    nx_secure_tls_remote_certificate_allocate(&tls_session, &client_remote_certificate, client_remote_cert_buffer, sizeof(client_remote_cert_buffer));
    nx_secure_tls_remote_certificate_allocate(&tls_session, &client_remote_issuer, client_remote_cert_buffer, sizeof(client_remote_cert_buffer));

    /* Add a CA Certificate to our trusted store for verifying incoming client certificates. */
    nx_secure_x509_certificate_initialize(&trusted_certificate, ica_cert_der, ica_cert_der_len, NX_NULL, 0, NULL, 0, NX_SECURE_X509_KEY_TYPE_NONE);
    nx_secure_tls_trusted_certificate_add(&tls_session, &trusted_certificate);

    /////////////////////////////////////////////////////////////////////////////////////////////////////////


    /*  Bypassing the TLS handshake, we have to set state such that the following calls work as expected. */
    /* protocol must be set for determing signature length while sending certificate. */
    tls_session.nx_secure_tls_protocol_version = NX_SECURE_TLS_VERSION_TLS_1_2;
    tls_session.nx_secure_tls_session_ciphersuite = &_nx_crypto_ciphersuite_lookup_table_user_key[1];
    tcp_socket.nx_tcp_socket_state = NX_TCP_ESTABLISHED;
    tcp_socket.nx_tcp_socket_connect_ip.nxd_ip_version =  NX_IP_VERSION_V4;
    /* Initialize ip interface to avoid ip thread accessing invalid interface. */
    tcp_socket.nx_tcp_socket_connect_interface = &ip_0.nx_ip_interface[0];
    tls_session.nx_secure_tls_tcp_socket = &tcp_socket;

    /* Test ClientKeyExchange processing. */
    _nx_secure_tls_process_client_key_exchange(&tls_session, client_key_exchange_packet_bytes, client_key_exchange_packet_bytes_size, NX_SECURE_TLS);

    if (test_failed)
    {
        printf("ERROR!\n");
        test_control_return(1);
    }

    printf("SUCCESS!\n");
    test_control_return(0);
}

#else
#ifdef CTEST
void test_application_define(void *first_unused_memory);
void test_application_define(void *first_unused_memory)
#else
void nx_secure_tls_user_defined_key_test_application_define(void *first_unused_memory)
#endif
{

    /* Print out test information banner.  */
    printf("NetX Secure Test:   TLS User-defined Key Type Test.....................N/A\n");
    test_control_return(3);
}
#endif