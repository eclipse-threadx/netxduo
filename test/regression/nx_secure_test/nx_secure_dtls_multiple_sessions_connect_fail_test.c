...
void test_application_define(void *first_unused_memory)
#else
VOID    nx_secure_dtls_multiple_sessions_connect_fail_test_application_define(void *first_unused_memory)
#endif
{
UINT     status;
CHAR    *pointer;
...
NX_PACKET *packet_ptr;
    ...
    printf("NetX Secure Test:   DTLS Multiple Sessions Connect Fail Test...........");
    ...
    for (i = 0; i < NUM_CLIENT_TEST; i++)
    {
        ...
        status = nx_secure_dtls_server_session_start(connect_session[i], NX_IP_PERIODIC_RATE);

        if (i != 1 && status)
        {
            printf("Clinet %d: Error in establishing DTLS server session: 0x%02X\n", i, status);
            ERROR_COUNTER();
        }
        else if (i == 1)
        {
            ...
            if (status)
            {
                ...
                /* Resume client 2 to connect to server.  */
    ...
    while (server_receive_count < NUM_SERVER_SESSIONS)
    {
        tx_thread_sleep(1);
    }
    ...
    for(i = 0; i < NUM_SERVER_SESSIONS; ++i)
    {
        nx_secure_dtls_session_end(&dtls_server.nx_dtls_server_sessions[i], NX_NO_WAIT);
    }
...
CHAR socket_name[] = "Client socket X";
    ...
    client_ip[client_index].nx_ip_udp_packet_receive = udp_packet_filter_bad_helloverify;
...
CHAR socket_name[] = "Client socket X";
    ...
    for (i = 0; i < sizeof(request_buffer[client_index]); i++)
    {
        request_buffer[client_index][i] = i << 1;
    }
    ...
    tx_thread_suspend(&client_thread[2]);
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Secure Test:   DTLS Multiple Sessions Connect Fail Test...........N/A\n");
