/* This test concentrates on DTLS ciphersuite TLS_ECDHE_ECDSA_WITH_AES_128_CBC_SHA. The client certificate is require. */
...
#if !defined(NX_SECURE_TLS_CLIENT_DISABLED) && !defined(NX_SECURE_TLS_SERVER_DISABLED) && defined(NX_SECURE_ENABLE_ECC_CIPHERSUITE) && defined(NX_SECURE_ENABLE_DTLS) && defined(NX_SECURE_ENABLE_CLIENT_CERTIFICATE_VERIFY)
...
static NX_SECURE_X509_CERT      server_trusted_ca2;
static NX_SECURE_X509_CERT      server_trusted_ca3;
static NX_SECURE_X509_CERT      server_local_certificate;
static NX_SECURE_X509_CERT      server_local_certificate2;
static NX_SECURE_X509_CERT      server_local_certificate3;
...
static UCHAR                    server_cert_buffer[CERT_BUFFER_SIZE * 2];
...
void test_application_define(void *first_unused_memory)
#else
VOID    nx_secure_dtls_ecc_client_cert_test_application_define(void *first_unused_memory)
#endif
{
UINT     status;
CHAR    *pointer;
...
UINT status;
    ...
    status = nx_secure_x509_certificate_initialize(&client_local_certificate,
                                                   ECTestServer7_256_der, ECTestServer7_256_der_len,
    ...
    status = nx_secure_dtls_session_local_certificate_add(dtls_session_ptr,
                                                          &client_local_certificate, 1);
    if (status)
    {
        ERROR_COUNTER();
    }
...
UINT status;
    ...
    status = nx_secure_dtls_server_x509_client_verify_configure(dtls_server_ptr, 2,
                                                                server_cert_buffer,
    ...
    status = nx_secure_x509_certificate_initialize(&server_trusted_ca, ECCA4_der, ECCA4_der_len,
                                                   NX_NULL, 0, NULL, 0,
                                                   NX_SECURE_X509_KEY_TYPE_NONE);
    if (status)
    {
    ...
    status = nx_secure_dtls_server_trusted_certificate_add(dtls_server_ptr, &server_trusted_ca, 2);
    if (status)
    {
        ERROR_COUNTER();
    }

    status = nx_secure_x509_certificate_initialize(&server_trusted_ca2, ECCA2_der, ECCA2_der_len,
                                                   NX_NULL, 0, NULL, 0,
                                                   NX_SECURE_X509_KEY_TYPE_NONE);
    if (status)
    {
    ...
    status = nx_secure_dtls_server_trusted_certificate_add(dtls_server_ptr, &server_trusted_ca2, 3);
    if (status)
    {
        ERROR_COUNTER();
    }

    status = nx_secure_x509_certificate_initialize(&server_trusted_ca3, ECCA3_der, ECCA3_der_len,
                                                   NX_NULL, 0, NULL, 0,
                                                   NX_SECURE_X509_KEY_TYPE_NONE);
    if (status)
    {
    ...
    status = nx_secure_dtls_server_trusted_certificate_add(dtls_server_ptr, &server_trusted_ca3, 4);
    if (status)
    {
        ERROR_COUNTER();
    }

    memset(&server_local_certificate2, 0, sizeof(server_local_certificate2));
    status = nx_secure_x509_certificate_initialize(&server_local_certificate2,
                                                   ECTestServer3_der, ECTestServer3_der_len,
                                                   NX_NULL, 0, ECTestServer3_key_der,
                                                   ECTestServer3_key_der_len,
                                                   NX_SECURE_X509_KEY_TYPE_EC_DER);
    if (status)
    {
        ERROR_COUNTER();
    ...
    status = nx_secure_dtls_server_local_certificate_add(dtls_server_ptr, &server_local_certificate2, 5);
    if (status)
    {
        ERROR_COUNTER();
    }

    memset(&server_local_certificate3, 0, sizeof(server_local_certificate3));
    status = nx_secure_x509_certificate_initialize(&server_local_certificate3,
    ...
    status = nx_secure_dtls_server_local_certificate_add(dtls_server_ptr, &server_local_certificate3, 6);
    if (status)
    {
        ERROR_COUNTER();
    }

    status = nx_secure_dtls_server_trusted_certificate_remove(dtls_server_ptr, (UCHAR *)"ECCA8", 5, 8);
    if (!status)
    {
        ERROR_COUNTER();
    }

    status = nx_secure_dtls_server_trusted_certificate_remove(dtls_server_ptr, (UCHAR *)"ECCA2", 5, 3);
    if (status)
    {
        ERROR_COUNTER();
    }

    status = nx_secure_dtls_server_trusted_certificate_remove(dtls_server_ptr, (UCHAR *)"ECCA3", 5, 0);
    if (status)
    {
        ERROR_COUNTER();
    }

    status = nx_secure_dtls_server_local_certificate_remove(dtls_server_ptr, (UCHAR *)"ECCA8", 5, 8);
    if (!status)
    {
        ERROR_COUNTER();
    }

    status = nx_secure_dtls_server_local_certificate_remove(dtls_server_ptr, (UCHAR *)"ECTestServer3", 13, 5);
    if (status)
    {
        ERROR_COUNTER();
    }

    status = nx_secure_dtls_server_local_certificate_remove(dtls_server_ptr, (UCHAR *)"ECTestServer4", 13, 0);
    if (status)
    {
        ERROR_COUNTER();
    }
...
NX_PACKET *packet_ptr;
    ...
    printf("NetX Secure Test:   DTLS ECC Client Cert Test..........................");
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Secure Test:   DTLS ECC Client Cert Test..........................N/A\n");
