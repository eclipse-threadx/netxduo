...
#if !defined(NX_SECURE_TLS_CLIENT_DISABLED) && !defined(NX_SECURE_TLS_SERVER_DISABLED) && defined(NX_SECURE_ENABLE_AEAD_CIPHER) && defined(NX_SECURE_ENABLE_ECC_CIPHERSUITE) && !defined(NX_SECURE_DISABLE_X509)
#define __LINUX__
...
#define NX_PACKET_POOL_BYTES  ((1536 + sizeof(NX_PACKET)) * 24)
...
static UCHAR server_packet_buffer[20000];
static UCHAR client_packet_buffer[20000];
...
static void test_receive_tcp_server();
static void test_send_bad_encrypted_client();

static void test_record_overflow_server();
static void test_record_overflow_client();


static TLS_HANDSHAKE_TEST_DATA test_data[] =
{
    { test_receive_tcp_server, test_send_bad_encrypted_client },
    { test_record_overflow_server, test_record_overflow_client },

};

static UINT ciphersuite_list[] = { TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256 };
static CIPHERSUITE ciphersuites_client = { ciphersuite_list, 1, NX_FALSE, NX_NULL};
...
static void    sync_thread_entry(ULONG thread_input)
{
    ...
    printf("NetX Secure Test:   TLS Receive Test...................................");
...
UINT status;
    ...
    nx_secure_tls_session_protocol_version_override(&server_tls_session, NX_SECURE_TLS_VERSION_TLS_1_2);
    ...
    nx_secure_tls_server_certificate_add(&server_tls_session, &server_certificate, 1);
...
UINT status;
    ...
    ciphersuites_setup(&ciphersuites_client, &tls_ciphers_client, ciphersuite_table_client);
    ...
    nx_secure_tls_session_protocol_version_override(&client_tls_session, NX_SECURE_TLS_VERSION_TLS_1_2);

    status = nx_secure_tls_ecc_initialize(&client_tls_session, nx_crypto_ecc_supported_groups,
                                          nx_crypto_ecc_supported_groups_size,
                                          nx_crypto_ecc_curves);
    if (status)
    {
...
static UCHAR test_send_data[] =
{
    //0x15, 0x03, 0x03, 0x00, 0x02, 0x02, 0x2e,
...
NX_PACKET *send_packet;
    ...
    status = nx_packet_data_append(send_packet, test_send_data, sizeof(test_send_data), &pool_1, NX_WAIT_FOREVER);
    EXPECT_EQ(NX_SUCCESS, status);
...
/* Test of a server receiving an encrypted record that is too long. */
static void test_record_overflow_server()
{
UINT status;
NX_PACKET *receive_packet;
...
static UCHAR test_send_record_overflow[18439] =
{
    0x17, 0x03, 0x03, 0x48, 0x02,
};
...
UINT alert_value = 0;
    ...
    status = nx_packet_data_append(send_packet, test_send_record_overflow, sizeof(test_send_record_overflow), &pool_1, NX_WAIT_FOREVER);
    EXPECT_EQ(NX_SUCCESS, status);
    ...
    status = nx_secure_tls_session_alert_value_get(&client_tls_session, &alert_level, &alert_value);

    if (alert_level != NX_SECURE_TLS_ALERT_LEVEL_FATAL && alert_value != NX_SECURE_TLS_ALERT_RECORD_OVERFLOW)
    {
        printf("No record overflow alert received.\n");
        error_counter++;
    }
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Secure Test:   TLS Receive Test...................................N/A\n");
