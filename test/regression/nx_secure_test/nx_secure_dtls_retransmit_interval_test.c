...
#define TOLERANT                    NX_IP_PERIODIC_RATE
...
void test_application_define(void *first_unused_memory)
#else
VOID    nx_secure_dtls_retransmit_interval_test_application_define(void *first_unused_memory)
#endif
{
UINT     status;
CHAR    *pointer;
...
ULONG interval = current_time - previous_time;
ULONG expected_interval = NX_SECURE_DTLS_RETRANSMIT_TIMEOUT << (NX_SECURE_DTLS_RETRANSMIT_RETRY_SHIFT * (retries - 1));

    if (expected_interval > NX_SECURE_DTLS_MAXIMUM_RETRANSMIT_TIMEOUT)
    {
        expected_interval = NX_SECURE_DTLS_MAXIMUM_RETRANSMIT_TIMEOUT;
    }
    previous_time = current_time;

    if (expected_interval > interval)
    {
        if ((expected_interval - interval) > TOLERANT)
        {
            ERROR_COUNTER();
        }
    }
    else if ((interval - expected_interval ) > TOLERANT)
    {
        ERROR_COUNTER();
    }
}
...
NX_PACKET *packet_ptr;
    ...
    printf("NetX Secure Test:   DTLS Retransmit Interval Test......................");
    ...
    tx_semaphore_get(&semaphore_receive, NX_WAIT_FOREVER);
...
NXD_ADDRESS server_address;
    ...
    /* Check interval and retries. */
    check_interval();
    if ((retries - 1) != NX_SECURE_DTLS_MAXIMUM_RETRANSMIT_RETRIES)
    {
        ERROR_COUNTER();
    }
    ...
    /* Restart DTLS session. */
    _nx_secure_dtls_session_reset(&dtls_client_session_0);
    status = nx_secure_dtls_client_session_start(&dtls_client_session_0, &client_socket_0, &server_address, SERVER_PORT, NX_WAIT_FOREVER);
    if (status)
    {
        ERROR_COUNTER();
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Secure Test:   DTLS Retransmit Interval Test......................N/A\n");
