...
0x00,
0x00, 0x3d + 4 + 4, /* extensions length with two empty extensions. */
0x00, 0x0a, /* ec groups */
0x00, 0x08,
...
/* empty sec_reneg extension with a padding 0x01. */
static UCHAR corrutped_sec_reneg_extension[] = {0xff, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
static UCHAR corrutped_keyshare_extension[] = {0x00, 0x33, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00};
static UCHAR corrutped_psk_extension[] = {0x00, 0x29, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
typedef struct
{
...
static TEST_POINT test_array[] =
{
    ...
    {NX_SECURE_TLS_INCORRECT_MESSAGE_LENGTH, corrutped_sec_reneg_extension, sizeof(corrutped_sec_reneg_extension)}, /* total extensions length field */
#endif
#if (NX_SECURE_TLS_TLS_1_3_ENABLED)
    {NX_SECURE_TLS_INCORRECT_MESSAGE_LENGTH, corrutped_keyshare_extension, sizeof(corrutped_keyshare_extension)}, /* key shares extension length field */
...
NX_PACKET *packet_ptr;
    ...
    printf("NetX Secure Test:   TLS Empty ClientHello Extension Test...............");
...
USHORT total_extension_length, message_length;
    ...
    for (j = 0; j < sizeof(test_array)/sizeof(TEST_POINT); j++)
    {
        ...
        NX_SECURE_MEMCPY(packet_buffer + sizeof(client_hello_empty_key_share), test_array[j].value, test_array[j].size);

        /* Send the Clienthello with padding empty extension. */
        status += nx_packet_data_append(send_packet, packet_buffer, sizeof(client_hello_empty_key_share) + test_array[j].size, tls_client_session_0.nx_secure_tls_packet_pool, NX_NO_WAIT);
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Secure Test:   TLS Empty ClientHello Extension Test...............N/A\n");
