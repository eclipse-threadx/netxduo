...
static NX_CRYPTO_METHOD fake_ecdh_method;
static NX_CRYPTO_METHOD fake_x509_ecdh_method;
static NX_CRYPTO_METHOD fake_x509_sha256_method;


static void NX_Secure_TLS_generate_premaster_test();
...
void test_application_define(void *first_unused_memory)
#else
void nx_secure_tls_generate_premaster_coverage_test_application_define(void *first_unused_memory)
#endif
{
    ...
    NX_Secure_TLS_generate_premaster_test();
...
static NX_SECURE_X509_CRYPTO _nx_crypto_x509_cipher_lookup_table_ecc_test[] =
{
    ...
    {NX_SECURE_TLS_X509_TYPE_ECDSA_SHA_256,  &fake_x509_ecdh_method,   &fake_x509_sha256_method}
};
...
TEST(NX_Secure_TLS, generate_premaster_test)
{
UINT status;
UINT public_key_size = 2;
    ...
    status = _nx_secure_tls_generate_premaster_secret(&session, 0);
    ...
    fake_tls_session_ciphersuite.nx_secure_tls_public_cipher = &fake_ecdh_method;
    /* Cover line 115 and 118 */
    fake_ecdh_method.nx_crypto_algorithm = NX_CRYPTO_KEY_EXCHANGE_ECDH;
    status = _nx_secure_tls_generate_premaster_secret(&session, 0);
    ...
    _nx_secure_x509_store_certificate_add(&server_certificate, &session.nx_secure_tls_credentials.nx_secure_tls_certificate_store,
    ...
    /* Install proper curves. */
    status = nx_secure_tls_ecc_initialize(&session, nx_crypto_ecc_supported_groups,
                                          nx_crypto_ecc_supported_groups_size,
                                          fake_ecc_curves);
    /* Cover line 140 */
    fake_ecdh_method.nx_crypto_operation = NX_NULL;
    status = _nx_secure_tls_generate_premaster_secret(&session, 0);
    EXPECT_EQ(NX_SECURE_TLS_MISSING_CRYPTO_ROUTINE, status);

    /* Test line 165 */
    fake_ecdh_method.nx_crypto_operation = fake_operation;
    fake_ecdh_method.nx_crypto_init = NX_NULL;
    status = _nx_secure_tls_generate_premaster_secret(&session, 0);
    ...
    fake_ecdh_method.nx_crypto_init = fake_init;
    ...
    /* Test line 207 */
    fake_ecdh_method.nx_crypto_cleanup = NX_NULL;
    status = _nx_secure_tls_generate_premaster_secret(&session, 0);
    EXPECT_EQ(NX_SECURE_TLS_SUCCESS, status);

    /* Test line 209 */
    fake_ecdh_method.nx_crypto_cleanup = fake_cleanup;
    status = _nx_secure_tls_generate_premaster_secret(&session, 0);
    EXPECT_EQ(NX_CRYPTO_NOT_SUCCESSFUL, status);    
