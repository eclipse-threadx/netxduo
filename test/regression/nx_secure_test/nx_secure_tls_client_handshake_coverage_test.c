...
/* This test assembles a TLS session by hand rather than through nx_secure_tls_session_create, so it
   also has to supply the CertificateVerify scratch area that create would carve out of the crypto
   metadata. */
static UCHAR certificate_verify_scratch[NX_SECURE_TLS_CERTIFICATE_VERIFY_SCRATCH_SIZE];
#endif
...
#ifndef NX_SECURE_DISABLE_X509
    session.nx_secure_tls_certificate_verify_scratch = certificate_verify_scratch;
    session.nx_secure_tls_certificate_verify_scratch_size = sizeof(certificate_verify_scratch);
#endif
