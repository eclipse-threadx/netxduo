...
static CIPHERSUITE ciphersuites_client[] =
{
    ...
    /* Client sends TLS 1.3 ciphersuites to negotiate TLS 1.2. */
    CIPHERSUITE_INIT(ciphersuite_list_1, NX_FALSE, &test_certs[0]),

    /* Client sends TLS 1.2 ciphersuites to negotiate TLS 1.3. */
    CIPHERSUITE_INIT(ciphersuite_list_2, NX_FALSE, &test_certs[0]),
};
...
static CIPHERSUITE ciphersuites_server[] =
{
    ...
    /* Client sends TLS 1.3 ciphersuites to negotiate TLS 1.2. */
    CIPHERSUITE_INIT(ciphersuite_list_0, NX_FALSE, &test_certs[0]),

    /* Client sends TLS 1.2 ciphersuites to negotiate TLS 1.3. */
    CIPHERSUITE_INIT(ciphersuite_list_0, NX_FALSE, &test_certs[0]),
};

static UCHAR tls_1_2_client_hello_tls_1_3_ciphersuite[] = {
0x16, 0x03, 0x03, 0x00, 0x64, 0x01, 0x00, 0x00, 0x60, 0x03,
0x03, 0x00, 0x00, 0x00, 0x00, 0x84, 0x67, 0x00, 0x00, 0xe1, 0x4a, 0x00, 0x00, 0x6c, 0x3d, 0x00,
0x00, 0xd6, 0x2c, 0x00, 0x00, 0xae, 0x72, 0x00, 0x00, 0x52, 0x69, 0x00, 0x00, 0x90, 0x5f, 0x00,
0x00, 0x00, 0x00, 0x02, 0x13, 0x01, 0x01, 0x00, 0x00, 0x35, 0x00, 0x0a, 0x00, 0x0c, 0x00, 0x0a,
0x00, 0x17, 0x00, 0x18, 0x00, 0x19, 0x00, 0x15, 0x00, 0x13, 0x00, 0x0b, 0x00, 0x02, 0x01, 0x00,
0xff, 0x01, 0x00, 0x01, 0x00, 0x00, 0x0d, 0x00, 0x16, 0x00, 0x14, 0x01, 0x01, 0x02, 0x01, 0x04,
0x01, 0x05, 0x01, 0x06, 0x01, 0x02, 0x03, 0x03, 0x03, 0x04, 0x03, 0x05, 0x03, 0x06, 0x03,
};
...
void test_application_define(void *first_unused_memory)
#else
VOID    nx_secure_tls_1_3_ciphersuites_test_application_define(void *first_unused_memory)
#endif
{
UINT     status;
CHAR    *pointer;
...
NX_PACKET *packet_ptr;
    ...
    printf("NetX Secure Test:   TLS 1.3 Ciphersuites Test..........................");
...
NXD_ADDRESS server_address;
    ...
    for (i = 0; i < sizeof(ciphersuites_client) / sizeof(CIPHERSUITE); i++)
    {
        ...
        if (i == 0)
        {
            nx_secure_tls_session_protocol_version_override(&tls_client_session_0, NX_SECURE_TLS_VERSION_TLS_1_2);
            ...
            status = nx_packet_data_append(packet_ptr, tls_1_2_client_hello_tls_1_3_ciphersuite, sizeof(tls_1_2_client_hello_tls_1_3_ciphersuite),
                                           &pool_0, NX_NO_WAIT);
            if (status)
            {
                ERROR_COUNTER();
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Secure Test:   TLS 1.3 Ciphersuites Test..........................N/A\n");
