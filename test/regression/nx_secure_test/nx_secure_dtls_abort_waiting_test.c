...
void test_application_define(void *first_unused_memory)
#else
VOID    nx_secure_dtls_abort_waiting_test_application_define(void *first_unused_memory)
#endif
{
UINT     status;
CHAR    *pointer;
...
UINT status;
    ...
    printf("NetX Secure Test:   DTLS Abort Waiting Test............................");
    ...
    /* The packet waiting will be aborted.  */
...
static void server_concurrent_thread_entry(ULONG thread_input)
{
    ...
    /* If server session is waiting for packet.  */
    while (!connect_session[1] -> nx_secure_dtls_thread_suspended)
    {
        tx_thread_sleep(NX_IP_PERIODIC_RATE);
    }

    /* End session to abort the waiting.  */
    nx_secure_dtls_session_end(connect_session[1], NX_NO_WAIT);
}
...
UCHAR *data;
    ...
    /* Drop the Server Hello Verify Request. */
...
NXD_ADDRESS server_address;
    ...
    /* Start DTLS session. */
    status = nx_secure_dtls_client_session_start(&dtls_client_session[1], &client_socket[1], &server_address, SERVER_PORT, NX_IP_PERIODIC_RATE);
    if (!status)
    {
        ERROR_COUNTER();
    }
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Secure Test:   DTLS Abort Waiting Test............................N/A\n");
