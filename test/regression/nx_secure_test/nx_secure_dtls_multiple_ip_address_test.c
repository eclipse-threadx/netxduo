...
#if !defined(NX_SECURE_TLS_CLIENT_DISABLED) && !defined(NX_SECURE_TLS_SERVER_DISABLED) && defined(NX_SECURE_ENABLE_DTLS) && defined(FEATURE_NX_IPV6)
...
static NXD_ADDRESS             client_address_0;
static NXD_ADDRESS             client_address_1;
...
static UCHAR                    response_buffer[2][BUFFER_SIZE];
...
void test_application_define(void *first_unused_memory)
#else
VOID    nx_secure_dtls_multiple_ip_address_test_application_define(void *first_unused_memory)
#endif
{
UINT     status;
CHAR    *pointer;
    ...
    status = nxd_ipv6_address_set(&server_ip, 0, &address_0, 64, NX_NULL);
    if (status)
    {
        ERROR_COUNTER();
    }
    ...
    address_1.nxd_ip_address.v6[3] = 0x00000003;

    status = nxd_ipv6_address_set(&server_ip, 0, &address_1, 64, NX_NULL);
    if (status)
    {
        ERROR_COUNTER();
    }
     ...
     /* Enable IPv6 traffic.  */
    status = nxd_ipv6_enable(&client_ip[0]);
    if (status)
    {
        ERROR_COUNTER();
    }
    ...
    /* Enable ICMP processing.  */
    status = nxd_icmp_enable(&client_ip[0]);
    if (status)
    {
        ERROR_COUNTER();
    }
    ...
    /* Set address with IPv6 address. */
    client_address_0.nxd_ip_version = NX_IP_VERSION_V6;
    client_address_0.nxd_ip_address.v6[0] = 0xfe800000;
    client_address_0.nxd_ip_address.v6[1] = 0x00000000;
    client_address_0.nxd_ip_address.v6[2] = 0x00000000;
    client_address_0.nxd_ip_address.v6[3] = 0x00000005;

    status = nxd_ipv6_address_set(&client_ip[0], 0, &client_address_0, 64, NX_NULL);
    if (status)
    {
        ERROR_COUNTER();
    }
     ...
     /* Enable IPv6 traffic.  */
    status = nxd_ipv6_enable(&client_ip[1]);
    if (status)
    {
        ERROR_COUNTER();
    }
    ...
    /* Enable ICMP processing.  */
    status = nxd_icmp_enable(&client_ip[1]);
    if (status)
    {
        ERROR_COUNTER();
    }
    ...
    /* Set address with IPv6 address. */
    client_address_1.nxd_ip_version = NX_IP_VERSION_V6;
    client_address_1.nxd_ip_address.v6[0] = 0xfe800000;
    client_address_1.nxd_ip_address.v6[1] = 0x00000000;
    client_address_1.nxd_ip_address.v6[2] = 0x00000000;
    client_address_1.nxd_ip_address.v6[3] = 0x00000006;

    status = nxd_ipv6_address_set(&client_ip[1], 0, &client_address_1, 64, NX_NULL);
    if (status)
    {
        ERROR_COUNTER();
    }
...
static UINT client_receive_count = 0;
...
UINT temp_index;
    ...
    printf("NetX Secure Test:   DTLS Multiple IP Address Test......................");
    ...
    /* Send packet to the connected clients.  */
    for (i = 0; i < NUM_SERVER_SESSIONS; i++)
    {

        /* Check if this session is in use.  */
        if (!dtls_server.nx_dtls_server_sessions[i].nx_secure_dtls_session_in_use)
        {
            continue;
        }

        /* Set the packet content.  */
        for (j = 0; j < sizeof(response_buffer[i]); j++)
        {
            response_buffer[i][j] = j << i;
        }

        /* Prepare packet to send. */
        status = nx_secure_dtls_packet_allocate(&dtls_server.nx_dtls_server_sessions[i], &server_pool, &packet_ptr, NX_NO_WAIT);
        if (status)
        {
            ERROR_COUNTER();
        }

        status = nx_packet_data_append(packet_ptr, response_buffer[i], sizeof(response_buffer[i]),
                                       &server_pool, NX_NO_WAIT);
        if (status)
        {
            ERROR_COUNTER();
        }
        ...
        /* Send the packet out.  */
        status = nx_secure_dtls_server_session_send(&(dtls_server.nx_dtls_server_sessions[i]), packet_ptr);
        if (status)
        {
            ERROR_COUNTER();
        }
    ...
    /* Wait client received the packets.  */
    while (client_receive_count < dtls_server.nx_dtls_server_sessions_count)
    {
        tx_thread_sleep(1);
    }

    /* Modify the local IP address index of session 1.  */
    temp_index = dtls_server.nx_dtls_server_sessions[1].nx_secure_dtls_local_ip_address_index;
    dtls_server.nx_dtls_server_sessions[1].nx_secure_dtls_local_ip_address_index = dtls_server.nx_dtls_server_sessions[0].nx_secure_dtls_local_ip_address_index;

    /* Prepare packet to send. */
    status = nx_secure_dtls_packet_allocate(&dtls_server.nx_dtls_server_sessions[1], &server_pool, &packet_ptr, NX_NO_WAIT);
    if (status)
    {
        ERROR_COUNTER();
    }

    status = nx_packet_data_append(packet_ptr, response_buffer[1], sizeof(response_buffer[1]), &server_pool, NX_NO_WAIT);
    if (status)
    {
        ERROR_COUNTER();
    }

    /* Send the packet with wrong IP address.  */
    status = nx_secure_dtls_server_session_send(&(dtls_server.nx_dtls_server_sessions[1]), packet_ptr);
    if (status)
    {
        ERROR_COUNTER();
    }

    /* Recover the local IP address index.  */
    dtls_server.nx_dtls_server_sessions[1].nx_secure_dtls_local_ip_address_index = temp_index;
    dtls_server.nx_dtls_server_sessions[1].nx_secure_dtls_tls_session.nx_secure_tls_local_sequence_number[0]--;

    /* Modify the UDP port of session 1.  */
    dtls_server.nx_dtls_server_sessions[1].nx_secure_dtls_udp_socket -> nx_udp_socket_port++;
    ...
    /* Send the packet with wrong port.  */
    status = nx_secure_dtls_server_session_send(&(dtls_server.nx_dtls_server_sessions[1]), packet_ptr);
    if (status)
    {
        ERROR_COUNTER();
    ...
    /* Recover the UDP port.  */
    dtls_server.nx_dtls_server_sessions[1].nx_secure_dtls_udp_socket -> nx_udp_socket_port--;
    dtls_server.nx_dtls_server_sessions[1].nx_secure_dtls_tls_session.nx_secure_tls_local_sequence_number[0]--;

    /* Wait clients deleted.  */
...
{
UINT status;
NXD_ADDRESS *server_address = &address_0;
NX_PACKET *packet_ptr;
ULONG request_length = 0;
NX_IPV6_HEADER *ip_header;
    ...
    status = nx_secure_dtls_session_receive(&dtls_client_session[0], &packet_ptr, 5 * NX_IP_PERIODIC_RATE);
    if (status)
    {
        ERROR_COUNTER();
    }
    else
    {
        ...
        if (memcmp(ip_header -> nx_ip_header_source_ip, server_address -> nxd_ip_address.v6, 16) != 0)
        {
            ERROR_COUNTER();
        }

        memset(request_buffer, 0, sizeof(request_buffer));
        nx_packet_data_retrieve(packet_ptr, request_buffer, &request_length);
        nx_packet_release(packet_ptr);

        if ((request_length != sizeof(response_buffer[0])) ||
            memcmp(request_buffer, response_buffer[0], request_length))
        {
            ERROR_COUNTER();
        }
...
{
UINT status;
NXD_ADDRESS *server_address = &address_1;
NX_PACKET *packet_ptr;
ULONG request_length = 0;
NX_IPV6_HEADER *ip_header;
    ...
    /* Receive packet from server.  */
    status = nx_secure_dtls_session_receive(&dtls_client_session[1], &packet_ptr, 5 * NX_IP_PERIODIC_RATE);
    if (status)
    {
        ERROR_COUNTER();
    }
    else
    {
        ...
        if ((request_length != sizeof(response_buffer[1])) ||
            memcmp(request_buffer, response_buffer[1], request_length))
        {
            ERROR_COUNTER();
        }
    ...
    /* Receive packet from server.  */
    /* Server send a packet with wrong IP address, this packet shouldn't be received.  */
    status = nx_secure_dtls_session_receive(&dtls_client_session[1], &packet_ptr, 1 * NX_IP_PERIODIC_RATE);
    if (!status)
    {
        ERROR_COUNTER();
    }
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Secure Test:   DTLS Multiple IP Address Test......................N/A\n");
