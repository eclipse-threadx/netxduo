...
void test_application_define(void *first_unused_memory)
#else
VOID    nx_secure_dtls_fragment_test_application_define(void *first_unused_memory)
#endif
{
UINT     status;
CHAR    *pointer;
...
UCHAR *data;
ULONG checksum;
ULONG ip_src_addr = 0x01020305;
ULONG ip_dest_addr = 0x01020304;
NX_PACKET *frag_pkt;
UINT fragment_offset;
UINT fragment_length = 32;
UINT temp_len;
    ...
    if ((data[8] == NX_SECURE_TLS_HANDSHAKE))
    {
        ...
        /* Client Hello: message sequence = 0, Client Key Exchange: message sequence = 2.  */
        if ((data[25] == 0) && (data[26] == 0 || data[26] == 2))
        {

            /* Copy the packet for fragment.  */
            nx_packet_copy(packet_ptr, &frag_pkt, &client_pool[0], NX_WAIT_FOREVER);

            /* Remove the fragment data.  */
            packet_ptr -> nx_packet_append_ptr -= fragment_length;
            packet_ptr -> nx_packet_length -= fragment_length;

            /* Update the fragment, record, and UDP length.  */
            temp_len = (UINT)((data[31] << 8 | data[32]) - fragment_length);
            fragment_offset = temp_len;
            data[31] = (UCHAR)(temp_len >> 8);
            data[32] = (UCHAR)(temp_len & 0xff);
            temp_len = (UINT)((data[19] << 8 | data[20]) - fragment_length);
            data[19] = (UCHAR)(temp_len >> 8);
            data[20] = (UCHAR)(temp_len & 0xff);
            temp_len = (UINT)((data[4] << 8 | data[5]) - fragment_length);
            data[4] = (UCHAR)(temp_len >> 8);
            data[5] = (UCHAR)(temp_len & 0xff);

            /* Reset checksum.  */
            data[6] = 0;
            data[7] = 0;

            /* Compute the new UDP checksum.  */
            ...
            /* Update checksum.  */
            data[6] = (UCHAR)(checksum >> 8);
            data[7] = (UCHAR)(checksum & 0xff);

            _nx_udp_packet_receive(ip_ptr, packet_ptr);

            /* Set the remaining bytes as the fragment packet.  */
            memmove(frag_pkt -> nx_packet_prepend_ptr + 33, frag_pkt -> nx_packet_append_ptr - fragment_length, fragment_length);
            frag_pkt -> nx_packet_length = 33 + fragment_length;
            frag_pkt -> nx_packet_append_ptr = frag_pkt -> nx_packet_prepend_ptr + frag_pkt -> nx_packet_length;

            data = frag_pkt -> nx_packet_prepend_ptr;

            /* Update the sequence number.  */
            data[18]++;
            dtls_client_session[0].nx_secure_dtls_tls_session.nx_secure_tls_local_sequence_number[0]++;

            /* Update the fragment offset.  */
            data[28] = (UCHAR)(fragment_offset >> 8);
            data[29] = (UCHAR)(fragment_offset & 0xff);

            /* Update the fragment, record, and UDP length.  */
            data[31] = 0;
            data[32] = fragment_length;
            data[19] = 0;
            data[20] = data[32] + 12;
            data[4] = 0;
            data[5] = data[20] + 21;
            ...
            /* Compute the new UDP checksum.  */
            checksum = _nx_ip_checksum_compute(frag_pkt,
                                               NX_PROTOCOL_UDP,
                                               (UINT)frag_pkt -> nx_packet_length,
                                               &ip_src_addr,
                                               &ip_dest_addr);
            checksum = ~checksum & NX_LOWER_16_MASK;
            ...
            _nx_udp_packet_receive(ip_ptr, frag_pkt);
...
UINT status;
    ...
    printf("NetX Secure Test:   DTLS Fragment Test.................................");
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Secure Test:   DTLS Fragment Test.................................N/A\n");
