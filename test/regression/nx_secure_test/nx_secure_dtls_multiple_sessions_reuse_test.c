...
void test_application_define(void *first_unused_memory)
#else
VOID    nx_secure_dtls_multiple_sessions_reuse_test_application_define(void *first_unused_memory)
#endif
{
UINT     status;
CHAR    *pointer;
...
{
    connect_session[server_connect_count & 0x7] = dtls_session;
...
NX_SECURE_DTLS_SESSION *session_ptr;
    ...
    printf("NetX Secure Test:   DTLS Multiple Sessions Reuse Test..................");
    ...
    for (i = 0; i < 100; i++)
    {
        ...
        session_ptr = connect_session[(server_connect_count - 1) & 0x7];

        /* Connect to the client.  */
        status = nx_secure_dtls_server_session_start(session_ptr, NX_WAIT_FOREVER);
        if (status)
        {
            ERROR_COUNTER();
        }

        /* End session for reuse.  */
        nx_secure_dtls_session_end(session_ptr, NX_NO_WAIT);
    }
...
CHAR socket_name[] = "Client socket X";
    ...
    /* Resume client 1 to connect to server.  */
    tx_thread_resume(&client_thread[1]);
...
CHAR socket_name[] = "Client socket X";
    ...
    /* Create UDP socket. */
    socket_name[14] = '0' + client_index;
    status = nx_udp_socket_create(&client_ip[client_index], &client_socket[client_index], socket_name, NX_IP_NORMAL, NX_DONT_FRAGMENT, 0x80, 5);
    if (status)
    {
        ERROR_COUNTER();
    }
    ...
    for (i = 0; i < 100; i++)
    {
        ...
        tx_semaphore_get(&semaphore_server, TX_WAIT_FOREVER);
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Secure Test:   DTLS Multiple Sessions Reuse Test..................N/A\n");
