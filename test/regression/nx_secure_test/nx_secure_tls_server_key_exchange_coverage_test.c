...
static void    certificate_header_initialize(UCHAR *header, UINT certificate_size);
...
/* Certificate.  */
static UCHAR certificate_header[6];

static UCHAR certificate_header_rsa[6];

/* Initialize a TLS Certificate message header from the DER certificate size.  */
static void certificate_header_initialize(UCHAR *header, UINT certificate_size)
{
UINT certificate_list_size;

    certificate_list_size = certificate_size + 3U;
    header[0] = (UCHAR)((certificate_list_size >> 16) & 0xFFU);
    header[1] = (UCHAR)((certificate_list_size >> 8) & 0xFFU);
    header[2] = (UCHAR)(certificate_list_size & 0xFFU);
    header[3] = (UCHAR)((certificate_size >> 16) & 0xFFU);
    header[4] = (UCHAR)((certificate_size >> 8) & 0xFFU);
    header[5] = (UCHAR)(certificate_size & 0xFFU);
}
...
UINT       test_cert_size = 0;
    ...
    /* Initialize the certificate message headers from the current certificates. */
    certificate_header_initialize(certificate_header, ECTestServer2_der_len);
    certificate_header_initialize(certificate_header_rsa, test_device_cert_der_len);
