...
void test_application_define(void *first_unused_memory)
#else
VOID    nx_secure_tls_1_3_receive_invalid_server_handshake_message_test_application_define(void *first_unused_memory)
#endif
{
UINT     status;
CHAR    *pointer;
...
UCHAR buffer[32];
    ...
    printf("NetX Secure Test:   TLS 1.3 Receive Invalid Server Handshake message...");
    ...
    status += _nx_secure_tls_send_handshake_record(&tls_server_session_0, send_packet, 0XFF, NX_WAIT_FOREVER);
    do_something_if_fail(status == NX_SUCCESS);
}
...
UINT i;
    ...
    do_something_if_fail(NX_SECURE_TLS_HANDSHAKE_FAILURE == status);
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Secure Test:   TLS 1.3 Receive Invalid Server Handshake message...N/A\n");
