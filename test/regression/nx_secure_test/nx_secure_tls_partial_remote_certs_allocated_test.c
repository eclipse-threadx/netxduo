/* Test automated certificate allocation optimization.  */
...
/* NOTE: For this test, we only allocate a single certificate even though the chain
         will contain two certs - this is to test the internal allocation when the
         application doesn't allocate enough slots to hold the certificates in the chain. */
static UCHAR remote_client_certs_buffer[(sizeof(NX_SECURE_X509_CERT) + 2000)];
static UCHAR remote_server_certs_buffer[(sizeof(NX_SECURE_X509_CERT) + 2000)];
...
UINT status;
    ...
    status = nx_secure_tls_remote_certificate_buffer_allocate(tls_session_ptr, 1, remote_server_certs_buffer, sizeof(remote_server_certs_buffer));
...
UINT status;
    ...
    status = nx_secure_tls_session_x509_client_verify_configure(tls_session_ptr, 0, NX_NULL, 0);
    if (status)
    {
        ERROR_COUNTER();
    }
    ...
    /* Need to allocate space for the certificate coming in from the remote host. */
    status = nx_secure_tls_remote_certificate_buffer_allocate(tls_session_ptr, 1, remote_client_certs_buffer, sizeof(remote_client_certs_buffer));
...
ULONG response_length;
    ...
    printf("NetX Secure Test:   TLS partial remote certs allocated Test............");
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Secure Test:   TLS partial remote certs allocated Test ...........N/A\n");
