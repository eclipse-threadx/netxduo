...
#if !defined(NX_SECURE_TLS_CLIENT_DISABLED) && !defined(NX_SECURE_TLS_SERVER_DISABLED) && defined(NX_SECURE_ENABLE_DTLS) && (NX_SECURE_TLS_TLS_1_1_ENABLED)
...
void test_application_define(void *first_unused_memory)
#else
VOID    nx_secure_dtls_version_1_0_test_application_define(void *first_unused_memory)
#endif
{
UINT     status;
CHAR    *pointer;
...
UINT status;
    ...
    printf("NetX Secure Test:   DTLS Version1.0 Test...............................");

    server_dtls_setup(&dtls_server);

    /* Override version to DTLS1.0.  */
    status = nx_secure_dtls_server_protocol_version_override(&dtls_server, NX_SECURE_DTLS_VERSION_1_0);
    if (status)
    {
        ERROR_COUNTER();
    }
    ...
    tx_semaphore_get(&semaphore_connect, NX_WAIT_FOREVER);

    status = nx_secure_dtls_server_session_start(connect_session[0], NX_IP_PERIODIC_RATE);

    /* Unsupported version(DTLS1.2), but after negotiation, server will support DTLS1.0  */
    if (status)
    {
    ...
    /* End the sessions.  */
    /* Server and client end sessions normally.  */
    status = nx_secure_dtls_session_end(connect_session[1], 5 * NX_IP_PERIODIC_RATE);
    if (status)
    {
        ERROR_COUNTER();
    }
...
NXD_ADDRESS server_address;
    ...
    /* Start DTLS session with version DTLS1.2; after negotiation, server will support DTLS1.0. */
    status = nx_secure_dtls_client_session_start(&dtls_client_session[0], &client_socket[0], &server_address, SERVER_PORT, NX_IP_PERIODIC_RATE);
    if (status)
    {
        ERROR_COUNTER();
    }
...
NXD_ADDRESS server_address;
    ...
    /* Override version to DTLS1.0.  */
    status = nx_secure_dtls_client_protocol_version_override(&dtls_client_session[1], NX_SECURE_DTLS_VERSION_1_0);
    if (status)
    {
        ERROR_COUNTER();
    }
    ...
    status = nx_secure_dtls_session_end(&dtls_client_session[1], 5 * NX_IP_PERIODIC_RATE);
    if (status)
    {
        ERROR_COUNTER();
    }
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Secure Test:   DTLS Version1.0 Test...............................N/A\n");
