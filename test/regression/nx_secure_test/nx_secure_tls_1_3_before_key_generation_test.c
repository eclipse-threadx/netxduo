...
UCHAR buffer[32];
    ...
    printf("NetX Secure Test:   TLS 1.3 Before Key Generation Test.................");
    ...
    do_something_if_fail(status == NX_SECURE_TLS_AEAD_DECRYPT_FAIL);

    /* All messages from client are received and server should not corrupted. */
    status = nx_secure_tls_session_start(&tls_server_session_0, &server_socket_0, NX_IP_PERIODIC_RATE);
    do_something_if_fail(status == NX_NO_PACKET);
...
UCHAR packet_buffer[64];
    ...
    tls_client_session_0.nx_secure_tls_protocol_version = 0x0303;
    ...
    ecdhe_data = &tls_client_session_0.nx_secure_tls_key_material.nx_secure_tls_ecc_key_data[0];
    ...
    ecdhe_data = &tls_client_session_0.nx_secure_tls_key_material.nx_secure_tls_ecc_key_data[1];
    ...
    status = nx_packet_data_append(send_packet, client_hello_two_key_share, sizeof(client_hello_two_key_share), tls_client_session_0.nx_secure_tls_packet_pool, NX_NO_WAIT);
    do_something_if_fail(status == NX_SUCCESS);
    ...
    /* Fill some garbage into the packet to make the record "encrypted". */
    memset(packet_buffer, 0xff, sizeof(packet_buffer));
    status = nx_packet_data_append(send_packet, packet_buffer, sizeof(packet_buffer), tls_client_session_0.nx_secure_tls_packet_pool, NX_NO_WAIT);
    do_something_if_fail(status == NX_SUCCESS);

    status = _nx_secure_tls_send_record(&tls_client_session_0, send_packet, NX_SECURE_TLS_APPLICATION_DATA, NX_WAIT_FOREVER);
    do_something_if_fail(status == NX_SUCCESS);
    tx_mutex_put(&_nx_secure_tls_protection);

    /* Receive messages from server. */
    status =  nx_tcp_socket_receive(&client_socket_0, &packet_ptr, NX_WAIT_FOREVER);
    do_something_if_fail(status == NX_SUCCESS);
    ...
    /* Send ClientHello without key shares to make server not able to generate a secret. */
    tx_mutex_get(&_nx_secure_tls_protection, TX_WAIT_FOREVER);
    tls_client_session_0.nx_secure_tls_tcp_socket = &client_socket_0;
    tls_client_session_0.nx_secure_tls_packet_pool = &pool_0;
    tls_client_session_0.nx_secure_tls_protocol_version = 0x0303;
    ...
    status = nx_packet_data_append(send_packet, client_hello_empty_key_share, sizeof(client_hello_empty_key_share), tls_client_session_0.nx_secure_tls_packet_pool, NX_NO_WAIT);
    do_something_if_fail(status == NX_SUCCESS);
    status = _nx_secure_tls_send_handshake_record(&tls_client_session_0, send_packet, NX_SECURE_TLS_CLIENT_HELLO, NX_WAIT_FOREVER);
    do_something_if_fail(status == NX_SUCCESS);
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Secure Test:   TLS 1.3 Before Key Generation Test.................N/A\n");
