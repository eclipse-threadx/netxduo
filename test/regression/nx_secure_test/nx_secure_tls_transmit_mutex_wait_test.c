...
static TX_SEMAPHORE             send_2_sem;
...
static ULONG                    thread_2_stack[THREAD_STACK_SIZE / sizeof(ULONG)];
...
static UCHAR                    request_buffer2[BUFFER_SIZE];
...
void test_application_define(void *first_unused_memory)
#else
VOID    nx_secure_tls_transmit_mutex_wait_test_application_define(void *first_unused_memory)
#endif
{
UINT     status;
CHAR    *pointer;
    ...
    tx_semaphore_create(&send_2_sem, "send_2_sem", 0);
    ...
    /* Create the second sending thread.  */
    tx_thread_create(&thread_2, "thread 2", ntest_2_entry, 0,
                     thread_2_stack, sizeof(thread_2_stack),
...
NX_PACKET *packet_ptr;
    ...
    printf("NetX Secure Test:   TLS Transmit Mutex Wait Test.......................");
    ...
    nx_packet_data_retrieve(packet_ptr, response_buffer, &response_length);
    nx_packet_release(packet_ptr);
    if ((response_length != sizeof(request_buffer2)) ||
        memcmp(request_buffer2, response_buffer, response_length))
    {
        ERROR_COUNTER();
    }
...
NXD_ADDRESS server_address;
    ...
    /* Create TCP socket. */
    status = nx_tcp_socket_create(&ip_1, &client_socket_0, "Client socket", NX_IP_NORMAL,
                                  NX_DONT_FRAGMENT, NX_IP_TIME_TO_LIVE, 8192, NX_NULL, NX_NULL);
    if (status)
    {
        ERROR_COUNTER();
    ...
    /* Prepare packet to send. */
    status = nx_packet_allocate(&pool_1, &packet_ptr, NX_TCP_PACKET, NX_NO_WAIT);
    if (status)
    {
        ERROR_COUNTER();
    }
...
NXD_ADDRESS server_address;
    ...
    tx_semaphore_get(&send_2_sem, TX_WAIT_FOREVER);
   ...
   /* Prepare packet to send. */
    ...
    status = nx_packet_data_append(packet_ptr, request_buffer2, sizeof(request_buffer2),
                                   &pool_1, NX_NO_WAIT);
    if (status)
    {
        ERROR_COUNTER();
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Secure Test:   TLS Transmit Mutex Wait Test.......................N/A\n");
