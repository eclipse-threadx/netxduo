...
void test_application_define(void *first_unused_memory)
#else
VOID    nx_secure_dtls_multiple_sessions_connect_test_application_define(void *first_unused_memory)
#endif
{
UINT     status;
CHAR    *pointer;
...
INT  i;
    ...
    printf("NetX Secure Test:   DTLS Multiple Sessions Connect Test................");
    ...
    status = nx_secure_dtls_session_end(connect_session[0], 500);
    if (status)
    {
        ERROR_COUNTER();
    }

    status = nx_secure_dtls_session_end(connect_session[1], 500);
    if (status)
    {
        ERROR_COUNTER();
    }
...
NXD_ADDRESS server_address;
    ...
    status = nx_secure_dtls_session_end(&dtls_client_session[0], 500);
    if (status)
    {
        ERROR_COUNTER();
    }
...
NXD_ADDRESS server_address;
    ...
    status = nx_secure_dtls_session_end(&dtls_client_session[1], 500);
    if (status)
    {
        ERROR_COUNTER();
    }
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Secure Test:   DTLS Multiple Sessions Connect Test................N/A\n");
