...
#else
void    netx_bsd_tcp_ioctl_nonblocking_test_application_define(void *first_unused_memory)
#endif
{
...
static ULONG stack_blocking_space[NUM_CLIENTS][DEMO_STACK_SIZE / sizeof(ULONG)];
...
static TX_THREAD helper_blocking_thread[NUM_CLIENTS];

static VOID bsd_server_blocking_helper_thread_entry(ULONG thread_input)
{
int ret;
...
char  buf[30];
    ...
    ret = select(sockfd + 1, &read_fd, NULL, &except_fd, &tv);
    if(ret < 0)
    {
        error_counter++;
        return;
...
static void test_tcp_blocking_server4(void)
{
int                sockfd;
...
int                ioctl_arg;
    ...
    ioctl_arg = NX_TRUE;
    if(ioctl(sockfd, FIONBIO, &ioctl_arg))
        error_counter++;
    ...
    for(i = 0; i < NUM_CLIENTS; i++)
    {
        ...
        status = tx_thread_create(&helper_blocking_thread[i], "helper thread", bsd_server_blocking_helper_thread_entry,
                i, stack_blocking_space[i], DEMO_STACK_SIZE, 2, 2, TX_NO_TIME_SLICE, TX_AUTO_START);
        if(status != TX_SUCCESS)
            error_counter++;
...
char mac_ip1[6];
#endif
    printf("NetX Test:   Basic BSD TCP Ioctl Non Blocking Connect Test.");
...
#endif
{
    ...
    printf("NetX Test:   Basic BSD TCP Ioctl Non Blocking Connect Test.N/A\n"); 
