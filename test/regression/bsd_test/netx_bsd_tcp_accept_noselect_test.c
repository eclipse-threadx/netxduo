...
#else
void    netx_bsd_tcp_accept_noselect_test_application_define(void *first_unused_memory)
#endif
{
...
static void    ntest_0_entry(ULONG thread_input)
{
    ...
    printf("NetX Test:   Basic BSD TCP Accept No Select Test...........");
...
NX_PACKET     *packet_ptr;
    ...
    /* Now we should be able to try another connection.  This one would fail.  */
    status = nx_tcp_client_socket_connect(&tcp_sockets[NUM_CLIENTS - 1], IP_ADDRESS(1, 2, 3, 4), 12345, 1);
    if(status == NX_SUCCESS)
    {
    ...
    /* Disconnect and clean up this socket. */
    status = nx_tcp_socket_disconnect(&tcp_sockets[NUM_CLIENTS - 1], NX_IP_PERIODIC_RATE / 10);
    ...
    for(i = 0; i < (NUM_CLIENTS - 1); i++)
    {
        ...
        status += nx_packet_data_append(packet_ptr, requests[i&3], strlen(requests[i&3]),
                                        &pool_0, NX_NO_WAIT);
        status += nx_tcp_socket_send(&tcp_sockets[i], packet_ptr, NX_IP_PERIODIC_RATE / 10);
    ...
    status = nx_tcp_socket_disconnect(&tcp_sockets[NUM_CLIENTS/2], NX_IP_PERIODIC_RATE / 10);
    if(status == NX_NOT_CONNECTED || status == NX_DISCONNECT_FAILED)
        status = 0;
    ...
    status = nx_tcp_client_socket_connect(&tcp_sockets[NUM_CLIENTS - 1], IP_ADDRESS(1, 2, 3, 4), 12345, TX_TIMER_TICKS_PER_SECOND);
    if(status != NX_SUCCESS)
    {
        /* At this point, we anticpate the BSD system to run out of resource and is unable to make
           this connection.  */
    ...
    status += nx_packet_data_append(packet_ptr, requests[(NUM_CLIENTS - 1) & 3], strlen(requests[(NUM_CLIENTS - 1) & 3]),
                                    &pool_0, NX_NO_WAIT);
    status += nx_tcp_socket_send(&tcp_sockets[NUM_CLIENTS - 1], packet_ptr, NX_IP_PERIODIC_RATE / 10);
    if(status != NX_SUCCESS)
        error_counter++;
...
#endif
{
    ...
    printf("NetX Test:   Basic BSD TCP Accept No Select Test...........N/A\n"); 
