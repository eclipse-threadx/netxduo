...
#else
void    netx_bsd_raw_basic_blocking_test_application_define(void *first_unused_memory)
#endif
{
...
int                addrlen;
    ...
    /* Validate the data. */
    if((ret != (int)(strlen(requests[0]) + 20)) || strncmp((buf + 20), requests[0], (ret - 20)))
        error_counter++;    
    ...
    /* Test raw packet type. */
    sockfd = socket(AF_PACKET, SOCK_RAW, 200);
    if(sockfd < 0)
        error_counter++;

    sock_addr.sll_family = AF_PACKET;
    sock_addr.sll_ifindex = 0;
    ret = sendto(sockfd, response[0], strlen(response[0]), 0, (struct sockaddr*)&sock_addr, sizeof(sock_addr));
    if(ret != (int)strlen(response[0]))
        error_counter++;
...
char mac_ip1[6];
#endif
    printf("NetX Test:   Basic BSD Raw Blocking Test...................");
...
#endif
{
    printf("NetX Test:   Basic BSD Raw Blocking Test...................N/A\n");
    test_control_return(3);
}
