...
#else
void    netx_bsd_tcp_getsockname_test_application_define(void *first_unused_memory)
#endif
{
...
static void start_tcp_client4(int port)
{
UINT status;
NX_PACKET *packet_ptr;
    ...
    status = nx_tcp_client_socket_connect(&client_socket, IP_ADDRESS(1,2,3,4), port, NX_IP_PERIODIC_RATE);
    if(status != NX_SUCCESS)
        error_counter++;
    ...
    status = nx_tcp_socket_receive(&client_socket, &packet_ptr, 2 * NX_IP_PERIODIC_RATE);
    if(status)
        error_counter++;
...
int                port = (int)param;
    ...
    for(j = 0; j < 2; j++)
    {
        ...
        /* Set the client data */
        client_data[j * 2 + i].sockfd = newsock;
        client_data[j * 2 + i].message_id = i;
        
        /* Create a helper thread to handle the new socket. */
        status = tx_thread_create(&helper_thread[j*2 + i], "helper thread", bsd_server_helper_thread_entry, 
                              j * 2 + i, stack_space[j*2 + i], DEMO_STACK_SIZE, 2, 2, TX_NO_TIME_SLICE, TX_AUTO_START);
...
UINT status;
    ...
    printf("NetX Test:   BSD TCP getsockname Test......................");
    ...
    start_tcp_client4(12345);

    start_tcp_client4(12346);
...
#endif
{
    ...
    printf("NetX Test:   BSD TCP getsockname Test......................N/A\n"); 
