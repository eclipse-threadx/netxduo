...
#define     SECONDARY_SERVER_PORT   85
...
#else
void    netx_http_client_change_connect_port_test_application_define(void *first_unused_memory)
#endif
{
...
NX_PACKET       *send_packet;
    ...
    /* Based on information from the Server we need to connect on a different port. */
    status = nx_http_client_set_connect_port(&my_client, SECONDARY_SERVER_PORT);
    ...
    /* Need a sleep to let the 'server' thread listen on the new port. */
    tx_thread_sleep(10);
...
NX_PACKET   *my_packet;
    ...
    printf("NetX Test:   HTTP Client Change Connect Port Test......................");
    ...
    /* Bind the TCP socket to the default HTTP  port.  */
    ...
    status = nx_http_response_packet_send(&server_socket, 80, 0);
    ...
    nx_tcp_server_socket_unlisten(&server_ip, SERVER_PORT);
        
    /* Listen on the alternative server port.  */
    status =  nx_tcp_server_socket_listen(&server_ip, SECONDARY_SERVER_PORT, &server_socket, 5, NX_NULL);
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Test:   HTTP Client Change Connect Port Test......................N/A\n"); 
