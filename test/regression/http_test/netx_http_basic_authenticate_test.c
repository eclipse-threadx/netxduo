...
#else
void    netx_http_basic_authenticate_test_application_define(void *first_unused_memory)
#endif
{
...
NX_PACKET       *send_packet;
    ...
    /* Basic authentication, name = "name", passwd = "password". */
#ifdef __PRODUCT_NETXDUO__ 
    ...
    /* Should  pass the authentication. */
    if(status)
        error_counter++;
    ...
    /* Basic Authentication, name = "name2", passwd = "password". */
#ifdef __PRODUCT_NETXDUO__ 

    /* Use the 'duo' service to send a GET request to the server (can use IPv4 or IPv6 addresses). */
    status =  nxd_http_client_get_start(&my_client, &server_ip_address, 
                                            "/client_test.htm", NX_NULL, 0, "name2", "password", 50);
#else

    /* Use the 'NetX' service to send a GET request to the server (can only use IPv4 addresses). */
    status =  nx_http_client_get_start(&my_client, HTTP_SERVER_ADDRESS, "/client_test.htm", 
                                       NX_NULL, 0, "name2", "password", 50);
#endif  

    /* Shouldn't pass the authentication. */ 
    if(status != NX_HTTP_AUTHENTICATION_ERROR)
        error_counter++;
    ...
    /* Basic Authentication, name = "name", passwd = "". */
#ifdef __PRODUCT_NETXDUO__ 

    /* Use the 'duo' service to send a GET request to the server (can use IPv4 or IPv6 addresses). */
    status =  nxd_http_client_get_start(&my_client, &server_ip_address, 
                                            "/client_test.htm", NX_NULL, 0, "name", "", 50);
#else
...
UINT            status;
    ...
    printf("NetX Test:   HTTP Basic Athentication Test.............................");
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Test:   HTTP Basic Athentication Test.............................N/A\n"); 
