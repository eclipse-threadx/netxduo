...
extern TAHI_TEST_SEQ tahi_04_016[];

extern int tahi_04_016_size;

static TAHI_TEST_SUITE test_suite[1];
static void build_test_suite(void)
{
    test_suite[0].test_case = &tahi_04_016[0];test_suite[0].test_case_size = tahi_04_016_size;
}
...
#else
void           netx_tahi_test_4_16_define(void *first_unused_memory)
#endif
{
    CHAR       *pointer;
    UINT       status;
    ...
    status = nx_packet_pool_create(&pool_0, "NetX Main Packet Pool", (1536 + NX_IPSEC_MAX_HEADER_SIZE), pointer, (1536 + NX_IPSEC_MAX_HEADER_SIZE)*16);
