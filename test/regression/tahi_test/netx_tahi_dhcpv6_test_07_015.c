...
extern TAHI_TEST_SEQ tahi_dhcpv6_07_015[];
extern int  tahi_dhcpv6_07_015_size;
...
#else
void netx_tahi_dhcpv6_test_07_015_define(void * first_unused_memory)
#endif
{
CHAR    *pointer;
UINT    status;
...
UINT        status;
    ...
    netx_tahi_run_test_case(&ip_0, &tahi_dhcpv6_07_015[0], tahi_dhcpv6_07_015_size);
...
UCHAR           cid[18] = {0x00, 0x01, 0x00, 0x0e, 0x00, 0x01, 0x00, 0x01,
    ...
    if(src_dst_port == 0x02220223)
    {
        ...
        {
            /* Modify the xid to be the same with Tahi test packet. */
...
ULONG                   message_type;
    ...
    if((dst_port & 0x0000FFFF) == 0x00000222)
    {
        ...
        _nx_dhcpv6_utility_get_data(packet_ptr -> nx_packet_prepend_ptr + 8, 1, &message_type);
        *(ULONG *)(packet_ptr -> nx_packet_prepend_ptr + 8) = 0 + message_type;
