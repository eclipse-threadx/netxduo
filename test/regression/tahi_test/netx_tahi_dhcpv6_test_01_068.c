...
static UINT                     renew_count;

extern TAHI_TEST_SEQ tahi_dhcpv6_01_068[];
extern int  tahi_dhcpv6_01_068_size;

#ifdef CTEST
VOID test_application_define(void *first_unused_memory)
#else
void netx_tahi_dhcpv6_test_01_068_define(void * first_unused_memory)
#endif
{
CHAR    *pointer;
UINT    status;
...
UINT        status;
    ...
    netx_tahi_run_test_case(&ip_0, &tahi_dhcpv6_01_068[0], tahi_dhcpv6_01_068_size);
...
UCHAR           cid[18] = {0x00, 0x01, 0x00, 0x0e, 0x00, 0x01, 0x00, 0x01,
    ...
    if(src_dst_port == 0x02220223)
    {
        ...
        else if((message_type == NX_DHCPV6_MESSAGE_TYPE_RENEW) && (renew_count == 0))
        {
            /* Record original xid, modify the xid to be the same with Tahi test packet. */
            _nx_dhcpv6_utility_get_data(packet_ptr -> nx_packet_prepend_ptr + 9, 3, &original_xid);
                ...
                (message_type == NX_DHCPV6_MESSAGE_TYPE_REBIND))
        {
            ...
            /* Record original xid, modify the xid to be the same with Tahi test packet. */
            _nx_dhcpv6_utility_get_data(packet_ptr -> nx_packet_prepend_ptr + 9, 3, &original_xid);
            *(ULONG *)(packet_ptr -> nx_packet_prepend_ptr + 8) = 0xa4e49600 + message_type;
        }
