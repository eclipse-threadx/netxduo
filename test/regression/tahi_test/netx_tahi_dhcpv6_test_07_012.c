...
extern TAHI_TEST_SEQ tahi_dhcpv6_07_012[];
extern int  tahi_dhcpv6_07_012_size;
...
#else
void netx_tahi_dhcpv6_test_07_012_define(void * first_unused_memory)
#endif
{
UINT    status;
...
UINT        status;
    ...
    netx_tahi_run_test_case(&ip_0, &tahi_dhcpv6_07_012[0], tahi_dhcpv6_07_012_size);
...
UCHAR           cid[18] = {0x00, 0x01, 0x00, 0x0e, 0x00, 0x01, 0x00, 0x01,
    ...
    else if((src_dst_port & 0x0000FFFF) == 0x00000035)
    {
        ...
        *(USHORT *)(packet_ptr -> nx_packet_prepend_ptr + 8) = 0x910b;
