...
extern TAHI_TEST_SEQ tahi_dhcpv6_04_010[];
extern int  tahi_dhcpv6_04_010_size;
...
#else
void netx_tahi_dhcpv6_test_04_010_define(void * first_unused_memory)
#endif
{
UINT    status;
...
UINT        status;
    ...
    netx_tahi_set_dhcpv6(&dhcp_client);
    ...
    netx_tahi_run_test_case(&ip_0, &tahi_dhcpv6_04_010[0], tahi_dhcpv6_04_010_size);
...
UCHAR           cid[18] = {0x00, 0x01, 0x00, 0x0e, 0x00, 0x01, 0x00, 0x01,
    ...
    else if((src_dst_port & 0x0000FFFF) == 0x00000035)
    {
        ...
        {
            /* Modify the transmit ID to be the same with the packet captured by wireshark. */
            *(USHORT *)(packet_ptr -> nx_packet_prepend_ptr + 8) = 0x9d14;
        }
