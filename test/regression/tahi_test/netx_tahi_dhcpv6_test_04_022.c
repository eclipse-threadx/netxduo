...
extern TAHI_TEST_SEQ tahi_dhcpv6_04_022[];
extern int  tahi_dhcpv6_04_022_size;
...
#else
void netx_tahi_dhcpv6_test_04_022_define(void * first_unused_memory)
#endif
{
UINT    status;
...
UINT        status;
    ...
    netx_tahi_run_test_case(&ip_0, &tahi_dhcpv6_04_022[0], tahi_dhcpv6_04_022_size);
...
NXD_ADDRESS host_ipv6_address;
    ...
    status = nxd_dns_host_by_name_get(&dns_client, (UCHAR *)"dhcpv6.test.example.com", &host_ipv6_address, 1, NX_IP_VERSION_V6);                
}
...
UCHAR           cid[18] = {0x00, 0x01, 0x00, 0x0e, 0x00, 0x01, 0x00, 0x01,
    ...
    if(src_dst_port == 0x02220223)
    {
        ...
        else if(message_type == NX_DHCPV6_MESSAGE_TYPE_RENEW)
        {
            /* Record original xid, modify the xid to be the same with Tahi test packet. */
            _nx_dhcpv6_utility_get_data(packet_ptr -> nx_packet_prepend_ptr + 9, 3, &original_xid);
    ...
    else if((src_dst_port & 0x0000FFFF) == 0x00000035)
    {
        ...
        else if(renew_counter == 1)
        {
            /* Modify the transmit ID to be the same with the packet captured by wireshark. */
            *(USHORT *)(packet_ptr -> nx_packet_prepend_ptr + 8) = 0x2139;
        }
