...
static UINT                tcp_received = NX_FALSE;
static UINT                connect_succeed = NX_FALSE;
...
#else
void    netx_websocket_non_block_test_application_define(void *first_unused_memory)
#endif
{
CHAR    *pointer;
UINT    status;
...
static VOID connection_status_callback(NX_WEBSOCKET_CLIENT *websocket_client_ptr, VOID *context, UINT status)
{
    ...
    if (status != NX_SUCCESS)
        ...
        connect_succeed = NX_TRUE;
}

static VOID tcp_receive_callback(NX_TCP_SOCKET *socket_ptr)
{
    if (&test_client == socket_ptr)
    {
        tcp_received = NX_TRUE;
    }
}
...
UINT            code;
    ...
    /* Set the tcp receive callback. */
    nx_tcp_socket_receive_notify(&test_client, tcp_receive_callback);

    /* Set WebSocket client connect callback */
    nx_websocket_client_connection_status_callback_set(&client_websocket, NX_NULL, connection_status_callback);
    ...
    if (i == 5 || connect_succeed == NX_FALSE || client_websocket.nx_websocket_client_mutex.tx_mutex_ownership_count != 0)
        SET_ERROR_COUNTER(&error_counter, __FILE__, __LINE__);
...
#ifdef NX_SECURE_ENABLE
    tcp_received = NX_FALSE;
    connect_succeed = NX_FALSE;
    ...
    status = nx_secure_tls_session_start(&tls_client_session, &test_client, NX_NO_WAIT);
    if (status)
        SET_ERROR_COUNTER(&error_counter, __FILE__, __LINE__);
    ...
    else
    {
        ...
        status = nx_secure_tls_packet_allocate(&tls_client_session, &client_pool, &packet_ptr, NX_WAIT_FOREVER);
        if (status)
            SET_ERROR_COUNTER(&error_counter, __FILE__, __LINE__);
        /* Append client test data.  */
        status = nx_packet_data_append(packet_ptr, client_test_data, sizeof(client_test_data), &client_pool, NX_WAIT_FOREVER);
        if (status)
            SET_ERROR_COUNTER(&error_counter, __FILE__, __LINE__);
        ...
        for (UINT i = 0; i < 5; i++)
        {
            ...
            {
                status = nx_websocket_client_receive(&client_websocket, &packet_ptr, &code, NX_NO_WAIT);
                if ((status == NX_SUCCESS) && (code == NX_WEBSOCKET_OPCODE_BINARY_FRAME))
                {
                    break;
                }
                else
        ...
        if (i == 5 || client_websocket.nx_websocket_client_mutex.tx_mutex_ownership_count != 0)
            SET_ERROR_COUNTER(&error_counter, __FILE__, __LINE__);
...
NX_PACKET       *packet_ptr;
    ...
    printf("NetX Test:   Websocket Non Block Test.....................................");
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Test:   Websocket Non Block Test.............................N/A\n");
