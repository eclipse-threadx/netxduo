...
{
0x02, 0x04, 0x01, 0x02, 0x03, 0x04, // FIN: false
};

static UCHAR server_response_2_2[] =
{
0x00, 0x04, 0x01, 0x02, 0x03, 0x04, // FIN: false
};

static UCHAR server_response_2_3[] =
{
0x80, 0x04, 0x01, 0x02, 0x03, 0x04, // FIN: true
};
...
#else
void    netx_websocket_fin_test_application_define(void *first_unused_memory)
#endif
{
CHAR    *pointer;
UINT    status;
...
UINT            code;
    ...
    else
    {
        ...
        /* Receive the responsed data from server.  */
        status = nx_websocket_client_receive(&client_websocket, &packet_ptr1, &code, NX_IP_PERIODIC_RATE);
        if(status)
            error_counter++;
        /* The second response from the server shall be the beginning frame of fragmented frames */
        if (client_websocket.nx_websocket_client_frame_fragmented == NX_FALSE)
            error_counter++;
        ...
        if(status)
            error_counter++;
        /* The third response from the server shall be the continuation frame of fragmented frames */
        if (client_websocket.nx_websocket_client_frame_fragmented == NX_FALSE)
            error_counter++;
        ...
        if(status)
            error_counter++;
        /* The fourth response from the server shall be the termination frame of fragmented frames */
        if (client_websocket.nx_websocket_client_frame_fragmented == NX_TRUE)
            error_counter++;
...
NX_PACKET       *packet_ptr;
    ...
    printf("NetX Test:   Websocket Fin Test.....................................");
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Test:   Websocket Fin Test.....................................N/A\n");
