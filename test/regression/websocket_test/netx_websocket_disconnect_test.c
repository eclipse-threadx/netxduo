...
static UCHAR server_response_2[] =
{
0x88, 0x00, 0x01, 0x02, 0x03, 0x04,
};
...
#else
void    netx_websocket_disconnect_test_application_define(void *first_unused_memory)
#endif
{
CHAR    *pointer;
UINT    status;
...
UINT            code;
    ...
    /* The disconnect shall fail since is succeed only when the state is connected */
    status = nx_websocket_client_disconnect(&client_websocket, 0);
    if (status != NX_WEBSOCKET_INVALID_STATE || client_websocket.nx_websocket_client_mutex.tx_mutex_ownership_count != 0)
        SET_ERROR_COUNTER(&error_counter, __FILE__, __LINE__);
    ...
    else
    {
        ...
        /* Test normal disconnect which shall return success */
        status = nx_websocket_client_disconnect(&client_websocket, 0);
        if (status || client_websocket.nx_websocket_client_mutex.tx_mutex_ownership_count != 0)
            SET_ERROR_COUNTER(&error_counter, __FILE__, __LINE__);
        if (client_websocket.nx_websocket_client_state != NX_WEBSOCKET_CLIENT_STATE_DISCONNECT_SENT)
            SET_ERROR_COUNTER(&error_counter, __FILE__, __LINE__);
        ...
        if (status != NX_WEBSOCKET_DISCONNECTED)
            SET_ERROR_COUNTER(&error_counter, __FILE__, __LINE__);
        if (client_websocket.nx_websocket_client_state != NX_WEBSOCKET_CLIENT_STATE_IDLE)
            SET_ERROR_COUNTER(&error_counter, __FILE__, __LINE__);

        /* Re-disconnect shall fail */
        status = nx_websocket_client_disconnect(&client_websocket, 0);
        if (status != NX_WEBSOCKET_INVALID_STATE || client_websocket.nx_websocket_client_mutex.tx_mutex_ownership_count != 0)
            SET_ERROR_COUNTER(&error_counter, __FILE__, __LINE__);

        /* Send data shall be fine */
        status = nx_packet_allocate(&client_pool, &packet_ptr, NX_TCP_PACKET, NX_IP_PERIODIC_RATE);
        if (status)
            SET_ERROR_COUNTER(&error_counter, __FILE__, __LINE__);
        status = nx_packet_data_append(packet_ptr, client_test_data, sizeof(client_test_data), &client_pool, NX_IP_PERIODIC_RATE);
        if (status)
            SET_ERROR_COUNTER(&error_counter, __FILE__, __LINE__);
        status = nx_websocket_client_send(&client_websocket, packet_ptr, NX_WEBSOCKET_OPCODE_BINARY_FRAME, NX_TRUE, NX_WAIT_FOREVER);
        if (status != NX_WEBSOCKET_NOT_CONNECTED)
            SET_ERROR_COUNTER(&error_counter, __FILE__, __LINE__);
    }
...
NX_PACKET       *packet_ptr;
    ...
    printf("NetX Test:   Websocket Disconnect Test.....................................");
    ...
    for (i = 0; i < TEST_LOOP; i++)
    {
        ...
        else
        {
            ...
            case 2:
                packet_ptr -> nx_packet_append_ptr = packet_ptr -> nx_packet_prepend_ptr;
                packet_ptr -> nx_packet_length = 0;
                nx_packet_data_append(packet_ptr, server_response_2, sizeof(server_response_2), &server_pool, NX_IP_PERIODIC_RATE);
                status = nx_tcp_socket_send(&test_server, packet_ptr, NX_IP_PERIODIC_RATE);
                if (status)
                    SET_ERROR_COUNTER(&error_counter, __FILE__, __LINE__);
                break;
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Test:   Websocket Disconnect Test.....................................N/A\n");
