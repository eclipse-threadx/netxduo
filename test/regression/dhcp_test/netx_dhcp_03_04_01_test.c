/* The servers SHOULD unicast the DHCPACK reply to the address given in the 'ciaddr' filed of the DHCPINFORM message.
 * rfc 2131, page 20, 3.4 Obtaining parameters with externally configured network address
 */
...
#define     CLIENT_IP_ADDRESS           IP_ADDRESS(10,0,0,11)
...
#else
void    netx_dhcp_03_04_01_test_application_define(void *first_unused_memory)
#endif
{
...
UINT    status;
    ...
    status = nx_ip_create(&client_ip, "DHCP Client", CLIENT_IP_ADDRESS, 0xFFFFFF00UL, &client_pool, _nx_ram_network_driver_1500, pointer, 2048, 1);
...
UINT        addresses_added;
    ...
    printf("NetX Test:   DHCP 03_04_01 Test........................................");
    ...
    if((error_counter) || (dhcp_ack_flag == 0))
    {
        printf("ERROR!\n");
...
NX_IP_HEADER   *ip_header;
    ...
    if(src_dst_port == 0x00440043)
    {
        ...
        if(message_type == NX_DHCP_TYPE_DHCPINFORM)
        {
            ...
            inform_ciaddr = dhcp_get_data((packet_ptr -> nx_packet_prepend_ptr + 20 + 8 + NX_BOOTP_OFFSET_CLIENT_IP), 4);

        }
    }
    /* Sent from server to client. */
    else if((src_dst_port == 0x00430044) && (dhcp_inform_count == 1))
    {
         /* Get the dhcp message type. */
        status = dhcp_get_option_value((packet_ptr -> nx_packet_prepend_ptr + 20 + 8), NX_DHCP_SERVER_OPTION_DHCP_TYPE, 
                &message_type, (packet_ptr -> nx_packet_length - 20 - 8));
        if(status)
         ...
         /* Check if the message is a DHCPNAK. */
        if(message_type == NX_DHCP_TYPE_DHCPACK)
        {
            ...
            ack_dst_addr = ip_header -> nx_ip_header_destination_ip;

            NX_CHANGE_ULONG_ENDIAN(ack_dst_addr);

            /* Check if Server unicast the DHCPACK to the address given in the 'ciaddr' field of the DHCPINFORM message. */
            if(ack_dst_addr != inform_ciaddr)
                error_counter++;
...
#endif
{
    ...
    printf("NetX Test:   NetX DHCP 03_04_01 Test...................................N/A\n"); 
