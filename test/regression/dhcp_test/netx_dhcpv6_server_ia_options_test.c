/* This tests dhcpv6 server process ia options from the client.  */
...
#define TEST_VALID_LIFETIME             0x1770
#define TEST_PREFERRED_LIFETIME         0x0BB8
...
#else
void    netx_dhcpv6_server_ia_options_test_application_define(void *first_unused_memory)
#endif
{
...
NX_PACKET   *my_packet;
    ...
    printf("NetX Test:   DHCPv6 Server IA Options Test.........................................");
    ...
    */
    ...
    tx_thread_sleep(NX_IP_PERIODIC_RATE * 6);
...
UCHAR message_type;
UCHAR reserved[3];
static UINT confirm_sent = NX_FALSE;
    ...
    if (ip_ptr == &client_ip)
    {
        ...
        /* Check if it is CONFIRM message from DHCPv6 client.  */
        if (message_type == NX_DHCPV6_MESSAGE_TYPE_CONFIRM)
        {
            confirm_sent = NX_TRUE;
        }
    }
    if (ip_ptr == &server_ip)
    {
        ...
        /* Check if it is REPLY message from DHCPv6 server.  */
        if (message_type == NX_DHCPV6_MESSAGE_TYPE_REPLY)
        {
            if (confirm_sent == NX_TRUE)
            {
                confirm_sent = NX_FALSE;

                if (dhcp_server.nx_dhcpv6_clients[0].nx_dhcpv6_ia.nx_valid_lifetime == TEST_VALID_LIFETIME)
                {
                    return(NX_FALSE);
                }

                if (dhcp_server.nx_dhcpv6_clients[0].nx_dhcpv6_ia.nx_preferred_lifetime == TEST_PREFERRED_LIFETIME)
                {
                    return(NX_FALSE);
                }
            }
...
ULONG          *ip_src_addr = NX_NULL;
ULONG          *ip_dest_addr = NX_NULL;


    udp_header_ptr = (NX_UDP_HEADER *)(packet_ptr -> nx_packet_prepend_ptr);

    /* Get the message type.  */
    message_type = *(packet_ptr -> nx_packet_prepend_ptr + 8);

    /* Check if the message is a CONFIRM. */
    if (message_type == NX_DHCPV6_MESSAGE_TYPE_CONFIRM)
    {

        /* Force change the valid lifetime and preferred lifetime. */
        *(packet_ptr -> nx_packet_prepend_ptr + 74) = 0x0B;
        *(packet_ptr -> nx_packet_prepend_ptr + 75) = 0xB8;
        *(packet_ptr -> nx_packet_prepend_ptr + 78) = 0x17;
        *(packet_ptr -> nx_packet_prepend_ptr + 79) = 0x70;
...
#endif
{
    printf("NetX Test:   DHCPv6 Server IA Options Test...............................N/A\n");
