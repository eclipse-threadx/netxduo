/* The 'request IP address' option MUST be set to the value of 'yiaddr' in the DHCPOFFER message from the server.
...
#else
void    netx_dhcp_03_01_02_test_application_define(void *first_unused_memory)
#endif
{
...
UINT        addresses_added;
    ...
    printf("NetX Test:   DHCP 03_01_02 Test........................................");
...
ULONG   request_ip_addr;
    ...
    if(message_type == NX_DHCP_TYPE_DHCPOFFER)
    {
        ...
        /* Get the "request ip address" option value. */
        status = dhcp_get_option_value((packet_ptr -> nx_packet_prepend_ptr + 8), NX_DHCP_SERVER_OPTION_DHCP_IP_REQ, 
                                       &request_ip_addr, (packet_ptr -> nx_packet_length - 8));
        if(status)
            error_counter++;
        
        /* Get yiaddr field value. */
        yiaddr = dhcp_get_data((packet_ptr -> nx_packet_prepend_ptr + 8 + NX_BOOTP_OFFSET_YOUR_IP), 4);

        /* Check wether they are equal. */
        if(request_ip_addr != yiaddr)
            error_counter++;
        else
            flag = 1; 
...
#endif
{
    ...
    printf("NetX Test:   NetX DHCP 03_01_02 Test...................................N/A\n"); 
