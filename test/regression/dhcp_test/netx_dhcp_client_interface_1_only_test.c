...
/* This is a test of the DHCP Client running on only one interface (secondary interface).
   Required: NX_MAX_PHYSICAL_INTERFACES >= 2 NX_DHCP_CLIENT_MAX_INTERFACES = 1. */
...
#else
void    netx_dhcp_cilent_interface_1_only_test_application_define(void *first_unused_memory)
#endif
{
...
ULONG       requested_ip;
    ...
    /* Note that we call this after setting the request IP, stage change notify and clear broadcast services.
    ...
    /* Try using the multi-interface method to start the client. */
    status =  _nx_dhcp_multi_interface_start(&dhcp_client);
    if (status != NX_DHCP_NOT_FOR_SINGLE_INTERFACES)
        error_counter++;

    /* Start the DHCP Client using the single DHCP Client interface method.  */
    ...
    /* This should release the IP address for the secondary interface. */
    nx_dhcp_release(&dhcp_client);

    /* Try to use the multiple interface method to stop the Client. */
    status = nx_dhcp_client_interface_deactivate(&dhcp_client, 1);
    if (status != NX_DHCP_NOT_FOR_SINGLE_INTERFACES) 
    {
        error_counter++;
    }
    ...
    /* Manually set IP address on secondary interface */    
    status = nx_ip_interface_address_set(&client_ip, 1, IP_ADDRESS(192,1,1,66), 0xFFFFFF00);
    if (status)
        error_counter++;
