...
#else
void    netx_dhcp_03_01_03_test_application_define(void *first_unused_memory)
#endif
{
...
UINT        addresses_added;
    ...
    printf("NetX Test:   DHCP 03_01_03 Test........................................");
...
static ULONG           dhcp_discover_bcd_addr;
static ULONG           dhcp_discover_secs;
ULONG           dhcp_request_bcd_addr;
ULONG           dhcp_request_secs;
UINT            status;
ULONG           message_type;
...
ULONG           src_dst_port;
    ...
    if(src_dst_port == 0x00440043)
    {
        if(dhcp_client.nx_dhcp_interface_record[0].nx_dhcp_state == NX_DHCP_STATE_INIT)
        {
                ...
                dhcp_discover_bcd_addr = ip_header -> nx_ip_header_destination_ip;

                /* Get secs field value. */
                dhcp_discover_secs = dhcp_get_data((packet_ptr -> nx_packet_prepend_ptr + 20 + 8 + NX_BOOTP_OFFSET_SECS), 2);
        }

        /* Make sure in selecting state and dhcpoffer has been received. */
        else if((dhcp_client.nx_dhcp_interface_record[0].nx_dhcp_state == NX_DHCP_STATE_SELECTING) && (dhcp_offer_flag != 0))
        {
...
#endif
                /* Get the  destination ip address. */
                dhcp_request_bcd_addr = ip_header -> nx_ip_header_destination_ip;

                /* Get secs field value. */
                dhcp_request_secs = dhcp_get_data((packet_ptr -> nx_packet_prepend_ptr + 20 + 8 + NX_BOOTP_OFFSET_SECS), 2);

                /* Check if the secs and broadcast address are the same as the original DHCPDISCOVER message. */
                if((dhcp_request_bcd_addr == dhcp_discover_bcd_addr) && (dhcp_request_secs == dhcp_discover_secs))
                    flag = 1;
                else
                    error_counter++;
...
#endif
{
    ...
    printf("NetX Test:   NetX DHCP 03_01_03 Test...................................N/A\n"); 
