...
#if !defined(NX_DISABLE_IPV4) && defined(NX_DHCP_CLIENT_RESTORE_STATE)
...
#else
void    netx_dhcp_basic_restore_test_application_define(void *first_unused_memory)
#endif
{
...
UINT        addresses_added;
    ...
    printf("NetX Test:   DHCP Basic Restore Test...................................");
...
UINT        buffer_size = 4;
ULONG       *long_ptr;
NX_DHCP_CLIENT_RECORD client_record;
ULONG       gateway_address = 0;
ULONG       restore_gateway_address = 0xff;
    ...
    status = nx_ip_gateway_address_get(&client_ip, &gateway_address);
    if (status)
        error_counter++;
#endif

    /* Get the record for restoring.  */
    status = nx_dhcp_client_get_record(&dhcp_client, &client_record);
    if (status)
        error_counter++;
...
#ifdef __PRODUCT_NETXDUO__
    /* Check if gateway address is cleared.  */
    status = nx_ip_gateway_address_get(&client_ip, &restore_gateway_address);
    if (!status)
        error_counter++;
#endif
    ...
    /* Resotr the client record.  */
    status = nx_dhcp_client_restore_record(&dhcp_client, &client_record, 600);
    if (status)
        error_counter++;
...
#ifdef __PRODUCT_NETXDUO__
    /* Check if the gateway address is retored.  */
    status = nx_ip_gateway_address_get(&client_ip, &restore_gateway_address);
    if (status || gateway_address != restore_gateway_address)
        error_counter++;
#endif
...
#endif
{
    ...
    printf("NetX Test:   NetX DHCP Basic Restore Test..............................N/A\n"); 
