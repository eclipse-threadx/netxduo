...
#else
void    netx_dhcp_arpprobe_fail_multiple_interface_test_application_define(void *first_unused_memory)
#endif
{
...
UINT status;
    ...
    {  
          ...
          /* Wait a bit for DHCP Clien to decline the IP address. */
          tx_thread_sleep(50);
...
UINT         i, j, k, pkt_number;
    ...
    status = send_arp_reply(&server0_ip, 1, dhcp_client.nx_dhcp_interface_record[1].nx_dhcp_ip_address);
