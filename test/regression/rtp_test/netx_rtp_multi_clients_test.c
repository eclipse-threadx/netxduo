...
#define RTP_CLIENT_1_RTP_PORT      6002
#define RTP_CLIENT_1_RTCP_PORT     6003
#define RTP_CLIENT_2_RTP_PORT      6002
#define RTP_CLIENT_2_RTCP_PORT     6003
...
UINT        status;
    ...
    printf("NetX Test:   RTP Multi-Clients Test............................................");
...
{
UINT        status;
NXD_ADDRESS client_1_ip_address;
NXD_ADDRESS client_2_ip_address;
...
UINT        time_start;
    ...
    client_1_ip_address.nxd_ip_version = NX_IP_VERSION_V4;
    client_1_ip_address.nxd_ip_address.v4 = RTP_CLIENT_1_ADDRESS;
    status = nx_rtp_sender_session_create(&rtp_0, &rtp_session_0, RTP_PAYLOAD_TYPE,
                                          0, &client_1_ip_address,
                                          RTP_CLIENT_1_RTP_PORT, RTP_CLIENT_1_RTCP_PORT);
    CHECK_STATUS(0, status);

    /* Setup rtp sender session.  */
    client_2_ip_address.nxd_ip_version = NX_IP_VERSION_V4;
    client_2_ip_address.nxd_ip_address.v4 = RTP_CLIENT_2_ADDRESS;
    status = nx_rtp_sender_session_create(&rtp_0, &rtp_session_1, RTP_PAYLOAD_TYPE,
                                          0, &client_2_ip_address,
                                          RTP_CLIENT_2_RTP_PORT, RTP_CLIENT_2_RTCP_PORT);
    CHECK_STATUS(0, status);
    ...
    for (UINT i = 0; i < TEST_CYCLES; i++)
    {
        ...
        /* Allocate a packet */
        status = nx_rtp_sender_session_packet_allocate(&rtp_session_1, &send_packet, 5 * NX_IP_PERIODIC_RATE);
        CHECK_STATUS(0, status);
        ...
        status = nx_rtp_sender_session_packet_send(&rtp_session_1, send_packet, TEST_TIMESTAMP, TEST_MSW, TEST_LSW, 1);
        CHECK_STATUS(0, status);
    }
...
UCHAR     *data;
    ...
    status =  nx_udp_socket_bind(&rtp_client_1_socket, RTP_CLIENT_1_RTP_PORT, NX_IP_PERIODIC_RATE);
    CHECK_STATUS(0, status);
...
UCHAR     *data;
    ...
    status =  nx_udp_socket_bind(&rtp_client_2_socket, RTP_CLIENT_2_RTP_PORT, NX_IP_PERIODIC_RATE);
    CHECK_STATUS(0, status);
    ...
    for (UINT i = 0; i < TEST_CYCLES; i++)
    {
        ...
        /* Move to check RTP data bytes for sequence number */
        data++;
        CHECK_STATUS((rtp_session_1.nx_rtp_session_sequence_number - 1), (data[0] << 8 | data[1]));

        /* Move to check RTP data bytes for time stamp */
        data += 2;
        CHECK_STATUS(rtp_session_1.nx_rtp_session_rtp_timestamp, (ULONG)(data[0] << 24 | data[1] << 16 | data[2] << 8 | data[3]));

        /* Move to check RTP data bytes for ssrc */
        data += 4;
        CHECK_STATUS(rtp_session_1.nx_rtp_session_ssrc, (ULONG)(data[0] << 24 | data[1] << 16 | data[2] << 8 | data[3]));
...
#endif
{
    ...
    /* Print out test information banner.  */
    printf("NetX Test:   RTP Multi-Clients Test............................................N/A\n");
