...
#
# Test script for the PTP Win64/VS 2022 regression test suite.
# AI Disclosure: portions of this file were generated with Copilot (Claude Sonnet 4.6).
# Author: Frédéric Desbiens
#
# Usage:
#   .\test_ptp.ps1 [-Arch win64] [-Configuration all|<name>] [-BuildDir <path>]
...
if (-not $BuildDir) {
    $BuildDir = Join-Path $repoRoot "build\tests\$Arch\ptp"
}
...
foreach ($currentConfiguration in $selectedConfigurations) {
    ...
    try {
        ...
        if (-not (Test-Path -LiteralPath $currentBuildDir)) {
            throw "Build directory does not exist for $Arch / ptp / ${currentConfiguration}: $currentBuildDir"
        }
        ...
        Write-Host "Testing $Arch / ptp / $currentConfiguration"
        $ctestArguments = @(
