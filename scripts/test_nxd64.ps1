...
#
# Test script for the NetX Duo 64-bit Win64/VS 2022 regression test suite.
# AI Disclosure: portions of this file were generated with Copilot (Claude Sonnet 4.6).
# Author: Frédéric Desbiens
#
# Usage:
#   .\test_nxd64.ps1 [-Arch win64] [-Configuration all|<name>] [-BuildDir <path>]
...
foreach ($currentConfiguration in $selectedConfigurations) {
    ...
    try {
        ...
        if (-not (Test-Path -LiteralPath $currentBuildDir)) {
            throw "Build directory does not exist for $Arch / netxduo64 / ${currentConfiguration}: $currentBuildDir"
        }
        ...
        Write-Host "Testing $Arch / netxduo64 / $currentConfiguration"
        $ctestArguments = @(
