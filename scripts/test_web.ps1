...
#
# Test script for the Web Win64/VS 2022 regression test suite.
# AI Disclosure: portions of this file were generated with Copilot (Claude Sonnet 4.6).
# Author: Frédéric Desbiens
#
# Usage:
#   .\test_web.ps1 [-Arch win64] [-Configuration all|<name>] [-BuildDir <path>]
...
foreach ($currentConfiguration in $selectedConfigurations) {
    ...
    try {
        ...
        if (-not (Test-Path -LiteralPath $currentBuildDir)) {
            throw "Build directory does not exist for $Arch / web / ${currentConfiguration}: $currentBuildDir"
        }
        ...
        Write-Host "Testing $Arch / web / $currentConfiguration"
        $ctestArguments = @(
