...
#
# Build script for the Web Win64/VS 2022 regression test suite.
# AI Disclosure: portions of this file were generated with Copilot (Claude Sonnet 4.6).
# Author: Frédéric Desbiens
#
# Usage:
#   .\build_web.ps1 [-Arch win64] [-Configuration all|<name>] [-ThreadXDir <path>]
...
if (-not $BuildDir) {
    $BuildDir = Join-Path $repoRoot "build\tests\$Arch\web"
}
...
foreach ($currentConfiguration in $selectedConfigurations) {
    ...
    Write-Host "Configuring $Arch / web / $currentConfiguration"
    Invoke-NativeCommand -FilePath 'cmake' -Arguments @(
        '-S', (Join-Path $repoRoot 'test\cmake\web'),
        '-B', $currentBuildDir,
        '-G', 'Ninja',
        ...
        # and crashes the web tests (access violation) on the win64 simulator.
        "-DNX_USER_FILE=$(Join-Path $repoRoot "ports\$($settings.NetXArch)\$($settings.NetXToolchain)\inc\nx_user_win.h")"
    )

    Write-Host "Building $Arch / web / $currentConfiguration"
