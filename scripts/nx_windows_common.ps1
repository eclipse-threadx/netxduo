...
function Invoke-CMakeBuild {
    ...
    Write-Warning "$buildToolName build timed out after $TimeoutSeconds seconds in $BuildDir. Retrying the interrupted Ninja build."
    ...
    if (-not $retryResult.Completed) {
        throw "$buildToolName build timed out again after $TimeoutSeconds seconds in $BuildDir"
    }

    throw "$buildToolName retry failed with exit code $($retryResult.ExitCode) in $BuildDir"
}
