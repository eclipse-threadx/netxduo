...
#
# Build script for the PTP Win64/VS 2022 regression test suite.
# AI Disclosure: portions of this file were generated with Copilot (Claude Sonnet 4.6).
# Author: Frédéric Desbiens
#
# Usage:
#   .\build_ptp.ps1 [-Arch win64] [-Configuration all|<name>] [-ThreadXDir <path>]
...
foreach ($currentConfiguration in $selectedConfigurations) {
    ...
    Write-Host "Configuring $Arch / ptp / $currentConfiguration"
    Invoke-NativeCommand -FilePath 'cmake' -Arguments @(
        '-S', (Join-Path $repoRoot 'test\cmake\ptp'),
        '-B', $currentBuildDir,
        '-G', 'Ninja',
